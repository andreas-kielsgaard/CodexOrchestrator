...
fn reconcile_one(c: &mut Connection, r: &Row) -> Result<(), String> {
    let existing = integration(c, &r.candidate)?;
    if existing.as_ref().is_some_and(|integration| integration.stage == "attention") { return Ok(()) }
    revalidate(r, c)?;
    // Reservation is inside the target lock, so concurrent candidates cannot bind the same
    // mutable parent before either one advances it.
    let _lock = Lock::take(&r.common, &target(c, &r.authority)?.reference)?;
    let mut i = match existing {
        Some(integration) => integration,
        None => {
            reserve(c, r)?;
            integration(c, &r.candidate)?.ok_or("integration_missing")?
        }
    };
