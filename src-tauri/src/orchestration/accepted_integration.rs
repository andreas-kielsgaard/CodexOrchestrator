...
#[cfg(test)] fn test_target_lock_queue()->&'static Mutex<Option<TestTargetLockQueue>>{static QUEUE:OnceLock<Mutex<Option<TestTargetLockQueue>>>=OnceLock::new();QUEUE.get_or_init(||Mutex::new(None))}
struct Lock{_file:File} impl Lock{fn take(common:&Path,reference:&str)->Result<Self,String>{let file=OpenOptions::new().read(true).write(true).create(true).open(common.join(format!("codex-orchestrator-{}.lock",stable_id("target-ref-lock",reference)))).map_err(|_|"target_lock_unavailable".to_string())?;#[cfg(test)]{let entered=test_target_lock_queue().lock().ok().and_then(|queue|queue.as_ref().filter(|queue|queue.common==common).map(|queue|queue.entered.clone()));if let Some(entered)=entered{entered.wait();file.lock().map_err(|_|"target_lock_unavailable".to_string())?;return Ok(Self{_file:file})}}file.try_lock().map_err(|_|"target_lock_unavailable".to_string())?;Ok(Self{_file:file})}}
...
#[cfg(test)] mod tests { use super::*; use std::sync::{Arc,Barrier}; use tempfile::TempDir;
 ...
 #[test] fn lock_contention_stays_pending_then_converges(){let(_d,mut c,row)=fixture();let held=Lock::take(&row.common,"refs/heads/main").unwrap();reconcile_accepted_integrations(&mut c).unwrap();assert_eq!(c.query_row::<i64,_,_>("SELECT COUNT(*) FROM accepted_work_unit_integrations",[],|r|r.get(0)).unwrap(),0);drop(held);reconcile_accepted_integrations(&mut c).unwrap();assert_eq!(c.query_row::<String,_,_>("SELECT stage FROM accepted_work_unit_integrations",[],|r|r.get(0)).unwrap(),"settled");}
 ...
 #[test] fn queued_same_candidate_connections_replay_one_settled_integration(){let(_d,path,row)=file_fixture();let c=Connection::open(&path).unwrap();initialize_accepted_integration_schema(&c).unwrap();drop(c);let held=Lock::take(&row.common,"refs/heads/main").unwrap();let entered=Arc::new(Barrier::new(3));*test_target_lock_queue().lock().unwrap()=Some(TestTargetLockQueue{common:row.common.clone(),entered:entered.clone()});let calls=(0..2).map(|_|{let path=path.clone();let row=row.clone();std::thread::spawn(move||{let mut c=Connection::open(path).unwrap();c.busy_timeout(std::time::Duration::from_secs(5)).unwrap();reconcile_one(&mut c,&row)})}).collect::<Vec<_>>();entered.wait();drop(held);for call in calls{assert!(call.join().unwrap().is_ok())}*test_target_lock_queue().lock().unwrap()=None;let mut replay=Connection::open(&path).unwrap();reconcile_accepted_integrations(&mut replay).unwrap();assert_eq!(replay.query_row::<i64,_,_>("SELECT COUNT(*) FROM accepted_work_unit_integrations",[],|r|r.get(0)).unwrap(),1);assert_eq!(replay.query_row::<i64,_,_>("SELECT COUNT(*) FROM accepted_work_unit_integration_evidence",[],|r|r.get(0)).unwrap(),1);assert_eq!(replay.query_row::<i64,_,_>("SELECT COUNT(*) FROM work_unit_settlements",[],|r|r.get(0)).unwrap(),1);assert_eq!(replay.query_row::<i64,_,_>("SELECT COUNT(*) FROM work_unit_prerequisite_contributions",[],|r|r.get(0)).unwrap(),1);assert_eq!(replay.query_row::<(String,Option<String>),_,_>("SELECT stage,attention_code FROM accepted_work_unit_integrations",[],|r|Ok((r.get(0)?,r.get(1)?))).unwrap(),("settled".into(),None));let mut third=Connection::open(&path).unwrap();reconcile_accepted_integrations(&mut third).unwrap();assert_eq!(third.query_row::<i64,_,_>("SELECT COUNT(*) FROM accepted_work_unit_integrations",[],|r|r.get(0)).unwrap(),1);assert_eq!(third.query_row::<i64,_,_>("SELECT COUNT(*) FROM accepted_work_unit_integration_evidence",[],|r|r.get(0)).unwrap(),1);assert_eq!(third.query_row::<i64,_,_>("SELECT COUNT(*) FROM work_unit_settlements",[],|r|r.get(0)).unwrap(),1);assert_eq!(third.query_row::<i64,_,_>("SELECT COUNT(*) FROM work_unit_prerequisite_contributions",[],|r|r.get(0)).unwrap(),1);assert_eq!(git(&row.repo,&["rev-list","--count","HEAD"]).unwrap(),"2");}
 #[test] fn two_concurrent_candidates_reserve_distinct_serialized_target_parents(){let(_d,path,row)=file_fixture();run(&row.repo,&["checkout","-b","candidate-two",&row.baseline]);fs::write(row.repo.join("b.txt"),"b\n").unwrap();run(&row.repo,&["add","."]);run(&row.repo,&["commit","-m","candidate two"]);let candidate_two=git(&row.repo,&["rev-parse","HEAD"]).unwrap();let tree_two=git(&row.repo,&["rev-parse","HEAD^{tree}"]).unwrap();run(&row.repo,&["update-ref","refs/codex/orchestrator/accepted/candidate-two",&candidate_two]);run(&row.repo,&["checkout","main"]);let c=Connection::open(&path).unwrap();c.execute("INSERT INTO work_units VALUES('unit-2'),('dependent-2')",[]).unwrap();c.execute("INSERT INTO work_unit_relationships VALUES('edge-two','depends_on','dependent-2','unit-2')",[]).unwrap();c.execute("INSERT INTO accepted_handler_candidates VALUES('candidate-two','unit-2','authority',?1,NULL,?2,?3,?4,'refs/codex/orchestrator/accepted/candidate-two','evidence-two')",params![now(),row.baseline,candidate_two,tree_two]).unwrap();drop(c);let barrier=Arc::new(Barrier::new(3));let calls=(0..2).map(|_|{let path=path.clone();let barrier=barrier.clone();std::thread::spawn(move||{let mut c=Connection::open(path).unwrap();c.busy_timeout(std::time::Duration::from_millis(50)).unwrap();barrier.wait();reconcile_accepted_integrations(&mut c)})}).collect::<Vec<_>>();barrier.wait();for call in calls{assert!(call.join().unwrap().is_ok())}let mut reopened=Connection::open(&path).unwrap();reconcile_accepted_integrations(&mut reopened).unwrap();let commits=reopened.prepare("SELECT integration_commit_id FROM accepted_work_unit_integrations ORDER BY integration_commit_id").unwrap().query_map([],|r|r.get::<_,String>(0)).unwrap().collect::<Result<Vec<_>,_>>().unwrap();assert_eq!(commits.len(),2);assert_eq!(git(&row.repo,&["rev-list","--count","HEAD"]).unwrap(),"3");assert_eq!(reopened.query_row::<i64,_,_>("SELECT COUNT(*) FROM accepted_work_unit_integrations WHERE stage='settled' AND attention_code IS NULL",[],|r|r.get(0)).unwrap(),2);assert_eq!(reopened.query_row::<i64,_,_>("SELECT COUNT(*) FROM work_unit_settlements",[],|r|r.get(0)).unwrap(),2);assert_eq!(reopened.query_row::<i64,_,_>("SELECT COUNT(*) FROM work_unit_prerequisite_contributions",[],|r|r.get(0)).unwrap(),2);}
