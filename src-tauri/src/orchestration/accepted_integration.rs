...
use super::accepted_candidate_authority::{
    revalidate_retained_accepted_candidate, sprint_target_binding_fingerprint,
};
...
pub(crate) fn reconcile_accepted_integrations(c: &mut Connection) -> Result<(), String> {
    ...
    for candidate in candidates {
        let Some(row) = load_row(c, &candidate)? else {
            continue;
        };
        if let Err(code) = reconcile_one(c, &row) {
            if !retryable(&code) {
                attention(c, &row.candidate, &code)?;
            }
        }
    }
    Ok(())
...
pub(crate) fn initialize_accepted_integration_schema(c: &Connection) -> Result<(), String> {
    c.execute_batch(ACCEPTED_INTEGRATION_SCHEMA)
    ...
    for (name, definition) in [
        ...
        if !columns.iter().any(|column| column == name) {
            c.execute_batch(&format!(
                "ALTER TABLE accepted_work_unit_integrations ADD COLUMN {name} {definition}"
            ))
            .map_err(|e| e.to_string())?;
        }
    }
...
fn reconcile_one(c: &mut Connection, r: &Row) -> Result<(), String> {
    ...
    match integration(c, &r.candidate)? {
    ...
    // Reservation records no Git effect. Revalidate only after the post-lock durable identity is
    // chosen, so invalid retained lineage still settles fail-closed as owned attention.
    revalidate(r, c)?;
    let mut i = integration(c, &r.candidate)?.ok_or("integration_missing")?;
    if i.stage == "attention" {
        return Ok(());
    }
    validate_integration_correlations(c, r, &i)?;
    let t = target(c, &r.authority)?; // reload after acquiring the inter-process lock
    if i.settled.is_some() || i.stage == "settled" {
        verify_settled(c, r, &i, &t)?;
        return Ok(());
    }
    let commit = match i.commit.clone() {
        Some(value) => {
            verify_commit(r, &i, &value)?;
            value
        }
        None => {
...
fn validate_integration_correlations(
    c: &Connection,
    r: &Row,
    i: &Integration,
) -> Result<(), String> {
    ...
    if exact && i.id == id && i.reference == target.reference && i.intent == intent {
        Ok(())
    } else {
        Err("durable_integration_correlation_mismatch".into())
    }
}

fn recover_and_advance(
    c: &mut Connection,
    r: &Row,
    i: &Integration,
    commit: &str,
    ...
    match i.stage.as_str() {
        "intent_reserved" | "object_created" => {
            if git(
                &r.worktree,
                &["show-ref", "--verify", "--hash", &t.reference],
            )? == commit
            {
    ...
    if ref_value == i.pre {
        git(&r.worktree, &["update-ref", &t.reference, commit, &i.pre])
            .map_err(|_| "target_ref_cas_lost".to_owned())?;
    } else if ref_value != commit {
        return Err("target_ref_advanced_or_foreign".into());
    }
...
fn require_pre_state(r: &Row, t: &Target, i: &Integration) -> Result<(), String> {
    ...
    if t.current != i.pre
        || t.version != i.version
        || t.binding != sprint_target_binding_fingerprint(&r.authority, &t.reference, &i.pre)
    {
        return Err("target_db_pre_state_mismatch".into());
    }
    if git(&r.worktree, &["symbolic-ref", "--quiet", "HEAD"])? != t.reference
        || git(&r.worktree, &["rev-parse", "HEAD^{commit}"])? != i.pre
        || git(
            &r.worktree,
            &["show-ref", "--verify", "--hash", &t.reference],
        )? != i.pre
    ...
    {
        return Err("target_ref_or_runtime_drift".into());
    }
    Ok(())
}

fn require_ref_and_runtime(
    r: &Row,
    i: &Integration,
    commit: &str,
    t: &Target,
    ...
    if git(&r.worktree, &["symbolic-ref", "--quiet", "HEAD"])? != t.reference
        ...
        || git(&r.worktree, &["rev-parse", "HEAD^{commit}"])? != commit
        || git(&r.worktree, &["status", "--porcelain"])? != ""
    {
        return Err("advanced_target_runtime_mismatch".into());
    }
    if db_advanced {
        if t.current != commit || t.version != i.version + 1 || t.binding != expected_binding {
            return Err("advanced_target_db_mismatch".into());
        }
    } else if t.current != i.pre
        || t.version != i.version
        || t.binding != sprint_target_binding_fingerprint(&r.authority, &t.reference, &i.pre)
    {
        return Err("advanced_target_db_pre_mismatch".into());
    }
    Ok(())
}
...
fn require_ref_advanced(r: &Row, i: &Integration, commit: &str, t: &Target) -> Result<(), String> {
    ...
    if git(&r.worktree, &["symbolic-ref", "--quiet", "HEAD"])? != t.reference
        ...
        || git(&r.worktree, &["rev-parse", "HEAD^{commit}"])? != commit
    {
        return Err("ref_advanced_target_mismatch".into());
    }
    let pre_tree = git(&r.repo, &["rev-parse", &format!("{}^{{tree}}", i.pre)])?;
    if git(&r.worktree, &["write-tree"])? != pre_tree
        || git(&r.worktree, &["diff-files", "--quiet"]).is_err()
    {
        return Err("ref_advanced_runtime_foreign".into());
    }
    Ok(())
}

fn validate_identity(r: &Row, t: &Target) -> Result<(), String> {
    if !safe_ref(&t.reference) || !oid(&r.baseline) || !oid(&r.commit) || !oid(&t.current) {
        return Err("unsafe_durable_identity".into());
    }
    if canon(&r.repo)? != git_path(&r.repo, "--show-toplevel")?
        || canon(&r.worktree)? != git_path(&r.worktree, "--show-toplevel")?
        || canon(&r.common)? != git_path(&r.worktree, "--git-common-dir")?
    {
        return Err("target_common_dir_drift".into());
    }
    if git(&r.repo, &["show-ref", "--verify", "--hash", &r.private_ref])? != r.commit
        || git(&r.repo, &["rev-parse", &format!("{}^{{tree}}", r.commit)])? != r.tree
        || git(
            &r.repo,
            &["merge-base", "--is-ancestor", &r.baseline, &r.commit],
        )
        .is_err()
    {
        return Err("candidate_private_ref_drift".into());
    }
    Ok(())
}

fn revalidate(r: &Row, c: &mut Connection) -> Result<(), String> {
    revalidate_retained_accepted_candidate(c, &r.candidate)?;
    Ok(())
}
...
fn advance_database(
    ...
    if changed == 0 {
        let exact: bool = tx.query_row("SELECT EXISTS(SELECT 1 FROM sprint_target_currents WHERE authority_id=?1 AND current_object_id=?2 AND binding_fingerprint=?3 AND version=?4)",params![r.authority,commit,binding,i.version+1],|x|x.get(0)).map_err(|e|e.to_string())?;
        if !exact {
            return Err("target_current_cas_lost".into());
        }
    }
...
fn persist_evidence_and_settlement(
    c: &mut Connection,
    r: &Row,
    i: &Integration,
    commit: &str,
    ...
    let actual_contributions: i64 = tx
        .query_row(
            "SELECT COUNT(*) FROM work_unit_prerequisite_contributions WHERE integration_id=?1",
            [&i.id],
            |x| x.get(0),
        )
        .map_err(|e| e.to_string())?;
    ...
    if actual_contributions != expected_contributions || divergent_contribution {
        return Err("durable_replay_conflict".into());
    }
...
fn exact_insert(
    ...
    exact: &str,
    exact_values: impl rusqlite::Params,
) -> Result<(), String> {
    match tx.execute(insert, values) {
        Ok(_) => Ok(()),
        Err(rusqlite::Error::SqliteFailure(error, _))
            if error.code == rusqlite::ErrorCode::ConstraintViolation =>
        {
            let is_exact: bool = tx
                .query_row(exact, exact_values, |r| r.get(0))
                .map_err(|e| e.to_string())?;
            if is_exact {
                Ok(())
            } else {
                Err("durable_replay_conflict".into())
            }
        }
...
fn verify_settled(c: &mut Connection, r: &Row, i: &Integration, t: &Target) -> Result<(), String> {
    ...
    validate_identity(r, t)?;
    verify_commit(r, i, commit)?;
    if t.reference != i.reference
        || t.binding != sprint_target_binding_fingerprint(&r.authority, &t.reference, &t.current)
        || git(&r.worktree, &["symbolic-ref", "--quiet", "HEAD"])? != t.reference
        || git(
            &r.worktree,
            &["show-ref", "--verify", "--hash", &t.reference],
        )? != t.current
        || git(&r.worktree, &["rev-parse", "HEAD^{commit}"])? != t.current
        || git(&r.worktree, &["status", "--porcelain"])? != ""
        || git(
            &r.repo,
            &["merge-base", "--is-ancestor", commit, &t.current],
        )
        .is_err()
    {
        return Err("settled_target_line_mismatch".into());
    }
    let tree = git(&r.repo, &["rev-parse", &format!("{commit}^{{tree}}")])?;
    let evidence = fingerprint(&[
        POLICY_VERSION,
    ...
    let actual_contributions: i64 = c
        .query_row(
            "SELECT COUNT(*) FROM work_unit_prerequisite_contributions WHERE integration_id=?1",
            [&i.id],
            |x| x.get(0),
    ...
    if !exact_evidence
        ...
        || notification_intent.is_none()
    {
        return Err("settled_evidence_or_settlement_missing".into());
    }
    Ok(())
}

fn merged_tree(r: &Row, pre: &str) -> Result<String, String> {
    let dir = std::env::temp_dir().join(format!("codex-integration-{}", uuid::Uuid::new_v4()));
    fs::create_dir_all(&dir).map_err(|_| "temporary_index_unavailable".to_string())?;
    let index = dir.join("index");
    let result = git_index(
        &r.repo,
        &["read-tree", "-m", &r.baseline, pre, &r.commit],
        &index,
    )
    .and_then(|_| git_index(&r.repo, &["write-tree"], &index));
    let _ = fs::remove_dir_all(dir);
    result.map_err(|_| "integration_conflict".into())
}

fn create_commit(r: &Row, i: &Integration, tree: &str) -> Result<(String, String), String> {
    let (name, email, date) = author(r)?;
    let fingerprint = commit_fingerprint(r, i, tree, &name, &email, &date);
    let message = commit_message(r, i, tree, &fingerprint);
    let mut process = Command::new("git")
        .args(["--no-replace-objects", "commit-tree", tree, "-p", &i.pre])
        .current_dir(&r.repo)
        ...
        .env("GIT_COMMITTER_DATE", &i.recorded)
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
    ...
    process
        ...
        .map_err(|_| "git_command_failed".to_string())?;
    let out = process
        .wait_with_output()
        .map_err(|_| "git_command_failed".to_string())?;
    if out.status.success() {
        Ok((decode_git(out.stdout)?.trim().to_owned(), fingerprint))
    } else {
        Err("git_command_failed".into())
    }
}

fn author(r: &Row) -> Result<(String, String, String), String> {
    let value = git(
        &r.repo,
        &["show", "-s", "--format=%an%x00%ae%x00%aI", &r.commit],
    )?;
    let mut parts = value.split('\0');
    Ok((
        parts.next().ok_or("candidate_author_invalid")?.to_owned(),
        parts.next().ok_or("candidate_author_invalid")?.to_owned(),
        parts.next().ok_or("candidate_author_invalid")?.to_owned(),
    ))
}
...
fn commit_message(r: &Row, i: &Integration, tree: &str, fingerprint: &str) -> String {
    format!("Codex Orchestrator accepted Work Unit integration v1\n\nPolicy-Version: {POLICY_VERSION}\nIntegration: {}\nIntent: {}\nEvidence: {}\nCandidate: {}\nCandidate-Commit: {}\nCandidate-Tree: {}\nBaseline: {}\nWork-Unit: {}\nAuthority: {}\nTarget-Ref: {}\nPre-Object: {}\nResult-Tree: {tree}\nFingerprint: {fingerprint}\n",i.id,i.intent,r.evidence,r.candidate,r.commit,r.tree,r.baseline,r.unit,r.authority,i.reference,i.pre)
}
fn verify_commit(r: &Row, i: &Integration, commit: &str) -> Result<(), String> {
    let tree = i.tree.as_deref().ok_or("integration_tree_missing")?;
    let fingerprint = i
        .fingerprint
        .as_deref()
        .ok_or("integration_fingerprint_missing")?;
    verify_commit_parts(r, i, commit, tree, fingerprint)
}
fn verify_commit_parts(
    r: &Row,
    i: &Integration,
    commit: &str,
    tree: &str,
    ...
    if git(&r.repo, &["rev-list", "--parents", "-n", "1", commit])? != format!("{commit} {}", i.pre)
        || git(&r.repo, &["rev-parse", &format!("{commit}^{{tree}}")])? != tree
        || tree != merged_tree(r, &i.pre)?
    {
        return Err("integration_object_mismatch".into());
    }
    let (name, email, date) = author(r)?;
    if fingerprint_value != commit_fingerprint(r, i, tree, &name, &email, &date) {
        return Err("integration_fingerprint_mismatch".into());
    }
    let actual = git(
        &r.repo,
        &[
    ...
    let mut p = actual.split('\0');
    let fields = [p.next(), p.next(), p.next(), p.next(), p.next(), p.next()];
    if fields
        != [
            Some(name.as_str()),
            Some(email.as_str()),
            ...
            Some(i.recorded.as_str()),
        ]
    {
        return Err("integration_identity_metadata_mismatch".into());
    }
    let expected = commit_message(r, i, tree, fingerprint_value);
    if p.next().unwrap_or("").trim_end() != expected.trim_end() {
        return Err("integration_message_mismatch".into());
    }
    Ok(())
}

fn attention(c: &mut Connection, candidate: &str, code: &str) -> Result<(), String> {
    c.execute("UPDATE accepted_work_unit_integrations SET attention_code=COALESCE(attention_code,?2),attention_recorded_at=COALESCE(attention_recorded_at,?3),stage='attention' WHERE candidate_id=?1",params![candidate,code,now()]).map_err(|e|e.to_string())?;
    Ok(())
}
fn retryable(code: &str) -> bool {
    matches!(
        code,
        "target_lock_unavailable" | "database is locked" | "database is busy"
    ) || code.contains("database is locked")
        || code.contains("database is busy")
}
#[cfg(test)]
use std::sync::{Arc, Barrier, Mutex, OnceLock};
#[cfg(test)]
struct TestTargetLockQueue {
    common: PathBuf,
    entered: Arc<Barrier>,
}
#[cfg(test)]
fn test_target_lock_queue() -> &'static Mutex<Option<TestTargetLockQueue>> {
    static QUEUE: OnceLock<Mutex<Option<TestTargetLockQueue>>> = OnceLock::new();
...
impl Lock {
    fn take(common: &Path, reference: &str) -> Result<Self, String> {
        let file = OpenOptions::new()
            .read(true)
            .write(true)
            .create(true)
            .open(common.join(format!(
                "codex-orchestrator-{}.lock",
                stable_id("target-ref-lock", reference)
            )))
            .map_err(|_| "target_lock_unavailable".to_string())?;
        #[cfg(test)]
        {
            let entered = test_target_lock_queue().lock().ok().and_then(|queue| {
                queue
                    .as_ref()
                    .filter(|queue| queue.common == common)
                    .map(|queue| queue.entered.clone())
            });
...
fn decode_git(bytes: Vec<u8>) -> Result<String, String> {
...
fn git(root: &Path, args: &[&str]) -> Result<String, String> {
    let out = Command::new("git")
        ...
        .map_err(|_| "git_unavailable".to_string())?;
    if out.status.success() {
        Ok(decode_git(out.stdout)?.trim().to_owned())
    } else {
        Err("git_command_failed".into())
    }
}
fn git_path(root: &Path, arg: &str) -> Result<String, String> {
    let path = PathBuf::from(git(root, &["rev-parse", arg])?);
...
fn git_index(root: &Path, args: &[&str], index: &Path) -> Result<String, String> {
    let out = Command::new("git")
        .arg("--no-replace-objects")
        .args(args)
...
fn canon(path: &Path) -> Result<String, String> {
    path.canonicalize()
...
fn stable_id(domain: &str, value: &str) -> String {
    fingerprint(&[domain, value])[..32].to_owned()
}
...
mod tests {
    ...
    fn fixture() -> (TempDir, Connection, Row) {
        ...
        .unwrap();
        let time = now();
        c.execute("INSERT INTO accepted_handler_candidates VALUES('candidate','unit','authority',?1,NULL,?2,?3,?4,'refs/codex/orchestrator/accepted/candidate','evidence')",params![time,baseline,candidate,tree]).unwrap();
        let binding = sprint_target_binding_fingerprint("authority", "refs/heads/main", &baseline);
        c.execute("INSERT INTO sprint_target_currents VALUES('authority','refs/heads/main',?1,?2,1,NULL,?3)",params![baseline,binding,now()]).unwrap();
        let row = Row {
    ...
    fn file_fixture() -> (TempDir, PathBuf, Row) {
        let dir = TempDir::new().unwrap();
        ...
        c.busy_timeout(std::time::Duration::from_millis(50))
            .unwrap();
        c.execute_batch("PRAGMA journal_mode=WAL;CREATE TABLE work_units(work_unit_id TEXT PRIMARY KEY);CREATE TABLE initiated_sprint_git_authorities(authority_id TEXT PRIMARY KEY,repository_root TEXT,repository_common_dir TEXT,worktree_root TEXT,baseline_object_id TEXT);CREATE TABLE accepted_handler_candidates(candidate_id TEXT PRIMARY KEY,work_unit_id TEXT,authority_id TEXT,pinned_at TEXT,attention_reason TEXT,attempt_baseline_object_id TEXT,candidate_commit_id TEXT,candidate_tree_id TEXT,private_ref_name TEXT,evidence_fingerprint TEXT);CREATE TABLE sprint_target_currents(authority_id TEXT PRIMARY KEY,target_ref_name TEXT,current_object_id TEXT,binding_fingerprint TEXT,version INTEGER,attention_reason TEXT,updated_at TEXT);CREATE TABLE work_unit_relationships(relationship_id TEXT PRIMARY KEY,relationship_kind TEXT,from_id TEXT,to_id TEXT);INSERT INTO work_units VALUES('unit'),('dependent');").unwrap();
        let common = root.join(".git");
        c.execute(
            "INSERT INTO initiated_sprint_git_authorities VALUES('authority',?1,?2,?1,?3)",
            params![root.to_string_lossy(), common.to_string_lossy(), baseline],
        ...
        .unwrap();
        c.execute("INSERT INTO accepted_handler_candidates VALUES('candidate','unit','authority',?1,NULL,?2,?3,?4,'refs/codex/orchestrator/accepted/candidate','evidence')",params![now(),baseline,candidate,tree]).unwrap();
        let binding = sprint_target_binding_fingerprint("authority", "refs/heads/main", &baseline);
        c.execute("INSERT INTO sprint_target_currents VALUES('authority','refs/heads/main',?1,?2,1,NULL,?3)",params![baseline,binding,now()]).unwrap();
        c.execute(
    ...
    #[test]
    fn candidate_initial_pin_and_exact_commit_adoption() {
        let (_d, mut c, row) = fixture();
        reconcile_accepted_integrations(&mut c).unwrap();
        let attention: Option<String> = c
            .query_row(
                "SELECT attention_code FROM accepted_work_unit_integrations",
                [],
                |r| r.get(0),
            )
            .unwrap();
        assert!(attention.is_none(), "attention={attention:?}");
        let(commit,tree):(String,String)=c.query_row("SELECT integration_commit_id,integration_tree_id FROM accepted_work_unit_integrations",[],|r|Ok((r.get(0)?,r.get(1)?))).unwrap();
        assert_eq!(tree, row.tree);
        assert_eq!(
            git(&row.repo, &["rev-list", "--parents", "-n", "1", &commit]).unwrap(),
            format!("{commit} {}", row.baseline)
        );
        assert_eq!(
            c.query_row::<i64, _, _>("SELECT COUNT(*) FROM work_unit_settlements", [], |r| r
                .get(0))
                .unwrap(),
            1
        );
    ...
    #[test]
    fn db_advanced_reopen_settles_exactly() {
        let (_d, mut c, _row) = fixture();
        reconcile_accepted_integrations(&mut c).unwrap();
        c.execute_batch("DELETE FROM accepted_work_unit_integration_evidence;DELETE FROM work_unit_settlements;UPDATE accepted_work_unit_integrations SET stage='db_advanced',settled_at=NULL;").unwrap();
        reconcile_accepted_integrations(&mut c).unwrap();
        assert_eq!(
            c.query_row::<i64, _, _>(
    ...
    #[test]
    fn lock_contention_stays_pending_then_converges() {
        let (_d, mut c, row) = fixture();
        let held = Lock::take(&row.common, "refs/heads/main").unwrap();
        reconcile_accepted_integrations(&mut c).unwrap();
        assert_eq!(
            c.query_row::<i64, _, _>(
    ...
    #[test]
    fn divergent_evidence_replay_is_rejected() {
        let (_d, mut c, _row) = fixture();
        reconcile_accepted_integrations(&mut c).unwrap();
        c.execute_batch("UPDATE accepted_work_unit_integration_evidence SET evidence_fingerprint='forged';UPDATE accepted_work_unit_integrations SET stage='db_advanced',settled_at=NULL;").unwrap();
        reconcile_accepted_integrations(&mut c).unwrap();
        assert_eq!(
            c.query_row::<String, _, _>(
                "SELECT attention_code FROM accepted_work_unit_integrations",
    ...
    #[test]
    fn ref_before_runtime_reopen_converges_once() {
        let (_d, mut c, row) = fixture();
        reconcile_accepted_integrations(&mut c).unwrap();
        let commit: String = c
            .query_row(
            ...
            .unwrap();
        run(&row.repo, &["read-tree", "--reset", "-u", &row.baseline]);
        let binding =
            sprint_target_binding_fingerprint(&row.authority, "refs/heads/main", &row.baseline);
        c.execute_batch("DELETE FROM accepted_work_unit_integration_evidence;DELETE FROM work_unit_settlements;").unwrap();
        c.execute("UPDATE sprint_target_currents SET current_object_id=?1,binding_fingerprint=?2,version=1",params![row.baseline,binding]).unwrap();
        c.execute(
            "UPDATE accepted_work_unit_integrations SET stage='ref_advanced',settled_at=NULL",
            [],
        )
        .unwrap();
        reconcile_accepted_integrations(&mut c).unwrap();
        assert_eq!(git(&row.repo, &["rev-parse", "HEAD"]).unwrap(), commit);
        assert_eq!(
            c.query_row::<String, _, _>(
                "SELECT stage FROM accepted_work_unit_integrations",
                [],
    ...
    #[test]
    fn ref_cas_before_stage_reopen_adopts_owned_pre_runtime() {
        let (_d, mut c, row) = fixture();
        reconcile_accepted_integrations(&mut c).unwrap();
        run(&row.repo, &["read-tree", "--reset", "-u", &row.baseline]);
        let binding =
        ...
        c.execute("UPDATE sprint_target_currents SET current_object_id=?1,binding_fingerprint=?2,version=1",params![row.baseline,binding]).unwrap();
        c.execute(
            "UPDATE accepted_work_unit_integrations SET stage='object_created',settled_at=NULL",
            [],
        )
        .unwrap();
        reconcile_accepted_integrations(&mut c).unwrap();
    ...
    #[test]
    fn runtime_before_stage_reopen_adopts_owned_clean_runtime() {
        let (_d, mut c, row) = fixture();
        reconcile_accepted_integrations(&mut c).unwrap();
        let binding =
            sprint_target_binding_fingerprint(&row.authority, "refs/heads/main", &row.baseline);
    ...
    #[test]
    fn queued_same_candidate_connections_replay_one_settled_integration() {
        let (_d, path, row) = file_fixture();
        let c = Connection::open(&path).unwrap();
        initialize_accepted_integration_schema(&c).unwrap();
        drop(c);
        let held = Lock::take(&row.common, "refs/heads/main").unwrap();
        let entered = Arc::new(Barrier::new(3));
        *test_target_lock_queue().lock().unwrap() = Some(TestTargetLockQueue {
        ...
        *test_target_lock_queue().lock().unwrap() = None;
        let mut replay = Connection::open(&path).unwrap();
        reconcile_accepted_integrations(&mut replay).unwrap();
        assert_eq!(
            replay
                .query_row::<i64, _, _>(
                    "SELECT COUNT(*) FROM accepted_work_unit_integrations",
        ...
        assert_eq!(
            replay
                .query_row::<i64, _, _>("SELECT COUNT(*) FROM work_unit_settlements", [], |r| r
                    .get(0))
                .unwrap(),
            1
        );
        ...
        assert_eq!(
            replay
                .query_row::<(String, Option<String>), _, _>(
                    "SELECT stage,attention_code FROM accepted_work_unit_integrations",
                    [],
        ...
        let mut third = Connection::open(&path).unwrap();
        reconcile_accepted_integrations(&mut third).unwrap();
        assert_eq!(
            third
                .query_row::<i64, _, _>(
                    "SELECT COUNT(*) FROM accepted_work_unit_integrations",
    ...
    #[test]
    fn two_concurrent_candidates_reserve_distinct_serialized_target_parents() {
        let (_d, path, row) = file_fixture();
        run(
            &row.repo,
            &["checkout", "-b", "candidate-two", &row.baseline],
        );
        fs::write(row.repo.join("b.txt"), "b\n").unwrap();
        run(&row.repo, &["add", "."]);
        run(&row.repo, &["commit", "-m", "candidate two"]);
        let candidate_two = git(&row.repo, &["rev-parse", "HEAD"]).unwrap();
        let tree_two = git(&row.repo, &["rev-parse", "HEAD^{tree}"]).unwrap();
        run(
            &row.repo,
            &[
                "update-ref",
                "refs/codex/orchestrator/accepted/candidate-two",
                &candidate_two,
            ],
        );
        run(&row.repo, &["checkout", "main"]);
        let c = Connection::open(&path).unwrap();
        c.execute(
            "INSERT INTO work_units VALUES('unit-2'),('dependent-2')",
            [],
        )
        .unwrap();
        c.execute("INSERT INTO work_unit_relationships VALUES('edge-two','depends_on','dependent-2','unit-2')",[]).unwrap();
        c.execute("INSERT INTO accepted_handler_candidates VALUES('candidate-two','unit-2','authority',?1,NULL,?2,?3,?4,'refs/codex/orchestrator/accepted/candidate-two','evidence-two')",params![now(),row.baseline,candidate_two,tree_two]).unwrap();
        drop(c);
            ...
            .map(|_| {
                ...
                std::thread::spawn(move || {
                    ...
                    c.busy_timeout(std::time::Duration::from_millis(50))
                        .unwrap();
                    barrier.wait();
                    reconcile_accepted_integrations(&mut c)
                })
            })
            .collect::<Vec<_>>();
        barrier.wait();
        ...
        reconcile_accepted_integrations(&mut reopened).unwrap();
        let commits=reopened.prepare("SELECT integration_commit_id FROM accepted_work_unit_integrations ORDER BY integration_commit_id").unwrap().query_map([],|r|r.get::<_,String>(0)).unwrap().collect::<Result<Vec<_>,_>>().unwrap();
        assert_eq!(commits.len(), 2);
        assert_eq!(
            git(&row.repo, &["rev-list", "--count", "HEAD"]).unwrap(),
            "3"
        );
        assert_eq!(reopened.query_row::<i64,_,_>("SELECT COUNT(*) FROM accepted_work_unit_integrations WHERE stage='settled' AND attention_code IS NULL",[],|r|r.get(0)).unwrap(),2);
        assert_eq!(
            reopened
                .query_row::<i64, _, _>("SELECT COUNT(*) FROM work_unit_settlements", [], |r| r
                    .get(0))
    ...
    #[test]
    fn two_candidates_serialize_against_reloaded_target() {
        let (_d, mut c, row) = fixture();
        c.execute_batch("INSERT INTO work_units VALUES('unit-2'),('dependent-1'),('dependent-2');INSERT INTO work_unit_relationships VALUES('edge-1','depends_on','dependent-1','unit'),('edge-2','depends_on','dependent-2','unit-2');").unwrap();
        run(
            &row.repo,
            &["checkout", "-b", "candidate-two", &row.baseline],
        );
        ...
        c.execute("INSERT INTO accepted_handler_candidates VALUES('candidate-two','unit-2','authority',?1,NULL,?2,?3,?4,'refs/codex/orchestrator/accepted/candidate-two','evidence-two')",params![now(),row.baseline,candidate_two,tree_two]).unwrap();
        reconcile_accepted_integrations(&mut c).unwrap();
        let commits=c.prepare("SELECT integration_commit_id FROM accepted_work_unit_integrations ORDER BY work_unit_id").unwrap().query_map([],|r|r.get::<_,String>(0)).unwrap().collect::<Result<Vec<_>,_>>().unwrap();
        assert_eq!(commits.len(), 2);
        assert_eq!(
            git(
                &row.repo,
                &["rev-list", "--parents", "-n", "1", &commits[1]]
            )
            .unwrap(),
            format!("{} {}", commits[1], commits[0])
        );
        let result_tree = git(
            &row.repo,
            &["rev-parse", &format!("{}^{{tree}}", commits[1])],
        )
        .unwrap();
        assert_eq!(
            git(
                &row.repo,
                &["show", &format!("{result_tree}:candidate.txt")]
            )
            .unwrap(),
            "candidate"
        );
        assert_eq!(
            git(&row.repo, &["show", &format!("{result_tree}:b.txt")]).unwrap(),
            "b"
        );
        reconcile_accepted_integrations(&mut c).unwrap();
        assert_eq!(
        ...
        assert_eq!(c.query_row::<i64,_,_>("SELECT COUNT(*) FROM accepted_work_unit_integrations WHERE attention_code IS NULL AND stage='settled'",[],|r|r.get(0)).unwrap(),2);
        assert_eq!(c.query_row::<i64,_,_>("SELECT COUNT(*) FROM work_unit_prerequisite_contributions WHERE (prerequisite_work_unit_id='unit' AND dependent_work_unit_id='dependent-1') OR (prerequisite_work_unit_id='unit-2' AND dependent_work_unit_id='dependent-2')",[],|r|r.get(0)).unwrap(),2);
        assert_eq!(c.query_row::<i64,_,_>("SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name IN ('work_unit_handler_activations','work_slice_planning_points','sprint_settlements','epic_settlements')",[],|r|r.get(0)).unwrap(),0);
    }
}
