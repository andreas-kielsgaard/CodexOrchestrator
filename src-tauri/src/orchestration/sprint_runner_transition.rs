...
impl SprintRunnerTransitionService {
    ...
    fn mark_epic_escalation_receiver(&self,handback:&str,column:&str)->Result<(),SprintRunnerTransitionError>{if !["delivery_persisted_at","harness_bound_at","launch_requested_at","launch_accepted_at","provider_activation_observed_at"].contains(&column){return Err(SprintRunnerTransitionError::Unavailable("invalid Epic escalation receiver stage".into()))}self.connection.lock().map_err(|_|SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?.execute(&format!("UPDATE epic_runner_escalation_receivers SET {column}=COALESCE({column},?2) WHERE handback_id=?1"),params![handback,chrono::Utc::now().to_rfc3339()]).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;Ok(())}
    ...
    fn epic_escalation_reassessment_context(&self,invocation:&AgentInvocationId)->Result<serde_json::Value,SprintRunnerTransitionError>{let row:Option<(String,String,String)>=self.connection.lock().map_err(|_|SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?.query_row("SELECT r.handback_id,h.context_json,m.details_json FROM epic_runner_escalation_receivers r JOIN work_unit_no_progress_handbacks h ON h.handback_id=r.handback_id JOIN sprint_runner_handback_dispositions m ON m.handback_id=r.handback_id WHERE r.reassessment_invocation_id=?1 AND r.delivery_persisted_at IS NOT NULL AND r.harness_bound_at IS NOT NULL AND r.launch_accepted_at IS NOT NULL AND r.semantic_reassessment_recorded_at IS NULL",[invocation.as_str()],|r|Ok((r.get(0)?,r.get(1)?,r.get(2)?))).optional().map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;let Some((_handback,context,details))=row else{return Err(SprintRunnerTransitionError::Forbidden)};let concern:serde_json::Value=serde_json::from_str(&context).map_err(|_|SprintRunnerTransitionError::Conflict)?;let disposition:serde_json::Value=serde_json::from_str(&details).map_err(|_|SprintRunnerTransitionError::Conflict)?;if disposition.get("movementKind").and_then(|value|value.as_str())!=Some("local_exhaustion_escalate"){return Err(SprintRunnerTransitionError::Conflict)}Ok(serde_json::json!({"handedBackConcern":{"classification":concern.get("classification"),"reason":concern.get("reason"),"evidence":concern.get("evidence")},"localExhaustion":{"rationale":disposition.get("rationale"),"summary":disposition.get("localExhaustionSummary")}}))}
