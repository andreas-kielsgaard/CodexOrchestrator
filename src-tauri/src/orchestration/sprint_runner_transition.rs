...
fn ensure_implementer_outcome_evidence_columns(connection: &Connection) -> Result<(), String> {
    for column in [
        "evidence_manifest_json TEXT", "comparison_fingerprint TEXT", "evidence_content_fingerprints_json TEXT", "evidence_ready_at TEXT", "semantic_completed_at TEXT", "semantic_completion_invocation_id TEXT", "lifecycle_observed_at TEXT", "lifecycle_status TEXT", "application_accepted_at TEXT", "handler_review_ready_at TEXT",
    ] {
...
struct ImplementationEvidenceSnapshot { manifest_json: String, comparison_fingerprint: String, content_fingerprints_json: String }

#[derive(Clone, Debug, Deserialize, Serialize, JsonSchema, Eq, PartialEq)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub(crate) struct HandlerReviewReturnReason { pub(crate) code: String, pub(crate) explanation: String }
#[derive(Clone)]
struct HandlerReviewContext { work_unit_id:String, attempt_id:String, session_id:String, review_invocation_id:String, revision_id:String, configuration_digest:String, repository_commit_ref:String, delivered_payload_json:String }
...
impl SprintRunnerTransitionService {
    ...
    pub(crate) fn on_agent_notification(
        ...
        if review { self.record_handler_review_lifecycle(&invocation.id,status)?; return self.finalize_handler_review_decisions(); }
