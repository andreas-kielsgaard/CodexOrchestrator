...
#[derive(Clone, Debug, Deserialize, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub(crate) struct ImplementationOutcomeClaims { pub(crate) summary: String, pub(crate) validation_statement: String }
...
impl SprintRunnerTransitionService {
    ...
    fn reconcile_implementer_reporting_continuations(self: &Arc<Self>) -> Result<(), SprintRunnerTransitionError> {
        ...
        for (work_unit, attempt, session_id, invocation_id) in activations {
            ...
            let prompt="Work Unit Implementer reporting continuation. Use only the two application-bound reporting tools. Outcome semantics are not yet available; do not perform review, acceptance, retry, settlement, dependency activation, or continuation.".to_string();
...
#[tool_handler(router=self.tool_router)] impl ServerHandler for WorkUnitImplementerReportingMcp { fn get_info(&self)->ServerInfo{ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions("Only the two application-bound Implementer reporting tools are exposed. Their semantics are not yet available.")} }
