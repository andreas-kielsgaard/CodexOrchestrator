...
use super::accepted_candidate_authority::{
    reconcile_accepted_candidate_authorities, ACCEPTED_CANDIDATE_AUTHORITY_SCHEMA,
};
...
fn migrate_legacy_implementer_activations(connection: &Connection) -> Result<(), String> {
    let columns = connection
        .prepare("PRAGMA table_info(work_unit_implementer_activations)")
    ...
    if !columns
        .iter()
        .any(|column| column == "implementer_attempt_id")
    {
        ...
        return Err(
            "Implementer activation table has neither legacy nor shared attempt identity".into(),
        );
    }
    connection
    ...
    let migrated = (|| -> Result<(), String> {
        ...
        let coherent_count: i64 = connection
            .query_row(
                "SELECT COUNT(*)
            ...
            .map_err(|error| {
                format!("validate Handler-correlated Implementer activations: {error}")
            })?;
        if legacy_count != coherent_count {
            return Err("legacy Implementer activation lacks coherent Handler correlation".into());
        }
        connection
            .execute_batch(SHARED_IMPLEMENTER_ACTIVATION_TABLE)
        ...
        if i64::try_from(copied)
            .map_err(|_| "legacy Implementer activation count overflow".to_string())?
            != legacy_count
        {
...
fn ensure_handler_action_failure_reason(connection: &Connection) -> Result<(), String> {
    let columns = connection
        .prepare("PRAGMA table_info(work_unit_handler_action_continuations)")
        .and_then(|mut statement| {
            statement
                .query_map([], |row| row.get::<_, String>(1))?
                .collect::<Result<Vec<_>, _>>()
    ...
    if !columns.iter().any(|column| column == "failure_reason") {
        connection
            .execute_batch(
                "ALTER TABLE work_unit_handler_action_continuations ADD COLUMN failure_reason TEXT",
            )
...
fn ensure_handler_activation_failure_reason(connection: &Connection) -> Result<(), String> {
    ...
    if !columns.iter().any(|column| column == "failure_reason") {
        connection
            .execute_batch(
                "ALTER TABLE work_unit_handler_activations ADD COLUMN failure_reason TEXT",
            )
...
fn ensure_implementer_outcome_evidence_columns(connection: &Connection) -> Result<(), String> {
    for column in [
        ...
        "comparison_fingerprint TEXT",
        "evidence_content_fingerprints_json TEXT",
        "file_review_capture_authorization_id TEXT",
        "evidence_ready_at TEXT",
        "semantic_completed_at TEXT",
        "semantic_completion_invocation_id TEXT",
        "lifecycle_observed_at TEXT",
        ...
        let name = column
            .split_whitespace()
            .next()
            .expect("outcome migration column");
        ...
        if !exists {
            connection
                .execute_batch(&format!(
                    "ALTER TABLE work_unit_implementer_outcomes ADD COLUMN {column}"
                ))
                .map_err(|error| error.to_string())?;
        }
    }
...
fn migrate_work_unit_attempt_history(connection: &Connection) -> Result<(), String> {
    let outcome_primary_key = connection
        .prepare("PRAGMA table_info(work_unit_implementer_outcomes)")
        .and_then(|mut statement| {
            statement
                .query_map([], |row| {
    ...
    if outcome_primary_key
        .iter()
        .any(|(name, position)| name == "attempt_id" && *position == 1)
    {
        return Ok(());
    }
    if !outcome_primary_key
        .iter()
        .any(|(name, position)| name == "work_unit_id" && *position == 1)
    {
    ...
    let migrated = (|| -> Result<(), String> {
        ...
        let reviews: i64 = connection
            .query_row(
                "SELECT COUNT(*) FROM work_unit_handler_reviews",
                [],
                |row| row.get(0),
            )
        ...
        let decisions: i64 = connection
            .query_row(
                "SELECT COUNT(*) FROM work_unit_handler_decisions",
                [],
                |row| row.get(0),
            )
...
"#).map_err(|error| format!("create attempt-history tables: {error}"))?;
        ...
        if copied_outcomes as i64 != outcomes
            || copied_reviews as i64 != reviews
            || copied_decisions as i64 != decisions
        {
            return Err("attempt-history migration copy was incomplete".into());
        }
    ...
    match migrated {
        Ok(()) => connection
            ...
            .map_err(|error| format!("commit attempt-history migration: {error}")),
        Err(error) => {
...
fn migrate_work_unit_retry_attempt_history(connection: &Connection) -> Result<(), String> {
    ...
    if !schema.contains("work_unit_id TEXT PRIMARY KEY") || !schema.contains("CHECK (ordinal=1)") {
        return Err(
            "retry-attempt history has an unknown primary key or ordinal constraint".into(),
        );
    }
    connection
        .execute_batch("PRAGMA foreign_keys=OFF; BEGIN IMMEDIATE;")
    ...
    let migrated = (|| -> Result<(), String> {
        let count: i64 = connection
            .query_row("SELECT COUNT(*) FROM work_unit_retry_attempts", [], |row| {
                row.get(0)
            })
...
"#).map_err(|error| format!("create retry-attempt history table: {error}"))?;
        ...
        if copied as i64 != count {
            return Err("retry-attempt migration copy was incomplete".into());
        }
...
fn migrate_handler_decision_retry_contract(connection: &Connection) -> Result<(), String> {
    ...
    if !schema.contains("retry_required_at IS NOT NULL") {
        return Ok(());
    }
    connection
        .execute_batch("PRAGMA foreign_keys=OFF; BEGIN IMMEDIATE;")
        .map_err(|error| format!("begin Handler decision migration: {error}"))?;
    let migrated = (|| -> Result<(), String> {
        let count: i64 = connection
            .query_row(
                "SELECT COUNT(*) FROM work_unit_handler_decisions",
            ...
            .map_err(|error| format!("count Handler decisions: {error}"))?;
        connection.execute_batch(r#"
...
"#).map_err(|error| format!("rebuild Handler decision schema: {error}"))?;
        let copied: i64 = connection
            ...
            .map_err(|error| format!("count rebuilt Handler decisions: {error}"))?;
        if copied != count {
            return Err("Handler decision migration copy was incomplete".into());
        }
        Ok(())
    })();
    match migrated {
        Ok(()) => connection
            .execute_batch("COMMIT; PRAGMA foreign_keys=ON;")
            .map_err(|error| format!("commit Handler decision migration: {error}")),
        Err(error) => {
            let _ = connection.execute_batch("ROLLBACK; PRAGMA foreign_keys=ON;");
            Err(error)
        }
...
#[derive(Clone, Debug, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub(crate) struct WorkSliceRefinement {
...
pub(crate) enum ImplementationOutcomeVariant {
    ReviewPending,
}

#[derive(Clone, Debug, Deserialize, Serialize, JsonSchema)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub(crate) struct ImplementationOutcomeClaims {
    pub(crate) outcome: ImplementationOutcomeVariant,
    pub(crate) summary: String,
    pub(crate) validation_statement: String,
}

#[derive(Clone)]
struct ImplementerReportingContext {
    work_unit_id: String,
    attempt_id: String,
    session_id: String,
...
struct ImplementationEvidenceSnapshot {
    manifest_json: String,
    comparison_fingerprint: String,
    content_fingerprints_json: String,
    capture_authorization_id: String,
}

#[derive(Clone, Debug, Deserialize, Serialize, JsonSchema, Eq, PartialEq)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
pub(crate) struct HandlerReviewReturnReason {
    pub(crate) code: String,
    pub(crate) explanation: String,
}
#[derive(Clone, Debug, Deserialize, Serialize, JsonSchema, Eq, PartialEq)]
#[serde(rename_all = "snake_case")]
pub(crate) enum IncompleteAttemptClassification {
    RefinementNeeded,
...
impl IncompleteAttemptClassification {
    fn as_str(&self) -> &'static str {
        match self {
            Self::RefinementNeeded => "refinement_needed",
            Self::FunctionalObjectiveNotSatisfied => "functional_objective_not_satisfied",
...
#[derive(Clone, Debug, Deserialize, Serialize, JsonSchema, Eq, PartialEq)]
#[serde(rename_all = "snake_case")]
pub(crate) enum EpicEscalationDownstreamTarget {
    SprintRunner,
    ExistingAgentAchievableDependency,
}
#[derive(Clone, Debug, Deserialize, Serialize, JsonSchema, Eq, PartialEq)]
#[serde(rename_all = "snake_case")]
pub(crate) enum EpicKnownAgentDependency {
    WorkUnitHandler,
}
#[derive(Clone, Debug, Deserialize, Serialize, JsonSchema, Eq, PartialEq)]
#[serde(rename_all = "camelCase", deny_unknown_fields)]
...
#[derive(Clone)]
struct HandlerReviewContext {
    work_unit_id: String,
    attempt_id: String,
    handler_authority_attempt_id: String,
    session_id: String,
...
impl SprintRunnerTransitionService {
    ...
    fn open_with_git_authority_runtime(
        ...
        let authority_repository =
            Arc::new(SqliteOrchestrationRepository::open(path).map_err(|error| {
                SprintRunnerTransitionError::Unavailable(format!(
                    "open Sprint Git authority repository: {error}"
                ))
            })?);
        let authority_binder =
            InitiatedSprintGitAuthorityService::new(authority_repository.clone(), runtime);
    ...
    #[cfg(test)]
    pub(crate) fn reconcile_reporting_for_test(&self) -> Result<(), SprintRunnerTransitionError> {
        self.reconcile_implementer_outcomes_v3()
    }

    #[cfg(test)]
    pub(crate) fn reconcile_handler_reviews_for_test(
        self: &Arc<Self>,
    ) -> Result<(), SprintRunnerTransitionError> {
    ...
    #[cfg(test)]
    pub(crate) fn reconcile_no_progress_handbacks_for_test(
        self: &Arc<Self>,
    ) -> Result<(), SprintRunnerTransitionError> {
        self.reconcile_no_progress_handbacks()
    }
    #[cfg(test)]
    pub(crate) fn reconcile_epic_escalation_receivers_for_test(
        self: &Arc<Self>,
    ) -> Result<(), SprintRunnerTransitionError> {
        self.reconcile_epic_escalation_receivers()
    }
    #[cfg(test)]
    pub(crate) fn reconcile_sprint_result_receivers_for_test(
        self: &Arc<Self>,
    ) -> Result<(), SprintRunnerTransitionError> {
        self.reconcile_sprint_result_receivers()
    }
    #[cfg(test)]
    pub(crate) fn sprint_result_reassessment_context_for_test(
        &self,
        invocation_id: &str,
    ) -> Result<serde_json::Value, SprintRunnerTransitionError> {
        let invocation = AgentInvocationId::new(invocation_id.to_owned())
            .map_err(|_| SprintRunnerTransitionError::Forbidden)?;
        self.strict_sprint_result_context(&invocation)
    }
    #[cfg(test)]
    pub(crate) fn record_sprint_result_disposition_for_test(
        self: &Arc<Self>,
        invocation_id: &str,
        disposition: EpicEscalationReassessmentDisposition,
    ) -> Result<(), SprintRunnerTransitionError> {
        let invocation = AgentInvocationId::new(invocation_id.to_owned())
            .map_err(|_| SprintRunnerTransitionError::Forbidden)?;
        self.record_sprint_result_disposition_strict(&invocation, disposition)
    }
    #[cfg(test)]
    pub(crate) fn epic_escalation_reassessment_context_for_test(
        &self,
        invocation_id: &str,
    ) -> Result<serde_json::Value, SprintRunnerTransitionError> {
        let invocation = AgentInvocationId::new(invocation_id.to_owned())
            .map_err(|_| SprintRunnerTransitionError::Forbidden)?;
        self.epic_escalation_reassessment_context(&invocation)
    }
    #[cfg(test)]
    pub(crate) fn record_epic_escalation_disposition_for_test(
        self: &Arc<Self>,
        invocation_id: &str,
        disposition: EpicEscalationReassessmentDisposition,
    ) -> Result<(), SprintRunnerTransitionError> {
        let invocation = AgentInvocationId::new(invocation_id.to_owned())
            .map_err(|_| SprintRunnerTransitionError::Forbidden)?;
        self.record_epic_escalation_disposition(&invocation, disposition)
    }

    #[cfg(test)]
    pub(crate) fn prepare_later_attempt_reporting_for_test(
        self: &Arc<Self>,
    ) -> Result<(), SprintRunnerTransitionError> {
    ...
    #[cfg(test)]
    pub(crate) fn reconcile_later_attempt_for_test(
        self: &Arc<Self>,
    ) -> Result<(), SprintRunnerTransitionError> {
        self.reconcile_implementer_outcomes_v3()?;
        self.reconcile_handler_reviews()
    ...
    pub(crate) fn handler_review_evidence_for_test(
        ...
        let invocation = AgentInvocationId::new(invocation_id.to_owned())
            .map_err(|_| SprintRunnerTransitionError::Forbidden)?;
        Ok(self
            .handler_review_context(&invocation, true)?
    ...
    pub(crate) fn prepared_handler_action_injection(
        ...
        self.mcp
            ...
            .ok()?
            .get(&invocation)
            .map(|action| action.injection.clone())
    }
    ...
    fn prepare_pre_start_action(
        self: &Arc<Self>,
        invocation_id: AgentInvocationId,
    ) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        let bearer = uuid::Uuid::new_v4().simple().to_string();
        let server = start_pre_start_server(
            self.clone(),
            invocation_id.clone(),
            bearer.clone(),
            vec!["tauri://localhost".into()],
        ...
        let injection = CodexMcpInjection::new_named(
            "sprint_runner_pre_start",
            &server.url(),
            bearer,
            &["report_pre_start_outcome".into()],
            true,
        );
        let mut active = self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Sprint Runner action registry is poisoned".into(),
            )
        })?;
        if let Some(existing) = active.get(&invocation_id) {
            let existing = existing.injection.clone();
            server.stop();
            return Ok(existing);
        }
        active.insert(
    ...
    fn prepare_epic_start_action(
        self: &Arc<Self>,
        invocation_id: AgentInvocationId,
    ) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        let bearer = uuid::Uuid::new_v4().simple().to_string();
        let server = start_epic_start_server(
            self.clone(),
            invocation_id.clone(),
            bearer.clone(),
            vec!["tauri://localhost".into()],
        ...
        .map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!(
                "start selected-Sprint action server: {e}"
            ))
        })?;
        let injection = CodexMcpInjection::new_named(
            "epic_runner_start",
    ...
    fn prepare_started_action(
        self: &Arc<Self>,
        invocation_id: AgentInvocationId,
    ) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        let bearer = uuid::Uuid::new_v4().simple().to_string();
        let server = start_started_server(
            self.clone(),
            invocation_id.clone(),
            bearer.clone(),
            vec!["tauri://localhost".into()],
        ...
        .map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!(
                "start reevaluation action server: {e}"
            ))
        })?;
        let injection = CodexMcpInjection::new_named(
            "sprint_runner_started",
            &server.url(),
            bearer,
            &["record_started_reevaluation".into()],
            true,
        );
        let mut active = self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
    ...
    fn prepare_planning_control_action(
        self: &Arc<Self>,
        invocation_id: AgentInvocationId,
    ) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        let bearer = uuid::Uuid::new_v4().simple().to_string();
        let server = start_planning_control_server(
            self.clone(),
            invocation_id.clone(),
            bearer.clone(),
            vec!["tauri://localhost".into()],
        ...
        .map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!(
                "start planning-control action server: {e}"
            ))
        })?;
        let injection = CodexMcpInjection::new_named(
            "sprint_runner_planning_control",
            &server.url(),
            bearer,
            &["request_work_slice_planner".into()],
            true,
        );
        let mut active = self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
    ...
    fn prepare_handback_reassessment_action(
        self: &Arc<Self>,
        invocation_id: AgentInvocationId,
    ) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        let bearer = uuid::Uuid::new_v4().simple().to_string();
        let server = start_handback_reassessment_server(
            self.clone(),
            invocation_id.clone(),
            bearer.clone(),
            vec!["tauri://localhost".into()],
        ...
        .map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!(
                "start Handback reassessment action server: {e}"
            ))
        })?;
        let injection = CodexMcpInjection::new_named(
            "sprint_runner_handback_reassessment",
            &server.url(),
            bearer,
            &[
                "read_sprint_handback_reassessment_context".into(),
                "record_sprint_handback_disposition".into(),
            ],
            true,
        );
        let mut active = self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Sprint Runner handback action registry is poisoned".into(),
            )
        })?;
        if let Some(existing) = active.get(&invocation_id) {
            let existing = existing.injection.clone();
    ...
    fn prepare_epic_escalation_reassessment_action(
        self: &Arc<Self>,
        invocation_id: AgentInvocationId,
    ) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        let bearer = uuid::Uuid::new_v4().simple().to_string();
        let server = start_epic_escalation_reassessment_server(
            self.clone(),
            invocation_id.clone(),
            bearer.clone(),
            vec!["tauri://localhost".into()],
        ...
        .map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!(
                "start Epic escalation receiver server: {e}"
            ))
        })?;
        let injection = CodexMcpInjection::new_named(
            "epic_runner_escalation_reassessment",
            &server.url(),
            bearer,
            &[
                "read_epic_escalation_reassessment_context".into(),
                "record_epic_escalation_disposition".into(),
            ],
            true,
        );
        let mut active = self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Epic escalation receiver registry is poisoned".into(),
            )
        })?;
        if let Some(existing) = active.get(&invocation_id) {
            let existing = existing.injection.clone();
    ...
    fn prepare_sprint_result_reassessment_action(
        self: &Arc<Self>,
        invocation_id: AgentInvocationId,
    ) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        let bearer = uuid::Uuid::new_v4().simple().to_string();
        let server = start_sprint_result_reassessment_server(
            self.clone(),
            invocation_id.clone(),
            bearer.clone(),
            vec!["tauri://localhost".into()],
        ...
        .map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!(
                "start Sprint-result receiver server: {e}"
            ))
        })?;
        let injection = CodexMcpInjection::new_named(
            "epic_runner_sprint_result_reassessment",
            &server.url(),
            bearer,
            &[
                "read_epic_sprint_result_reassessment_context".into(),
                "record_epic_sprint_result_disposition".into(),
            ],
            true,
        );
        let mut active = self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Sprint-result receiver registry is poisoned".into(),
            )
        })?;
        if let Some(existing) = active.get(&invocation_id) {
            let existing = existing.injection.clone();
    ...
    fn prepare_work_slice_planner_action(
        self: &Arc<Self>,
        invocation_id: AgentInvocationId,
    ) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        let bearer = uuid::Uuid::new_v4().simple().to_string();
        let server = start_work_slice_planner_server(
            self.clone(),
            invocation_id.clone(),
            bearer.clone(),
            vec!["tauri://localhost".into()],
        ...
        .map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!(
                "start Work Slice Planner action server: {e}"
            ))
        })?;
        let injection = CodexMcpInjection::new_named(
            "work_slice_planner",
            ...
            bearer,
            &[
                "read_current_planning_context".into(),
                "submit_work_slice_proposal".into(),
                "request_work_slice_refinement".into(),
                "complete_work_slice_planning".into(),
            ],
            true,
        );
        let mut active = self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Work Slice Planner action registry is poisoned".into(),
            )
        })?;
        if let Some(existing) = active.get(&invocation_id) {
            let existing = existing.injection.clone();
    ...
    fn prepare_work_unit_handler_action(
        ...
        let bearer = uuid::Uuid::new_v4().simple().to_string();
        let server = start_work_unit_handler_server(
            self.clone(),
            invocation_id.clone(),
            bearer.clone(),
            vec!["tauri://localhost".into()],
        ...
        .map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!(
                "start Work Unit Handler action server: {e}"
            ))
        })?;
        let injection = CodexMcpInjection::new_named(
            "work_unit_handler",
            &server.url(),
            bearer,
            &["request_work_unit_implementer".into()],
            true,
        );
        let mut active = self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Work Unit Handler action registry is poisoned".into(),
            )
        })?;
        if let Some(existing) = active.get(&invocation_id) {
            let existing = existing.injection.clone();
    ...
    fn prepare_work_unit_implementer_reporting_action(
        self: &Arc<Self>,
        invocation_id: AgentInvocationId,
    ) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        let bearer = uuid::Uuid::new_v4().simple().to_string();
        let server = start_work_unit_implementer_reporting_server(
            self.clone(),
            invocation_id.clone(),
            bearer.clone(),
            vec!["tauri://localhost".into()],
        ...
        .map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!(
                "start Work Unit Implementer reporting server: {e}"
            ))
        })?;
        let injection = CodexMcpInjection::work_unit_implementer_reporting(&server.url(), bearer);
        if !injection.is_exact_work_unit_implementer_reporting_transport() {
            server.stop();
            return Err(SprintRunnerTransitionError::Conflict);
        }
        let mut active = self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Work Unit Implementer reporting registry is poisoned".into(),
            )
        })?;
        if let Some(existing) = active.get(&invocation_id) {
            let existing = existing.injection.clone();
    ...
    fn prepare_work_unit_handler_review_action(
        self: &Arc<Self>,
        invocation_id: AgentInvocationId,
    ) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        let bearer = uuid::Uuid::new_v4().simple().to_string();
        let server = start_work_unit_handler_review_server(
            self.clone(),
            invocation_id.clone(),
            bearer.clone(),
            vec!["tauri://localhost".into()],
        ...
        .map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!(
                "start Work Unit Handler review server: {e}"
            ))
        })?;
        let injection = CodexMcpInjection::new_named(
            ...
            bearer,
            &[
                "read_handler_review_evidence".into(),
                "accept_implementation_outcome".into(),
                "return_implementation_outcome".into(),
            ],
            true,
        );
        let mut active = self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Work Unit Handler review registry is poisoned".into(),
            )
        })?;
        if let Some(existing) = active.get(&invocation_id) {
            let existing = existing.injection.clone();
    ...
    pub(crate) fn reconcile_startup(
        ...
        self.reconcile_epic_escalation_receivers()?;
        {
            let mut connection = self.connection.lock().map_err(|_| {
                SprintRunnerTransitionError::Unavailable(
                    "planning database lock is poisoned".into(),
                )
            })?;
            reconcile_accepted_candidate_authorities(&mut connection)
                .map_err(SprintRunnerTransitionError::Unavailable)?;
            reconcile_accepted_integrations(&mut connection)
                .map_err(SprintRunnerTransitionError::Unavailable)?;
            reconcile_work_unit_dependency_wave(&mut connection)
                .map_err(SprintRunnerTransitionError::Unavailable)?;
            reconcile_work_slice_execution_settlement(&mut connection)
                .map_err(SprintRunnerTransitionError::Unavailable)?;
            sprint_continuation_settlement::reconcile(&mut connection)
                .map_err(SprintRunnerTransitionError::Unavailable)?;
        }
        self.reconcile_sprint_result_receivers()?;
    ...
    fn observe_existing_terminals(
        ...
        let Some((session_id, outcome_invocation)) = record else {
            return Ok(());
        };
        let session = AgentSessionId::new(session_id)
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        let Ok(history) = self.sessions.load_session(&session) else {
            return Ok(());
        };
        if let Some(found) = history
            .invocations
            .iter()
            .find(|candidate| candidate.invocation.id.as_str() == outcome_invocation)
        {
        ...
        if let Some(started) = started {
            if let Some(found) = history
                .invocations
                .iter()
                .find(|candidate| candidate.invocation.id.as_str() == started)
            {
                if found.invocation.status.is_terminal() {
    ...
    fn reconcile_sprint_continuation_boundary(&self) -> Result<(), SprintRunnerTransitionError> {
        let mut connection = self.connection.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into())
        })?;
        sprint_continuation_settlement::reconcile(&mut connection)
            .map_err(SprintRunnerTransitionError::Unavailable)
    }
    ...
    fn reconcile_epic_settlement_boundary(&self) -> Result<(), SprintRunnerTransitionError> {
        ...
        crate::orchestration::epic_settlement::reconcile(&mut connection)
            .map_err(SprintRunnerTransitionError::Unavailable)
    }
    ...
    pub(crate) fn record_pre_start_outcome(
        ...
        if let Some(existing) = found.0 {
            return if existing == fact
                && found.1.as_deref() == Some(invocation_id.as_str())
                && found.2.as_deref() == Some(&input.forecast_and_concerns)
                && found.3.as_deref() == Some(&input.material_uncertainty)
                && found.4.as_deref() == Some(&input.application_owned_prerequisite)
            {
                drop(conn);
                drop(_guard);
    ...
    pub(crate) fn on_agent_notification(
        ...
        if retry_implementer {
            if status != "completed" {
                return self.fail_retry_for_invocation(
                    &invocation.id,
                    "retry_implementer_lifecycle_not_completed",
                );
            }
            self.reconcile_implementer_reporting_continuations()?;
            return Ok(());
        }
        if self.record_reporting_lifecycle_v3(&invocation.id, status)? {
            self.reconcile_implementer_outcome_acceptance_v2()?;
            return self.reconcile_handler_reviews();
        }
        ...
        if escalation.is_some() {
            return self.observe_epic_escalation_receiver_terminals();
        }
        ...
        if unblocked_escalation {
            return self.reconcile_epic_escalation_receivers();
        }
        ...
        let Some(sprint_id) = sprint else {
            return Ok(());
        };
        let lock = self.transition_lock(&sprint_id)?;
        let _guard = lock.lock().map_err(|_| {
    ...
    pub(crate) fn start_selected_sprint(
        self: &Arc<Self>,
        invocation_id: &AgentInvocationId,
    ) -> Result<(), SprintRunnerTransitionError> {
        let sprint: Option<String> = self.connection.lock().map_err(|_| SprintRunnerTransitionError::Unavailable("Sprint Runner transition database lock is poisoned".into()))?.query_row(
    ...
    pub(crate) fn record_started_reevaluation(
        self: &Arc<Self>,
        invocation_id: &AgentInvocationId,
        input: StartedReevaluation,
    ) -> Result<(), SprintRunnerTransitionError> {
        validate_outcome(&input.repository_branch_evaluation)?;
        validate_outcome(&input.started_forecast_and_concerns)?;
        let desired = conversation_harness::profile(ConversationHarnessRole::SprintRunner)
            .map_err(SprintRunnerTransitionError::Unavailable)?;
        let conn = self.connection.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Sprint Runner transition database lock is poisoned".into(),
        ...
        let Some((sprint_id, started, launched)) = sprint else {
            return Err(SprintRunnerTransitionError::Forbidden);
        };
        drop(conn);
        let lock = self.transition_lock(&sprint_id)?;
        ...
        if started.is_none() || launched.is_none() {
            return Err(SprintRunnerTransitionError::Forbidden);
        }
        ...
        if let Some(old) = old {
            return if old
                == (
                    input.repository_branch_evaluation,
                    input.started_forecast_and_concerns,
                ) {
                Ok(())
            } else {
                Err(SprintRunnerTransitionError::Conflict)
    ...
    fn authorized_runner(
        ...
        let desired = conversation_harness::profile(ConversationHarnessRole::EpicRunner)
            .map_err(SprintRunnerTransitionError::Unavailable)?;
        let reassessment = conversation_harness::profile(
        ...
        match result {
            ...
            Some(runner)
                if runner.harness_key == "epic_runner"
                    && (runner.harness_version == 2
                        || runner.harness_version == i64::from(desired.version)) =>
            {
                Ok(runner)
            }
            Some(runner)
                if runner.harness_key == reassessment.key
                    && runner.harness_version == i64::from(reassessment.version) =>
            {
                Ok(runner)
            }
    ...
    fn planning_route_authority(
            ...
            .load_initiated_sprint_git_authority_for_sprint(sprint_id)
        {
            ...
            | Err(InitiatedSprintGitAuthorityError::Forbidden) => {
                Err(SprintRunnerTransitionError::Forbidden)
            }
            Err(InitiatedSprintGitAuthorityError::Conflict) => {
                Err(SprintRunnerTransitionError::Conflict)
            }
            Err(InitiatedSprintGitAuthorityError::Unavailable) => Err(
                SprintRunnerTransitionError::Unavailable("load Sprint Git authority".into()),
            ),
        }
    }
            ...
            Err(SprintRunnerTransitionError::Forbidden) => self
                ...
                .and_then(|bound| {
                    self.authority_repository
                        ...
                        .map_err(|_| {
                            SprintRunnerTransitionError::Unavailable(
                                "load bound Sprint Git authority".into(),
                            )
                        })?
    ...
    fn reconcile_sprint(
        ...
        loop {
            let observed = self
                ...
                .get(sprint_id)
                .map(|drain| drain.generation)
                .ok_or_else(|| {
                    SprintRunnerTransitionError::Unavailable(
                        "Sprint Runner reconciliation drain disappeared".into(),
                    )
                })?;
            if let Err(error) = self.reconcile_sprint_pass(sprint_id) {
                ...
                if let Some(drain) = drains.get_mut(sprint_id) {
                    drain.running = false;
                }
                return Err(error);
            }
    ...
    fn reconcile_sprint_pass(
        ...
        if record.harness_key != harness.key || record.harness_version != harness.version {
            if record.harness_key == "sprint_runner"
                && record.harness_version == 1
                && record.launch_accepted_at.is_some()
            {
            ...
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?
        {
            ...
            ApplicationInvocationLaunchEvidence::PersistedNotAccepted => {}
            ApplicationInvocationLaunchEvidence::NeverPersisted => {
                let injection = self.prepare_pre_start_action(invocation.clone())?;
    ...
    fn reconcile_v1_pre_start_upgrade(
        self: &Arc<Self>,
        record: &SprintRecord,
        ...
        let terminal = history
            .invocations
            .iter()
            .find(|candidate| candidate.invocation.id.as_str() == record.invocation_id)
        ...
        let invocation = AgentInvocationId::new(id)
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        match self
            .sessions
            .application_invocation_launch_evidence(&invocation, &session)
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?
        {
            ...
            ApplicationInvocationLaunchEvidence::NeverPersisted => {
                ...
                self.mark(&record.sprint_id, "pre_start_upgrade_harness_applied_at")?;
                if launch.launch_accepted {
                    self.mark(&record.sprint_id, "pre_start_upgrade_launch_accepted_at")?;
                }
            }
        };
        self.reconcile_accepted_pre_start(&record.sprint_id)
    }

    fn reconcile_accepted_pre_start(
        self: &Arc<Self>,
        sprint_id: &str,
    ) -> Result<(), SprintRunnerTransitionError> {
        ...
        if fact.is_some()
            && lifecycle.as_deref() == Some("completed")
            && outcome_invocation == lifecycle_invocation
            && accepted.is_none()
        {
            let now = chrono::Utc::now().to_rfc3339();
            let invocation = stable_id("epic-runner-start-continuation", sprint_id);
            let harness = conversation_harness::profile(ConversationHarnessRole::EpicRunner)
                .map_err(SprintRunnerTransitionError::Unavailable)?;
            let delivery = stable_id("accepted-pre-start-parent-delivery", sprint_id);
        ...
        if let (Some(invocation_id), None) = (record.0.clone(), record.1) {
            let session = AgentSessionId::new(epic_session)
                .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            let history = self
                .sessions
                .load_session(&session)
                .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            let origin_terminal = history
                .invocations
                .iter()
                .find(|candidate| candidate.invocation.id.as_str() == origin_epic_invocation)
                .is_some_and(|candidate| candidate.invocation.status.is_terminal());
            #[cfg(test)]
            ...
            let invocation = AgentInvocationId::new(invocation_id.clone())
                .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            match self
                .sessions
                .application_invocation_launch_evidence(&invocation, &session)
                .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?
            {
                ...
                ApplicationInvocationLaunchEvidence::NeverPersisted => {
                    ...
                    self.mark(sprint_id, "parent_continuation_delivery_persisted_at")?;
                    self.mark(sprint_id, "epic_continuation_harness_applied_at")?;
                    if launch.launch_accepted {
                        self.mark(sprint_id, "epic_continuation_launch_accepted_at")?;
                    }
                }
        ...
        if started.is_some() {
            ...
            if launch.is_none() {
                let session = AgentSessionId::new(sprint_session)
                    .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
                let invocation = AgentInvocationId::new(id)
                    .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
                match self
                    ...
                    .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?
                {
                    ...
                    ApplicationInvocationLaunchEvidence::NeverPersisted => {
                        let harness =
                            conversation_harness::profile(ConversationHarnessRole::SprintRunner)
                                .map_err(SprintRunnerTransitionError::Unavailable)?;
                        let injection = self.prepare_started_action(invocation.clone())?;
                        let mut additional_args = harness.runtime_configuration_args();
                        additional_args.extend(injection.configuration_args);
                        ...
                        self.mark(sprint_id, "sprint_continuation_harness_applied_at")?;
                        if launch.launch_accepted {
                            self.mark(sprint_id, "sprint_continuation_launch_accepted_at")?;
                        }
                    }
    ...
    fn reconcile_planning_control(
        ...
        let Some((session_id, epic_id, fact, lifecycle, control, accepted)) = record else {
            return Ok(());
        };
        if fact.is_none() || lifecycle.as_deref() != Some("completed") {
            return Ok(());
        }
        let harness =
            conversation_harness::profile(ConversationHarnessRole::SprintRunnerPlanningControl)
                .map_err(SprintRunnerTransitionError::Unavailable)?;
        let invocation_id = control
            .unwrap_or_else(|| stable_id("sprint-runner-planning-control-continuation", sprint_id));
        self.connection.lock().map_err(|_| SprintRunnerTransitionError::Unavailable("Sprint Runner transition database lock is poisoned".into()))?.execute(
            ...
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?
        {
            ApplicationInvocationLaunchEvidence::LaunchAccepted => {
                ...
                self.mark(sprint_id, "planning_control_launch_accepted_at")?;
                self.mark(sprint_id, "planning_ready_at")?;
            }
            ApplicationInvocationLaunchEvidence::PersistedNotAccepted => {
                self.mark(sprint_id, "planning_control_delivery_persisted_at")?
            }
            ApplicationInvocationLaunchEvidence::NeverPersisted => {
                ...
                self.mark(sprint_id, "planning_control_delivery_persisted_at")?;
                self.mark(sprint_id, "planning_control_harness_applied_at")?;
                if launch.launch_accepted {
                    self.mark(sprint_id, "planning_control_launch_accepted_at")?;
                    self.mark(sprint_id, "planning_ready_at")?;
                }
    ...
    pub(crate) fn request_work_slice_planner(
        self: &Arc<Self>,
        invocation_id: &AgentInvocationId,
        _input: WorkSlicePlannerRequest,
    ) -> Result<SprintRunnerTransitionStatus, SprintRunnerTransitionError> {
        ...
        let planning_control =
            conversation_harness::profile(ConversationHarnessRole::SprintRunnerPlanningControl)
                .map_err(SprintRunnerTransitionError::Unavailable)?;
        let planner_harness =
            conversation_harness::profile(ConversationHarnessRole::WorkSlicePlanner)
                .map_err(SprintRunnerTransitionError::Unavailable)?;
        ...
        if let Some(existing) = existing {
            let expected = CurrentPlanningRequest {
                planning_point_id: point.clone(),
                request_fact_id: request.clone(),
                parent_sprint_runner_session_id: session_id.clone(),
                parent_planning_control_invocation_id: invocation_id.as_str().to_owned(),
                authority_id: authority.authority_id.clone(),
                authority_epic_id: authority.epic_id.clone(),
                authority_provenance_id: authority.provenance_id.clone(),
                authority_repository_id: authority.repository_id.clone(),
                authority_worktree_id: authority.worktree_id.clone(),
                authority_baseline_object_id: authority.baseline_object_id.clone(),
                authority_current_object_id: authority.current_object_id.clone(),
                authority_source_fingerprint: authority.source_fingerprint.clone(),
                repository_worktree_route: authority.worktree_root.clone(),
                planner_harness_key: planner_harness.key.clone(),
                planner_harness_version: i64::from(planner_harness.version),
                planner_session_id: planner_session.clone(),
                planner_invocation_id: planner_invocation.clone(),
            };
    ...
    fn reconcile_work_slice_acceptance(
        &self,
        sprint_id: &str,
    ) -> Result<(), SprintRunnerTransitionError> {
        ...
        let Some((point, session_id, invocation_id, key, version)) = record else {
            return Ok(());
        };
        let desired = conversation_harness::profile(ConversationHarnessRole::WorkSlicePlanner)
            .map_err(SprintRunnerTransitionError::Unavailable)?;
        if key != desired.key || version != i64::from(desired.version) {
            return Ok(());
        }
        let session = AgentSessionId::new(session_id)
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        let history = self
            ...
            .load_session(&session)
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        let Some(found) = history.invocations.iter().find(|candidate| {
            candidate.invocation.id.as_str() == invocation_id
                && candidate.invocation.status.is_terminal()
        }) else {
            return Ok(());
        };
        let status = lifecycle_status(found.invocation.status);
        let now = chrono::Utc::now().to_rfc3339();
        ...
        if status == "completed" {
            self.materialize_accepted_work_slice_plan(sprint_id)?;
        }
        Ok(())
    }
    ...
    fn materialize_accepted_work_slice_plan(
        &self,
        sprint_id: &str,
    ) -> Result<(), SprintRunnerTransitionError> {
        let lock = self.transition_lock(sprint_id)?;
        ...
        let Some((point, revision, epic, sprint)) = source else {
            return Err(SprintRunnerTransitionError::Forbidden);
        };
        let proposal_json: String = tx
            .query_row(
                "SELECT proposal_json FROM work_slice_proposal_revisions WHERE revision_id=?1",
                [&revision],
                |row| row.get(0),
            )
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        let proposal: WorkSliceProposal = serde_json::from_str(&proposal_json)
            .map_err(|_| SprintRunnerTransitionError::Conflict)?;
        validate_work_slice_proposal(&proposal)?;
        let materialization = stable_id(
            "work-unit-materialization",
            &format!("{epic}:{sprint}:{point}:{revision}"),
        );
        ...
        let units = proposal
            ...
            .map(|(ordinal, lane)| {
                (
                    stable_id("work-unit", &format!("{materialization}:{ordinal}")),
                    ordinal as i64,
                    lane.title.clone(),
                    lane.specification.clone(),
                )
            })
            .collect::<Vec<_>>();
        ...
        let expected_units = units
            .iter()
            .map(|(id, ordinal, title, specification)| {
        ...
        let by_title = units
            .iter()
            .map(|(id, _, title, _)| (title.as_str(), id.as_str()))
            .collect::<std::collections::HashMap<_, _>>();
        let mut relationships = vec![
            (
                stable_id(
                    "work-unit-relationship",
                    &format!("{materialization}:planning_point:{point}:{work_slice}"),
                ),
                "planning_point",
                point.clone(),
                work_slice.clone(),
            ...
            (
                stable_id(
                    "work-unit-relationship",
                    &format!("{materialization}:sprint:{sprint}:{work_slice}"),
                ),
                "sprint",
                sprint.clone(),
                work_slice.clone(),
        ...
        for (id, ordinal, title, _) in &units {
            relationships.push((
                stable_id(
                    "work-unit-relationship",
                    &format!("{materialization}:lane:{work_slice}:{id}"),
                ),
                "lane",
                work_slice.clone(),
                id.clone(),
            ...
            relationships.push((
                stable_id(
                    "work-unit-relationship",
                    &format!("{materialization}:order:{work_slice}:{id}"),
                ),
                "order",
                work_slice.clone(),
                id.clone(),
            ...
            for dependency in &proposal.lanes[*ordinal as usize].depends_on {
                let dependency_id = by_title
                    .get(dependency.as_str())
                    .ok_or(SprintRunnerTransitionError::Conflict)?;
                relationships.push((
                    stable_id(
                        "work-unit-relationship",
                        &format!("{materialization}:depends_on:{id}:{dependency_id}"),
                    ),
                    "depends_on",
                    id.clone(),
        ...
        for (id, kind, from, to, ordinal) in &relationships {
            tx.execute("INSERT OR IGNORE INTO work_unit_relationships (relationship_id,materialization_id,relationship_kind,from_id,to_id,ordinal) VALUES (?1,?2,?3,?4,?5,?6)", params![id,materialization,kind,from,to,ordinal]).map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        }
        ...
        let mut expected_relationships = relationships
            .into_iter()
            .map(|(id, kind, from, to, ordinal)| (id, kind.to_owned(), from, to, ordinal))
            .collect::<Vec<_>>();
    ...
    fn reconcile_materializations(&self) -> Result<(), SprintRunnerTransitionError> {
        ...
        for sprint in sprints {
            self.materialize_accepted_work_slice_plan(&sprint)?;
        }
        Ok(())
    }
    ...
    fn reconcile_implementer_activations(
        self: &Arc<Self>,
    ) -> Result<(), SprintRunnerTransitionError> {
    ...
    fn reconcile_implementer_reporting_continuations(
        self: &Arc<Self>,
    ) -> Result<(), SprintRunnerTransitionError> {
        let Some(handler) = self
            .work_unit_handler
        ...
        {
            let session = AgentSessionId::new(session_id.clone())
                .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?;
            let history = self
                .sessions
                .load_session(&session)
            ...
            let completed = history.invocations.iter().any(|entry| {
                entry.invocation.id.as_str() == invocation_id
                    && entry.invocation.status == AgentInvocationStatus::Completed
            });
            if !completed {
                continue;
            }
            ...
            if handler.commit_implementer_candidate(&attempt).is_err() {
                self.record_implementer_candidate_failure(
                    &work_unit,
                    &attempt,
                    ordinal,
                    &session_id,
                    &invocation_id,
                    "implementer_candidate_revalidation_failed",
                )?;
                continue;
            }
            let original = match handler.load_pinned_implementer_revision(
                &original_revision,
                &original_digest,
                &original_commit,
            ) {
                Ok(value)
                    if !value.profile.mcp.required
                        && value.profile.mcp.enabled_tools.is_empty() =>
                {
                    value
                }
                _ => {
            ...
            let candidate = match handler.construct_for_pinned_profile(
                &attempt,
            ...
            if candidate
                .bind_correlated_invocation(
                    session.clone(),
                    AgentInvocationId::new(invocation_id.clone())
                        .map_err(|_| SprintRunnerTransitionError::Conflict)?,
                )
                .is_err()
            {
            ...
            if candidate
                .comparison()
                .ok()
                .filter(|comparison| !comparison.is_empty())
                .is_none()
                || candidate
                ...
                || manifest.iter().any(|entry| {
                    candidate
                        .evidence_content(&entry.evidence_ref)
                        .ok()
                ...
                })
            {
                self.record_implementer_candidate_failure(
                    ...
                    &session_id,
                    &invocation_id,
                    "implementer_candidate_evidence_unavailable",
                )?;
                continue;
            }
            let reporting = handler
                .current_implementer_reporting_revision()
                .map_err(|_| {
                    SprintRunnerTransitionError::Unavailable(
                        "immutable Implementer reporting Harness revision unavailable".into(),
                    )
                })?;
            let reporting_invocation =
                stable_id("work-unit-implementer-reporting-invocation", &attempt);
            ...
            let reporting = handler
                .load_pinned_implementer_revision(&revision, &digest, &commit)
                .map_err(|_| SprintRunnerTransitionError::Conflict)?;
            if !reporting.profile.mcp.required
                || reporting.profile.mcp.enabled_tools
                    != [
            ...
            let package = handler
                ...
                .map_err(|_| SprintRunnerTransitionError::Conflict)?;
            let reporting_invocation = AgentInvocationId::new(stored_reporting)
                .map_err(|_| SprintRunnerTransitionError::Conflict)?;
            let injection =
                self.prepare_work_unit_implementer_reporting_action(reporting_invocation.clone())?;
            let mut runtime = package.runtime_launch_configuration();
            runtime
                .extension
                .additional_args
                .extend(injection.configuration_args);
            ...
            self.sessions
                .prepare_idempotent_application_invocation(
                    SendIdempotentApplicationAgentSessionMessageCommand {
                        invocation_id: reporting_invocation.clone(),
                        message: SendAgentSessionMessageCommand {
                            session_id: Some(session.clone()),
                            submitted_text: prompt.clone(),
                            title: None,
                            working_directory: Some(package.working_directory().into()),
                            requested_options: Some(runtime.requested_options.clone()),
                        },
                    },
                )
                .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?;
            ...
            package
                .bind_correlated_invocation(session.clone(), reporting_invocation.clone())
                .map_err(|_| SprintRunnerTransitionError::Conflict)?;
            self.mark_reporting(&attempt, "reporting_harness_bound_at")?;
            match self
                .sessions
                .application_invocation_launch_evidence(&reporting_invocation, &session)
                .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?
            {
                ApplicationInvocationLaunchEvidence::LaunchAccepted => {
                    self.mark_reporting(&attempt, "reporting_launch_accepted_at")?;
                    self.mark_reporting(&attempt, "reporting_ready_at")?
                }
                ApplicationInvocationLaunchEvidence::PersistedNotAccepted => {
                    self.mark_reporting(&attempt, "reporting_launch_requested_at")?;
                    let launch = self
                        .sessions
                        .launch_prepared_application_invocation_with_launch_observation(
                            SendIdempotentApplicationAgentSessionMessageCommand {
                                invocation_id: reporting_invocation,
                                message: SendAgentSessionMessageCommand {
                    ...
                    if launch.launch_accepted {
                        ...
                        return Err(SprintRunnerTransitionError::Unavailable(
                            "Implementer reporting launch was not accepted".into(),
                        ));
                    }
                }
                ApplicationInvocationLaunchEvidence::NeverPersisted => {
    ...
    fn mark_reporting(
        ...
        if ![
            "reporting_prepared_at",
            "reporting_harness_bound_at",
            "reporting_launch_requested_at",
            "reporting_launch_accepted_at",
            "reporting_ready_at",
        ]
        .contains(&column)
        {
            return Err(SprintRunnerTransitionError::Unavailable(
                "invalid reporting stage".into(),
            ));
        }
        self.connection.lock().map_err(|_|SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?.execute(&format!("UPDATE work_unit_implementer_outcomes SET {column}=COALESCE({column},?2) WHERE attempt_id=?1"),params![attempt,chrono::Utc::now().to_rfc3339()]).map_err(|error|SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        Ok(())
    }

    fn reporting_row_for_invocation(
        &self,
        invocation: &AgentInvocationId,
    ) -> Result<(String, String, String, String, String), SprintRunnerTransitionError> {
        ...
        if attempt != activation_attempt
            || session != activation_session
            || original != activation_original
            || reporting != stable_id("work-unit-implementer-reporting-invocation", &attempt)
            || reporting != invocation.as_str()
        {
            return Err(SprintRunnerTransitionError::Conflict);
        };
        let handler = self
        ...
        let history = self
            .sessions
            .load_session(
                &AgentSessionId::new(session.clone())
                    .map_err(|_| SprintRunnerTransitionError::Forbidden)?,
            )
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        let original_completed = history.invocations.iter().any(|entry| {
            entry.invocation.id.as_str() == original
                && entry.invocation.status == AgentInvocationStatus::Completed
        });
    ...
    fn legacy_submit_implementation_outcome(
        &self,
        invocation: &AgentInvocationId,
        input: ImplementationOutcomeClaims,
    ) -> Result<(), SprintRunnerTransitionError> {
        validate_outcome(&input.summary)?;
        validate_outcome(&input.validation_statement)?;
        let (unit, _attempt, session, _original, stored) =
            self.reporting_row_for_invocation(invocation)?;
        if stored != invocation.as_str() {
            return Err(SprintRunnerTransitionError::Forbidden);
        };
        let history = self
            .sessions
        ...
        let payload = serde_json::to_string(&input)
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        let fingerprint = stable_id("implementer-outcome", &payload);
        let now = chrono::Utc::now().to_rfc3339();
    ...
    fn legacy_complete_implementation_outcome(
        &self,
        invocation: &AgentInvocationId,
    ) -> Result<(), SprintRunnerTransitionError> {
        let (unit, _attempt, session, _original, stored) =
    ...
    fn reporting_context(
        ...
        let history = self
            .sessions
            .load_session(
                &AgentSessionId::new(context.session_id.clone())
                    .map_err(|_| SprintRunnerTransitionError::Forbidden)?,
            )
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        let original = history.invocations.iter().any(|entry| {
            entry.invocation.id.as_str() == context.implementer_invocation_id
                && entry.invocation.status == AgentInvocationStatus::Completed
        });
        let reporting = history
            .invocations
            .iter()
            .find(|entry| entry.invocation.id == *invocation)
            .ok_or(SprintRunnerTransitionError::Forbidden)?;
        if !original || (require_live && reporting.invocation.status.is_terminal()) {
            return Err(SprintRunnerTransitionError::Forbidden);
        }
        Ok(context)
    }
    fn evidence_snapshot(
        ...
        (ImplementerReportingContext, ImplementationEvidenceSnapshot),
        SprintRunnerTransitionError,
    > {
        let context = self.reporting_context(invocation, require_live)?;
        let handler = self
            .work_unit_handler
            .lock()
            .map_err(|_| {
        ...
        let manifest = package
            ...
            .map_err(|_| SprintRunnerTransitionError::Forbidden)?;
        if manifest.is_empty() || manifest.len() > 500 {
            return Err(SprintRunnerTransitionError::Forbidden);
        }
        let comparison = package
            .comparison()
            .map_err(|_| SprintRunnerTransitionError::Forbidden)?;
        if comparison.is_empty() || comparison.len() > 256_000 {
            return Err(SprintRunnerTransitionError::Forbidden);
        }
        let mut manifest_values = Vec::with_capacity(manifest.len());
        let mut contents = Vec::with_capacity(manifest.len());
        for entry in manifest {
            if !safe_id(&entry.evidence_ref)
                || entry.display_name.trim().is_empty()
                || entry.change_kind.trim().is_empty()
            {
                return Err(SprintRunnerTransitionError::Forbidden);
            }
            let content = package
                .evidence_content(&entry.evidence_ref)
                .map_err(|_| SprintRunnerTransitionError::Forbidden)?;
            if content.is_empty() || content.len() > 256_000 {
                return Err(SprintRunnerTransitionError::Forbidden);
            }
            let reference = entry.evidence_ref;
            manifest_values.push(serde_json::json!({"evidenceRef":reference.clone(),"displayName":entry.display_name,"changeKind":entry.change_kind}));
            contents.push(serde_json::json!({"evidenceRef":reference,"contentFingerprint":fingerprint_bytes("implementer-evidence-content",&content)}));
        }
        manifest_values.sort_by(|a, b| a["evidenceRef"].as_str().cmp(&b["evidenceRef"].as_str()));
        contents.sort_by(|a, b| a["evidenceRef"].as_str().cmp(&b["evidenceRef"].as_str()));
        let capture_authorization_id = package
            .capture_authorization_id()
            .map_err(|_| SprintRunnerTransitionError::Forbidden)?;
        Ok((
            context,
            ImplementationEvidenceSnapshot {
                manifest_json: serde_json::to_string(&manifest_values)
                    .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?,
                comparison_fingerprint: fingerprint_bytes(
                    "implementer-evidence-comparison",
                    &comparison,
                ),
                content_fingerprints_json: serde_json::to_string(&contents)
                    .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?,
                capture_authorization_id,
            },
        ))
    ...
    fn capture_implementation_evidence(
        ...
        let (context, snapshot) = self.evidence_snapshot(invocation, true)?;
        let mut connection = self.connection.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into())
        })?;
        ...
        let identical = existing.0.as_deref() == Some(&snapshot.manifest_json)
            && existing.1.as_deref() == Some(&snapshot.comparison_fingerprint)
            && existing.2.as_deref() == Some(&snapshot.content_fingerprints_json)
            && existing.3.as_deref() == Some(&snapshot.capture_authorization_id);
        if existing.4.is_some() && identical {
            transaction
                .commit()
                .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            return Ok(());
    ...
    pub(crate) fn submit_implementation_outcome(
        &self,
        invocation: &AgentInvocationId,
        input: ImplementationOutcomeClaims,
    ) -> Result<(), SprintRunnerTransitionError> {
        validate_outcome(&input.summary)?;
        validate_outcome(&input.validation_statement)?;
        if input.outcome != ImplementationOutcomeVariant::ReviewPending {
            return Err(SprintRunnerTransitionError::Invalid);
        }
        let context = self.reporting_context(invocation, true)?;
        let payload = serde_json::to_string(&input)
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        let fingerprint = stable_id("implementer-outcome", &payload);
    ...
    pub(crate) fn complete_implementation_outcome(
        &self,
        invocation: &AgentInvocationId,
    ) -> Result<(), SprintRunnerTransitionError> {
        let context = self.reporting_context(invocation, true)?;
        self.capture_implementation_evidence(invocation)?;
    ...
    fn record_reporting_lifecycle(
        ...
        let observed = history
            ...
            .find(|entry| entry.invocation.id == *invocation)
            .ok_or(SprintRunnerTransitionError::Forbidden)?;
        if !observed.invocation.status.is_terminal()
            || lifecycle_status(observed.invocation.status) != status
        {
            return Err(SprintRunnerTransitionError::Conflict);
        }
    ...
    fn reconcile_implementer_outcomes(&self) -> Result<(), SprintRunnerTransitionError> {
        ...
        for candidate in candidates {
            ...
            if let Some(entry) = history.invocations.iter().find(|entry| {
                ...
                self.record_reporting_lifecycle(
                    &invocation,
                    lifecycle_status(entry.invocation.status),
                )?;
            }
        }
        self.reconcile_implementer_outcome_acceptance()
    }
    fn reconcile_implementer_outcome_acceptance(&self) -> Result<(), SprintRunnerTransitionError> {
        let candidates:Vec<(String,String,String,String,String,String)>=self.connection.lock().map_err(|_|SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?.prepare("SELECT work_unit_id,reporting_invocation_id,semantic_payload_json,evidence_manifest_json,comparison_fingerprint,evidence_content_fingerprints_json FROM work_unit_implementer_outcomes WHERE outcome_variant='review_pending' AND validation_result='valid' AND evidence_ready_at IS NOT NULL AND semantic_completed_at IS NOT NULL AND semantic_completion_invocation_id=reporting_invocation_id AND lifecycle_status='completed' ORDER BY work_unit_id").and_then(|mut s|s.query_map([],|r|Ok((r.get(0)?,r.get(1)?,r.get(2)?,r.get(3)?,r.get(4)?,r.get(5)?)))?.collect::<Result<Vec<_>,_>>()).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        for (unit, invocation_id, payload, manifest, comparison, contents) in candidates {
            let claims: ImplementationOutcomeClaims = serde_json::from_str(&payload)
                .map_err(|_| SprintRunnerTransitionError::Conflict)?;
            if claims.outcome != ImplementationOutcomeVariant::ReviewPending {
                return Err(SprintRunnerTransitionError::Conflict);
            }
            let invocation = AgentInvocationId::new(invocation_id)
                .map_err(|_| SprintRunnerTransitionError::Conflict)?;
            let (context, actual) = self.evidence_snapshot(&invocation, false)?;
            if context.work_unit_id != unit
                || actual.manifest_json != manifest
                || actual.comparison_fingerprint != comparison
                || actual.content_fingerprints_json != contents
            {
                return Err(SprintRunnerTransitionError::Conflict);
            }
            let mut connection = self.connection.lock().map_err(|_| {
    ...
    fn record_reporting_lifecycle_v2(
        &self,
        invocation: &AgentInvocationId,
        status: &str,
    ) -> Result<bool, SprintRunnerTransitionError> {
    ...
    fn valid_accepted_outcome(
        ...
        let (
            ...
            Some(result),
            Some(completed),
            Some(completion_invocation),
            Some(lifecycle_observed),
        ...
        if variant != "review_pending"
            || result != "valid"
            || completion_invocation != invocation.as_str()
            || lifecycle_observed.is_empty()
        ...
        let claims: ImplementationOutcomeClaims = match serde_json::from_str(&payload) {
            Ok(value) => value,
        ...
        let canonical = serde_json::to_string(&claims)
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        Ok(
            claims.outcome == ImplementationOutcomeVariant::ReviewPending
                && canonical == payload
                && claims.summary == summary
                && claims.validation_statement == validation
                && fingerprint == stable_id("implementer-outcome", &payload),
        )
    }
    fn reconcile_implementer_outcomes_v2(&self) -> Result<(), SprintRunnerTransitionError> {
        ...
        for candidate in candidates {
            ...
            if let Some(entry) = history.invocations.iter().find(|entry| {
                ...
                self.record_reporting_lifecycle_v2(
                    &invocation,
                    lifecycle_status(entry.invocation.status),
                )?;
            }
    ...
    fn record_reporting_lifecycle_v3(
        &self,
        invocation: &AgentInvocationId,
        status: &str,
    ) -> Result<bool, SprintRunnerTransitionError> {
        ...
        let failure = (status == "completed" && !semantic_effects)
            .then_some("reporting_required_semantic_effects_missing");
    ...
    fn reconcile_implementer_outcomes_v3(&self) -> Result<(), SprintRunnerTransitionError> {
        ...
        for candidate in candidates {
            ...
            if let Some(entry) = history.invocations.iter().find(|entry| {
                ...
                self.record_reporting_lifecycle_v3(
                    &invocation,
                    lifecycle_status(entry.invocation.status),
                )?;
            }
    ...
    fn reconcile_handler_reviews(self: &Arc<Self>) -> Result<(), SprintRunnerTransitionError> {
        ...
        let _reconciliation_guard = reconciliation_lock.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Handler review reconciliation lock is poisoned".into(),
            )
        })?;
        let ready: Vec<(String, String)> = self.connection.lock().map_err(|_| SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?
            ...
            .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        for (work_unit, attempt) in ready {
            self.prepare_handler_review(&work_unit, &attempt)?;
        }
        ...
        for (session, review) in reviews {
            let session =
                AgentSessionId::new(session).map_err(|_| SprintRunnerTransitionError::Conflict)?;
            let invocation = AgentInvocationId::new(review)
                .map_err(|_| SprintRunnerTransitionError::Conflict)?;
            let history = self
                .sessions
                .load_session(&session)
            ...
            if let Some(entry) = history.invocations.iter().find(|entry| {
                ...
                self.record_handler_review_lifecycle(
                    &invocation,
                    lifecycle_status(entry.invocation.status),
                )?;
            }
    ...
    fn prepare_handler_review(
        ...
        let reporting_id = AgentInvocationId::new(reporting.clone())
            .map_err(|_| SprintRunnerTransitionError::Conflict)?;
        if !self.valid_accepted_outcome(work_unit, &reporting_id)? {
            return Err(SprintRunnerTransitionError::Conflict);
        }
        let (_, actual) = self.evidence_snapshot(&reporting_id, false)?;
        if actual.manifest_json != manifest
            || actual.comparison_fingerprint != comparison
            || actual.content_fingerprints_json != contents
        {
            return Err(SprintRunnerTransitionError::Conflict);
        ...
        let revision = handler.current_handler_review_revision().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "immutable Handler review Harness revision unavailable".into(),
            )
        })?;
        let review = stable_id("work-unit-handler-review-invocation", attempt);
        let payload =
            handler_review_payload(&summary, &validation, &manifest, &comparison, &contents)?
                .to_string();
    ...
    fn launch_handler_review(
        self: &Arc<Self>,
        review: &str,
    ) -> Result<(), SprintRunnerTransitionError> {
        let invocation = AgentInvocationId::new(review.to_owned())
            .map_err(|_| SprintRunnerTransitionError::Conflict)?;
        ...
        let package = handler
            .construct_for_pinned_profile(
                &context.handler_authority_attempt_id,
                WorkUnitHarnessRole::Handler,
                pinned.profile,
            )
            .map_err(|_| SprintRunnerTransitionError::Conflict)?;
        let session = AgentSessionId::new(context.session_id.clone())
            .map_err(|_| SprintRunnerTransitionError::Conflict)?;
        ...
        package
            .bind_correlated_invocation(session.clone(), invocation.clone())
            .map_err(|_| SprintRunnerTransitionError::Conflict)?;
            ...
            .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?
        {
            ApplicationInvocationLaunchEvidence::LaunchAccepted => {
                ...
                self.mark_handler_review(&context.attempt_id, "launch_accepted_at")?;
                self.mark_handler_review(&context.attempt_id, "review_ready_at")?;
            }
            ApplicationInvocationLaunchEvidence::PersistedNotAccepted => {
                self.mark_handler_review(&context.attempt_id, "launch_requested_at")?;
                let launched = self
                ...
                if launched.launch_accepted {
                    ...
                    return Err(SprintRunnerTransitionError::Unavailable(
                        "Handler review launch was not accepted".into(),
                    ));
                }
            }
            ApplicationInvocationLaunchEvidence::NeverPersisted => {
    ...
    fn mark_handler_review(
        &self,
        attempt: &str,
        column: &str,
    ) -> Result<(), SprintRunnerTransitionError> {
        if ![
            "delivery_persisted_at",
            "harness_bound_at",
            "launch_requested_at",
            "launch_accepted_at",
            "review_ready_at",
    ...
    fn prelaunch_handler_review_context(
        &self,
        invocation: &AgentInvocationId,
    ) -> Result<HandlerReviewContext, SprintRunnerTransitionError> {
        ...
        if context.review_invocation_id != invocation.as_str()
            || context.review_invocation_id
                != stable_id("work-unit-handler-review-invocation", &context.attempt_id)
            || context.delivered_payload_fingerprint
                != stable_id(
                    "work-unit-handler-review-delivery",
                    &context.delivered_payload_json,
                )
        {
            return Err(SprintRunnerTransitionError::Conflict);
        }
        ...
        let pinned = handler
            ...
            .map_err(|_| SprintRunnerTransitionError::Conflict)?;
        if pinned.profile.mcp.enabled_tools
            != [
    ...
    fn handler_review_context(
        ...
        let reporting = AgentInvocationId::new(stable_id(
            "work-unit-implementer-reporting-invocation",
            &context.attempt_id,
        ))
        .map_err(|_| SprintRunnerTransitionError::Conflict)?;
        if !self.valid_accepted_outcome(&context.work_unit_id, &reporting)? {
            return Err(SprintRunnerTransitionError::Conflict);
        }
        ...
        if manifest != snapshot.manifest_json
            || comparison != snapshot.comparison_fingerprint
            || contents != snapshot.content_fingerprints_json
        {
            return Err(SprintRunnerTransitionError::Conflict);
        }
        let expected = handler_review_payload(
            ...
            &snapshot.content_fingerprints_json,
        )?;
        let delivered: serde_json::Value = serde_json::from_str(&context.delivered_payload_json)
            .map_err(|_| SprintRunnerTransitionError::Conflict)?;
        ...
        let entry = history
            ...
            .find(|entry| entry.invocation.id == *invocation)
            .ok_or(SprintRunnerTransitionError::Forbidden)?;
        if live && entry.invocation.status.is_terminal() {
            return Err(SprintRunnerTransitionError::Forbidden);
        }
        Ok(context)
    }

    fn record_handler_review_judgment(
        &self,
        invocation: &AgentInvocationId,
        variant: &str,
        reason: Option<HandlerReviewReturnReason>,
    ...
    fn record_handler_incomplete_disposition(
        &self,
        invocation: &AgentInvocationId,
        disposition: HandlerReviewIncompleteDisposition,
    ) -> Result<(), SprintRunnerTransitionError> {
        validate_handler_review_return(&HandlerReviewReturnReason {
            code: disposition.code.clone(),
            explanation: disposition.explanation.clone(),
        })?;
        self.record_handler_review_judgment_inner(
            invocation,
            "return",
            Some(HandlerReviewReturnReason {
                code: disposition.code,
                explanation: disposition.explanation,
            }),
            Some((disposition.classification, disposition.meaningful_progress)),
        )
    }

    fn record_handler_review_judgment_inner(
        &self,
        invocation: &AgentInvocationId,
        variant: &str,
        reason: Option<HandlerReviewReturnReason>,
        incomplete: Option<(IncompleteAttemptClassification, bool)>,
    ) -> Result<(), SprintRunnerTransitionError> {
        if (variant == "return" && reason.is_none())
            || (variant != "return" && (reason.is_some() || incomplete.is_some()))
        {
            return Err(SprintRunnerTransitionError::Invalid);
        }
        ...
        let reason = reason
            ...
            .transpose()?;
        let incomplete_fingerprint =
            incomplete
                .as_ref()
                .map(|(classification, meaningful_progress)| {
                    stable_id(
                        "work-unit-handler-incomplete-judgment",
                        &format!(
                            "{}:{}:{}",
                            context.review_invocation_id,
        ...
        let fingerprint = stable_id(
            "work-unit-handler-review-judgment",
            &format!(
                "{}:{variant}:{}",
                context.review_invocation_id,
        ...
        let Some((old_variant, old_reason, old_fingerprint)) = existing else {
            return Err(SprintRunnerTransitionError::Forbidden);
        };
        if let Some(old_variant) = old_variant {
            ...
            if old_variant == variant
                && old_reason == reason
                && old_fingerprint.as_deref() == Some(&fingerprint)
                && exact_incomplete
            {
                transaction
                    .commit()
        ...
        if let (Some((classification, meaningful_progress)), Some(incomplete_fingerprint)) =
    ...
    fn record_handler_review_lifecycle(
        &self,
        invocation: &AgentInvocationId,
        status: &str,
    ) -> Result<bool, SprintRunnerTransitionError> {
        ...
        if !entry.invocation.status.is_terminal()
            || lifecycle_status(entry.invocation.status) != status
        {
            return Err(SprintRunnerTransitionError::Conflict);
        }
    ...
    fn finalize_handler_review_decisions(&self) -> Result<(), SprintRunnerTransitionError> {
        ...
        {
            let decision = match judgment.as_str() {
                "accept" => "accepted",
                "return" => "returned",
            ...
            let decision_fingerprint = stable_id(
                "work-unit-handler-decision",
                &format!("{review}:{fingerprint}"),
            );
            let now = chrono::Utc::now().to_rfc3339();
            let mut connection = self.connection.lock().map_err(|_| {
                SprintRunnerTransitionError::Unavailable(
            ...
            if let Some((classification, meaningful_progress)) = incomplete {
                ...
                if !meaningful_progress {
                    ...
                    let handback_id = stable_id(
                        "work-unit-no-progress-handback",
                        &format!("{attempt}:{review}"),
                    );
                    let context_fingerprint =
                        stable_id("work-unit-no-progress-handback-context", &context);
    ...
    fn reconcile_work_unit_retries(self: &Arc<Self>) -> Result<(), SprintRunnerTransitionError> {
        ...
        for source in sources {
            self.reconcile_work_unit_retry(&handler, source)?;
        }
        Ok(())
    }
    ...
    fn reconcile_no_progress_handbacks(
        self: &Arc<Self>,
    ) -> Result<(), SprintRunnerTransitionError> {
        ...
        for (handback, sprint, session, context_fingerprint) in sources {
            self.reconcile_no_progress_handback(
                &handback,
                &sprint,
                &session,
                &context_fingerprint,
    ...
    fn reconcile_no_progress_handback(
        self: &Arc<Self>,
        handback: &str,
        sprint: &str,
        session: &str,
        ...
        let harness = conversation_harness::profile(
            ConversationHarnessRole::SprintRunnerHandbackReassessment,
        )
        .map_err(SprintRunnerTransitionError::Unavailable)?;
        ...
        transaction
            ...
            .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        drop(conn);
        let session = AgentSessionId::new(session.to_owned())
            .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        let history = self
            .sessions
            .load_session(&session)
    ...
    fn mark_handback_delivery(
        ...
        if ![
            ...
            "launch_requested_at",
            "launch_accepted_at",
            "provider_activation_observed_at",
        ]
        .contains(&column)
        {
            return Err(SprintRunnerTransitionError::Unavailable(
                "invalid Handback delivery stage".into(),
            ));
        }
    ...
    fn handback_reassessment_context(
        &self,
        invocation: &AgentInvocationId,
    ) -> Result<(String, serde_json::Value), SprintRunnerTransitionError> {
        ...
        let Some((handback, context, fingerprint)) = row else {
            return Err(SprintRunnerTransitionError::Forbidden);
        };
        if fingerprint != stable_id("work-unit-no-progress-handback-context", &context) {
            return Err(SprintRunnerTransitionError::Conflict);
        }
        let concern: serde_json::Value =
            serde_json::from_str(&context).map_err(|_| SprintRunnerTransitionError::Conflict)?;
        ...
        Ok((
            handback,
            serde_json::json!({"handedBackConcern": concern, "currentSprintWorkState": {"eligibleWorkUnitCount": eligible, "blockedWorkUnitCount": blocked}}),
        ))
    }

    fn record_handback_disposition(
        self: &Arc<Self>,
        invocation: &AgentInvocationId,
        input: SprintHandbackDisposition,
    ) -> Result<(), SprintRunnerTransitionError> {
        ...
        let serialized = serde_json::to_string(&input)
            .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        let fingerprint = stable_id(
            "sprint-runner-handback-disposition",
            &format!("{handback}:{serialized}"),
        );
        ...
        if changed == 0 {
            let exact: bool = transaction.query_row("SELECT EXISTS(SELECT 1 FROM sprint_runner_handback_dispositions WHERE handback_id=?1 AND disposition_id=?2 AND disposition_fingerprint=?3 AND preserves_handback=1)",params![handback,disposition,fingerprint],|row|row.get(0)).map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?;
            if !exact {
                return Err(SprintRunnerTransitionError::Conflict);
            }
        }
        ...
        if input.movement_kind == "wait_for_agent_dependency" {
            let routes=transaction.prepare("SELECT h.work_unit_id FROM work_unit_handler_activations h LEFT JOIN work_unit_settlements s ON s.work_unit_id=h.work_unit_id JOIN sprint_runner_handback_deliveries d ON d.sprint_id=h.sprint_id WHERE d.handback_id=?1 AND h.eligibility_state='eligible' AND h.handler_ready_at IS NOT NULL AND s.work_unit_id IS NULL ORDER BY h.work_unit_id").and_then(|mut statement|statement.query_map([&handback],|row|row.get::<_,String>(0))?.collect::<Result<Vec<_>,_>>()).map_err(|error|SprintRunnerTransitionError::Unavailable(error.to_string()))?;
            if routes.len() != 1 {
                return Err(SprintRunnerTransitionError::Conflict);
            }
            let route = &routes[0];
            let fingerprint = stable_id(
                "sprint-handback-dependency-route",
                &format!("{handback}:{route}"),
            );
            let changed=transaction.execute("INSERT OR IGNORE INTO sprint_handback_dependency_routes (handback_id,work_unit_id,route_fingerprint,recorded_at) VALUES (?1,?2,?3,?4)",params![handback,route,fingerprint,now]).map_err(|error|SprintRunnerTransitionError::Unavailable(error.to_string()))?;
            if changed == 0 {
                let exact:bool=transaction.query_row("SELECT EXISTS(SELECT 1 FROM sprint_handback_dependency_routes WHERE handback_id=?1 AND work_unit_id=?2 AND route_fingerprint=?3)",params![handback,route,fingerprint],|row|row.get(0)).map_err(|error|SprintRunnerTransitionError::Unavailable(error.to_string()))?;
                if !exact {
                    return Err(SprintRunnerTransitionError::Conflict);
                }
            }
        ...
        if local_exhaustion {
            let intent = stable_id("sprint-runner-handback-escalation-intent", &handback);
            let request = stable_id(
                "sprint-runner-handback-escalation-delivery-request",
                &handback,
            );
            transaction.execute("INSERT OR IGNORE INTO sprint_runner_handback_escalations (handback_id,escalation_intent_id,delivery_request_id,requested_at,delivery_requested_at) VALUES (?1,?2,?3,?4,?4)",params![handback,intent,request,now]).map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        }
        transaction
            .commit()
            .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?;
    ...
    fn reconcile_epic_escalation_receiver(
        ...
        let _guard = lock.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Epic escalation transition lock is poisoned".into(),
            )
        })?;
        let harness = conversation_harness::profile(
            ConversationHarnessRole::EpicRunnerEscalationReassessment,
        )
        .map_err(SprintRunnerTransitionError::Unavailable)?;
        let invocation = stable_id("epic-runner-escalation-reassessment", &handback);
        let delivery = stable_id("epic-runner-escalation-delivery", &handback);
        let correlation = stable_id(
            "epic-runner-escalation-correlation",
            &format!(
                "{handback}:{intent}:{request}:{sprint}:{epic}:{session}:{governing}:{context}"
            ),
        );
        ...
        {
            ApplicationInvocationLaunchEvidence::LaunchAccepted => {
                let _ = self.prepare_epic_escalation_reassessment_action(invocation)?;
                for stage in [
                    "delivery_persisted_at",
                    "harness_bound_at",
                    "launch_requested_at",
                    ...
                    self.mark_epic_escalation_receiver(&handback, stage)?;
                }
            }
            ApplicationInvocationLaunchEvidence::PersistedNotAccepted => {
                let injection =
                    self.prepare_epic_escalation_reassessment_action(invocation.clone())?;
                let mut args = harness.runtime_configuration_args();
                args.extend(injection.configuration_args);
                for stage in [
                    "delivery_persisted_at",
                ...
                let launch = self
                    ...
                    .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
                if launch.launch_accepted {
                    self.mark_epic_escalation_receiver(&handback, "launch_accepted_at")?;
                }
            }
            ApplicationInvocationLaunchEvidence::NeverPersisted => {
                let injection =
                ...
                args.extend(injection.configuration_args);
                self.sessions
                    .prepare_idempotent_application_invocation(message.clone())
                    .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
                for stage in [
                    "delivery_persisted_at",
                    "harness_bound_at",
    ...
    fn mark_epic_escalation_receiver(
        &self,
        handback: &str,
        column: &str,
    ) -> Result<(), SprintRunnerTransitionError> {
        ...
        {
            return Err(SprintRunnerTransitionError::Unavailable(
                "invalid Epic escalation receiver stage".into(),
            ));
        }
        let now = chrono::Utc::now().to_rfc3339();
        ...
        connection.execute(&format!("UPDATE epic_runner_escalation_receivers SET {column}=COALESCE({column},?2) WHERE handback_id=?1"),params![handback,now]).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        if column == "delivery_persisted_at" {
            let changed=connection.execute("UPDATE sprint_runner_handback_escalations SET delivery_persisted_at=COALESCE(delivery_persisted_at,?2) WHERE handback_id=?1",params![handback,chrono::Utc::now().to_rfc3339()]).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            if changed != 1 {
                return Err(SprintRunnerTransitionError::Conflict);
            }
        }
    ...
    fn observe_epic_escalation_receiver_terminals(
        ...
        for (handback, session, invocation) in receivers {
            ...
            if let Some(entry) = history.invocations.iter().find(|entry| {
                ...
                let status = lifecycle_status(entry.invocation.status);
                self.mark_epic_escalation_receiver(&handback, "provider_activation_observed_at")?;
    ...
    fn epic_escalation_reassessment_context(
        &self,
        invocation: &AgentInvocationId,
    ) -> Result<serde_json::Value, SprintRunnerTransitionError> {
        ...
        let Some((_handback, context, details, epic, sprint, title, movement)) = row else {
            return Err(SprintRunnerTransitionError::Forbidden);
        };
        let concern: serde_json::Value =
            serde_json::from_str(&context).map_err(|_| SprintRunnerTransitionError::Conflict)?;
        let disposition: serde_json::Value =
            serde_json::from_str(&details).map_err(|_| SprintRunnerTransitionError::Conflict)?;
        if disposition
            .get("movementKind")
            .and_then(|value| value.as_str())
            != Some("local_exhaustion_escalate")
        {
            return Err(SprintRunnerTransitionError::Conflict);
        }
        let connection = self.connection.lock().map_err(|_| {
        ...
        let plan: Option<String> = connection
            .query_row(
                "SELECT proposal_json FROM epic_bootstrap_transitions WHERE epic_id=?1",
                [&epic],
                |r| r.get(0),
            )
            .optional()
        ...
        let dependencies = if handler_known {
            serde_json::json!([{ "owner":"work_unit_handler", "enablingResult":"application-observed Handler result", "resumptionPath":"reassess this exact escalation after that result" }])
        } else {
            serde_json::json!([])
        };
        Ok(
            serde_json::json!({"acceptedEpicPlan":{"available":plan.is_some(),"suggestedEpicName":plan.as_ref().and_then(|value|value.get("suggestedEpicName")),"otherAvailableEpicWork":other},"currentSprintState":{"title":title,"intendedMovement":movement,"eligibleWorkUnitCount":eligible,"blockedWorkUnitCount":blocked},"knownAgentAchievableDependencies":dependencies,"handedBackConcern":{"classification":concern.get("classification"),"reason":concern.get("reason"),"evidence":concern.get("evidence")},"localExhaustion":{"rationale":disposition.get("rationale"),"summary":disposition.get("localExhaustionSummary")}}),
        )
    }

    fn record_epic_escalation_disposition(
        self: &Arc<Self>,
        invocation: &AgentInvocationId,
        input: EpicEscalationReassessmentDisposition,
    ) -> Result<(), SprintRunnerTransitionError> {
        ...
        if matches!(
            ...
            let changed=tx.execute("INSERT OR IGNORE INTO sprint_handback_dependency_routes (handback_id,work_unit_id,route_fingerprint,recorded_at) VALUES (?1,?2,?3,?4)",params![handback,route,fingerprint,chrono::Utc::now().to_rfc3339()]).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            if changed == 0 {
                let exact:bool=tx.query_row("SELECT EXISTS(SELECT 1 FROM sprint_handback_dependency_routes WHERE handback_id=?1 AND work_unit_id=?2 AND route_fingerprint=?3)",params![handback,route,fingerprint],|r|r.get(0)).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
                if !exact {
                    return Err(SprintRunnerTransitionError::Conflict);
                }
            }
        ...
        let fingerprint = stable_id(
            "epic-runner-escalation-disposition",
            &format!("{handback}:{serialized}"),
        );
        let disposition_id = stable_id("epic-runner-escalation-disposition-id", &handback);
        let semantic = stable_id("epic-runner-escalation-semantic-reassessment", &handback);
        let now = chrono::Utc::now().to_rfc3339();
        ...
        if let Some(request) = input.downstream_request {
            let request_json = serde_json::to_string(&request)
                .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            let request_id = stable_id("epic-runner-escalation-downstream-request", &handback);
            let request_fingerprint = stable_id(
                "epic-runner-escalation-downstream-request",
                &format!("{handback}:{request_json}"),
            );
            let kind = match request.target {
                EpicEscalationDownstreamTarget::SprintRunner => "sprint_runner",
                EpicEscalationDownstreamTarget::ExistingAgentAchievableDependency => {
            ...
            if changed == 0 {
                let exact:bool=tx.query_row("SELECT EXISTS(SELECT 1 FROM epic_runner_escalation_downstream_requests WHERE handback_id=?1 AND request_id=?2 AND request_fingerprint=?3)",params![handback,request_id,request_fingerprint],|r|r.get(0)).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
                if !exact {
                    return Err(SprintRunnerTransitionError::Conflict);
                }
            }
        ...
        if let Some(attention) = input.human_external_attention {
            let attention_json = serde_json::to_string(&attention)
                .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            let attention_id = stable_id("epic-runner-escalation-attention", &handback);
            let attention_fingerprint = stable_id(
                "epic-runner-escalation-attention",
                &format!("{handback}:{attention_json}"),
            );
            ...
            if changed == 0 {
                let exact:bool=tx.query_row("SELECT EXISTS(SELECT 1 FROM epic_runner_escalation_attentions WHERE handback_id=?1 AND attention_id=?2 AND attention_fingerprint=?3)",params![handback,attention_id,attention_fingerprint],|r|r.get(0)).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
                if !exact {
                    return Err(SprintRunnerTransitionError::Conflict);
                }
            }
    ...
    fn reconcile_sprint_result_receivers(
        self: &Arc<Self>,
    ) -> Result<(), SprintRunnerTransitionError> {
    ...
    fn reconcile_sprint_result_receiver(
        self: &Arc<Self>,
        (result, decision, sprint, epic, session, governing, chronology): (
            String,
            String,
            String,
            String,
        ...
        let _guard = lock.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Sprint-result transition lock is poisoned".into(),
            )
        })?;
        let harness = conversation_harness::profile(
            ConversationHarnessRole::EpicRunnerSprintResultReassessment,
        ...
        .map_err(SprintRunnerTransitionError::Unavailable)?;
        let invocation = stable_id("epic-runner-sprint-result-reassessment", &result);
        let delivery = stable_id("epic-runner-sprint-result-delivery", &result);
        let correlation = stable_id(
            "epic-runner-sprint-result-correlation",
            &format!("{result}:{decision}:{sprint}:{epic}:{session}:{governing}:{chronology}"),
        );
        let mut conn = self.connection.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into())
        })?;
        ...
        {
            ApplicationInvocationLaunchEvidence::LaunchAccepted => {
                let _ = self.prepare_sprint_result_reassessment_action(invocation)?;
                for column in [
                    "delivery_persisted_at",
                    "harness_bound_at",
                    "launch_requested_at",
                    ...
                    self.mark_sprint_result_receiver(&result, column)?;
                }
            }
            ApplicationInvocationLaunchEvidence::PersistedNotAccepted
            | ApplicationInvocationLaunchEvidence::NeverPersisted => {
                let injection =
                    self.prepare_sprint_result_reassessment_action(invocation.clone())?;
                let mut args = harness.runtime_configuration_args();
                args.extend(injection.configuration_args);
                if matches!(
                    self.sessions
                ...
                let launch = self
                    ...
                    .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
                if launch.launch_accepted {
                    self.mark_sprint_result_receiver(&result, "launch_accepted_at")?;
                }
            }
        }
        Ok(())
    }
    fn mark_sprint_result_receiver(
        &self,
        result: &str,
        column: &str,
    ) -> Result<(), SprintRunnerTransitionError> {
        ...
        let changed=self.connection.lock().map_err(|_|SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?.execute(&format!("UPDATE epic_runner_sprint_result_receivers SET {column}=COALESCE({column},?2) WHERE result_id=?1"),params![result,chrono::Utc::now().to_rfc3339()]).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        if changed != 1 {
            return Err(SprintRunnerTransitionError::Conflict);
        }
        Ok(())
    }
    fn observe_sprint_result_receiver_terminals(&self) -> Result<(), SprintRunnerTransitionError> {
        let rows:Vec<(String,String,String)>=self.connection.lock().map_err(|_|SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?.prepare("SELECT result_id,governing_runner_session_id,reassessment_invocation_id FROM epic_runner_sprint_result_receivers WHERE launch_accepted_at IS NOT NULL AND reassessment_lifecycle_observed_at IS NULL").and_then(|mut s|s.query_map([],|r|Ok((r.get(0)?,r.get(1)?,r.get(2)?)))?.collect::<Result<Vec<_>,_>>()).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        for (result, session, invocation) in rows {
            let session =
                AgentSessionId::new(session).map_err(|_| SprintRunnerTransitionError::Conflict)?;
            let invocation = AgentInvocationId::new(invocation)
                .map_err(|_| SprintRunnerTransitionError::Conflict)?;
            ...
            if let Some(entry) = history.invocations.iter().find(|entry| {
                ...
                self.connection.lock().map_err(|_|SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?.execute("UPDATE epic_runner_sprint_result_receivers SET reassessment_lifecycle_status=COALESCE(reassessment_lifecycle_status,?2),reassessment_lifecycle_observed_at=COALESCE(reassessment_lifecycle_observed_at,?3) WHERE result_id=?1 AND (reassessment_lifecycle_status IS NULL OR reassessment_lifecycle_status=?2)",params![result,lifecycle_status(entry.invocation.status),chrono::Utc::now().to_rfc3339()]).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            }
        }
        Ok(())
    }
    fn sprint_result_reassessment_context(
        &self,
        invocation: &AgentInvocationId,
    ) -> Result<serde_json::Value, SprintRunnerTransitionError> {
        let row:Option<(String,String,String,String)>=self.connection.lock().map_err(|_|SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?.query_row("SELECT r.result_id,r.result_kind,d.continuation_kind,r.sprint_id FROM epic_runner_sprint_result_receivers x JOIN sprint_upward_results r ON r.result_id=x.result_id JOIN sprint_continuation_decisions d ON d.decision_id=r.decision_id WHERE x.reassessment_invocation_id=?1 AND x.delivery_persisted_at IS NOT NULL AND x.harness_bound_at IS NOT NULL AND x.launch_accepted_at IS NOT NULL",[invocation.as_str()],|r|Ok((r.get(0)?,r.get(1)?,r.get(2)?,r.get(3)?))).optional().map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        let Some((_result, kind, continuation, sprint)) = row else {
            return Err(SprintRunnerTransitionError::Forbidden);
        };
        let connection = self.connection.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into())
        })?;
        let attention:Option<String>=connection.query_row("SELECT attention_code FROM sprint_continuation_attentions a JOIN sprint_upward_results r ON r.decision_id=a.decision_id WHERE r.sprint_id=?1 ORDER BY a.recorded_at DESC LIMIT 1",[&sprint],|r|r.get(0)).optional().map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        Ok(
            serde_json::json!({"sprintResult":{"kind":kind,"continuationKind":continuation,"structuredAttention":attention},"boundary":{"requestOrIntentIsNotDelivery":true,"noLaterSprintStartAvailable":true}}),
        )
    }
    fn record_sprint_result_disposition(
        self: &Arc<Self>,
        invocation: &AgentInvocationId,
        input: EpicEscalationReassessmentDisposition,
    ) -> Result<(), SprintRunnerTransitionError> {
        validate_epic_escalation_disposition(&input)?;
        let _ = self.sprint_result_reassessment_context(invocation)?;
        let serialized = serde_json::to_string(&input)
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        let mut connection = self.connection.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into())
        ...
        let fingerprint = stable_id(
            "epic-runner-sprint-result-disposition",
            &format!("{result}:{serialized}"),
        );
        let disposition = stable_id("epic-runner-sprint-result-disposition-id", &result);
        let semantic = stable_id("epic-runner-sprint-result-semantic-reassessment", &result);
        let now = chrono::Utc::now().to_rfc3339();
        let changed=tx.execute("INSERT OR IGNORE INTO epic_runner_sprint_result_dispositions (result_id,disposition_id,movement_kind,details_json,disposition_fingerprint,selected_at,preserves_result) VALUES (?1,?2,?3,?4,?5,?6,1)",params![result,disposition,input.movement_kind,serialized,fingerprint,now]).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        if changed == 0 {
            let exact:bool=tx.query_row("SELECT EXISTS(SELECT 1 FROM epic_runner_sprint_result_dispositions WHERE result_id=?1 AND disposition_id=?2 AND disposition_fingerprint=?3 AND preserves_result=1)",params![result,disposition,fingerprint],|r|r.get(0)).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            if !exact {
                return Err(SprintRunnerTransitionError::Conflict);
            }
        }
    ...
    fn strict_sprint_result_context(
        &self,
        invocation: &AgentInvocationId,
    ) -> Result<serde_json::Value, SprintRunnerTransitionError> {
        let connection = self.connection.lock().map_err(|_| {
        ...
        let row:Option<(String,String,String,Option<String>)>=connection.query_row("SELECT r.result_kind,d.continuation_kind,r.sprint_id,a.attention_code FROM epic_runner_sprint_result_receivers x JOIN sprint_upward_results r ON r.result_id=x.result_id AND r.decision_id=x.decision_id JOIN sprint_continuation_decisions d ON d.decision_id=x.decision_id AND d.sprint_id=x.sprint_id LEFT JOIN sprint_continuation_attentions a ON a.decision_id=x.decision_id WHERE x.reassessment_invocation_id=?1 AND x.delivery_persisted_at IS NOT NULL AND x.harness_bound_at IS NOT NULL AND x.launch_accepted_at IS NOT NULL",[invocation.as_str()],|row|Ok((row.get(0)?,row.get(1)?,row.get(2)?,row.get(3)?))).optional().map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        let Some((kind, continuation, sprint, attention)) = row else {
            return Err(SprintRunnerTransitionError::Forbidden);
        };
        Ok(
            serde_json::json!({"sprintResult":{"kind":kind,"continuationKind":continuation,"structuredAttention":attention},"boundary":{"requestOrIntentIsNotDelivery":true,"applicationMayDeriveOneApprovedSuccessor":true,"sprint":sprint}}),
        )
    }
    fn record_sprint_result_disposition_strict(
        self: &Arc<Self>,
        invocation: &AgentInvocationId,
        input: EpicEscalationReassessmentDisposition,
    ) -> Result<(), SprintRunnerTransitionError> {
        ...
        let disposition_id = stable_id("epic-runner-sprint-result-disposition-id", &result);
        let fingerprint = stable_id(
            "epic-runner-sprint-result-disposition",
            &format!("{result}:{serialized}"),
        );
        ...
        if let Some(request) = input.downstream_request {
            ...
            let id = stable_id("epic-runner-sprint-result-request", &result);
            let fp = stable_id(
                "epic-runner-sprint-result-request",
                &format!("{result}:{json}"),
            );
            tx.execute("INSERT OR IGNORE INTO epic_runner_sprint_result_downstream_requests (result_id,request_id,request_kind,request_json,request_fingerprint,requested_at) VALUES (?1,?2,?3,?4,?5,?6)",params![result,id,kind,json,fp,now]).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        ...
        if let Some(attention) = input.human_external_attention {
            let json = serde_json::to_string(&attention)
                .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            let id = stable_id("epic-runner-sprint-result-attention", &result);
            let fp = stable_id(
                "epic-runner-sprint-result-attention",
                &format!("{result}:{json}"),
            );
            tx.execute("INSERT OR IGNORE INTO epic_runner_sprint_result_attentions (result_id,attention_id,attention_json,attention_fingerprint,requested_at) VALUES (?1,?2,?3,?4,?5)",params![result,id,json,fp,now]).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            let exact:bool=tx.query_row("SELECT EXISTS(SELECT 1 FROM epic_runner_sprint_result_attentions WHERE result_id=?1 AND attention_id=?2 AND attention_json=?3 AND attention_fingerprint=?4)",params![result,id,json,fp],|row|row.get(0)).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            if !exact {
                return Err(SprintRunnerTransitionError::Conflict);
            }
        }
    ...
    fn reconcile_sprint_result_realizations(
        self: &Arc<Self>,
    ) -> Result<(), SprintRunnerTransitionError> {
        ...
        for result in results {
            self.reconcile_sprint_result_realization(&result)?;
        }
        self.reconcile_epic_settlement_boundary()
    }
    fn reconcile_sprint_result_realization(
        self: &Arc<Self>,
        result: &str,
    ) -> Result<(), SprintRunnerTransitionError> {
        ...
        let _guard = lock.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Sprint-result realization lock is poisoned".into(),
            )
        })?;
        let settled = result_kind == "settled"
            && decision_state == "settled"
            && current.as_deref() == Some(decision.as_str());
        ...
        if movement_kind != "advance_to_next_approved_sprint" {
            return self.persist_sprint_result_retained_attention(
                result,
                &decision,
                &sprint,
                &epic,
                "disposition_retains_attention",
            );
        }
        if !settled {
            return self.persist_sprint_result_retained_attention(
        ...
        if prior_unsettled {
            return self.persist_sprint_result_retained_attention(
                ...
                &sprint,
                &epic,
                "nonconsecutive_plan_state",
            );
        }
        let successor:Option<String>=self.connection.lock().map_err(|_|SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?.query_row("SELECT id FROM initiated_sprints WHERE epic_id=?1 AND ordinal > (SELECT ordinal FROM initiated_sprints WHERE id=?2 AND epic_id=?1) ORDER BY ordinal,id LIMIT 1",params![epic,sprint],|row|row.get(0)).optional().map_err(|error|SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        let Some(successor) = successor else {
            return self.persist_sprint_result_terminal_readiness(result, &decision, &sprint, &epic);
        };
        let leap:bool=self.connection.lock().map_err(|_|SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?.query_row("SELECT EXISTS(SELECT 1 FROM initiated_sprints later JOIN sprint_runner_transitions transition ON transition.sprint_id=later.id AND transition.epic_id=later.epic_id WHERE later.epic_id=?1 AND later.ordinal > (SELECT ordinal FROM initiated_sprints WHERE id=?2 AND epic_id=?1) AND later.id<>?3)",params![epic,sprint,successor],|row|row.get(0)).map_err(|error|SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        if leap {
            return self.persist_sprint_result_retained_attention(
                result,
                &decision,
                &sprint,
                &epic,
                "nonconsecutive_successor_transition",
            );
        }
        self.persist_sprint_result_realization(
            result,
            &decision,
            &sprint,
            &epic,
    ...
    fn persist_sprint_result_realization(
        ...
        let realization = stable_id("epic-runner-sprint-result-realization", result);
        let fingerprint = stable_id(
            "epic-runner-sprint-result-realization",
            &format!(
                "{result}:{decision}:{sprint}:{epic}:{outcome}:{}",
                successor.unwrap_or("")
            ),
        );
        let connection = self.connection.lock().map_err(|_| {
    ...
    fn persist_sprint_result_terminal_readiness(
        &self,
        result: &str,
        decision: &str,
        sprint: &str,
        ...
        let readiness = stable_id("epic-runner-sprint-result-terminal-readiness", result);
        let fingerprint = stable_id(
            "epic-runner-sprint-result-terminal-readiness",
            &format!("{result}:{decision}:{sprint}:{epic}"),
        );
        let connection = self.connection.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into())
        })?;
        connection.execute("INSERT OR IGNORE INTO epic_runner_sprint_result_terminal_readiness (result_id,readiness_id,readiness_fingerprint,recorded_at) VALUES (?1,?2,?3,?4)",params![result,readiness,fingerprint,chrono::Utc::now().to_rfc3339()]).map_err(|error|SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        let exact:bool=connection.query_row("SELECT EXISTS(SELECT 1 FROM epic_runner_sprint_result_terminal_readiness WHERE result_id=?1 AND readiness_id=?2 AND readiness_fingerprint=?3)",params![result,readiness,fingerprint],|row|row.get(0)).map_err(|error|SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        if exact {
            Ok(())
        } else {
            Err(SprintRunnerTransitionError::Conflict)
    ...
    fn persist_sprint_result_retained_attention(
        &self,
        result: &str,
        decision: &str,
        sprint: &str,
        ...
        let attention = stable_id("epic-runner-sprint-result-retained-attention", result);
        let fingerprint = stable_id(
            "epic-runner-sprint-result-retained-attention",
            &format!("{result}:{code}"),
        );
        let connection = self.connection.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into())
        })?;
        connection.execute("INSERT OR IGNORE INTO epic_runner_sprint_result_retained_attentions (result_id,attention_id,attention_code,attention_fingerprint,recorded_at) VALUES (?1,?2,?3,?4,?5)",params![result,attention,code,fingerprint,chrono::Utc::now().to_rfc3339()]).map_err(|error|SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        let exact:bool=connection.query_row("SELECT EXISTS(SELECT 1 FROM epic_runner_sprint_result_retained_attentions WHERE result_id=?1 AND attention_id=?2 AND attention_code=?3 AND attention_fingerprint=?4)",params![result,attention,code,fingerprint],|row|row.get(0)).map_err(|error|SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        if exact {
            Ok(())
        } else {
            Err(SprintRunnerTransitionError::Conflict)
    ...
    fn reconcile_work_unit_retry(
        ...
        let retry_exists: bool = self
            ...
            .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        let reporting = AgentInvocationId::new(source.reporting_invocation_id.clone())
            .map_err(|_| SprintRunnerTransitionError::Conflict)?;
        let (_, snapshot) = match self.evidence_snapshot(&reporting, false) {
            Ok(value) => value,
            Err(error) if retry_exists => {
                return self.fail_retry_for_origin(
                    &source.origin_attempt_id,
                    "retry_evidence_revalidation_failed",
                    error,
                )
            }
            Err(error) => return Err(error),
        };
        if snapshot.manifest_json != source.manifest_json
            || snapshot.comparison_fingerprint != source.comparison_fingerprint
            || snapshot.content_fingerprints_json != source.content_fingerprints_json
        {
            return if retry_exists {
                self.fail_retry_for_origin(
                    &source.origin_attempt_id,
                    "retry_evidence_revalidation_failed",
        ...
        let authority = match self
            .authority_repository
            .load_initiated_sprint_git_authority(&source.authority_id)
        {
            Ok(Some(authority)) => authority,
            Ok(None) if retry_exists => {
                return self.fail_retry_for_origin(
                    &source.origin_attempt_id,
                    "retry_authority_revalidation_failed",
                    SprintRunnerTransitionError::Conflict,
                )
            }
            ...
            Err(_) if retry_exists => {
                return self.fail_retry_for_origin(
                    ...
                    "retry_authority_revalidation_failed",
                    SprintRunnerTransitionError::Unavailable(
                        "load retry Sprint Git authority".into(),
                    ),
                )
            }
            Err(_) => {
        ...
        let pinned_reporting = match handler.load_pinned_implementer_revision(
            &source.reporting_revision_id,
            &source.reporting_configuration_digest,
            &source.reporting_repository_commit_ref,
        ) {
            Ok(value) => value,
            Err(_) if retry_exists => {
                return self.fail_retry_for_origin(
                    &source.origin_attempt_id,
                    "retry_harness_revalidation_failed",
                    SprintRunnerTransitionError::Conflict,
                )
            }
            Err(_) => return Err(SprintRunnerTransitionError::Conflict),
        };
        let origin_package = match handler.construct_for_pinned_profile(
            &source.origin_attempt_id,
            WorkUnitHarnessRole::Implementer,
            pinned_reporting.profile,
        ) {
            Ok(value) => value,
            Err(_) if retry_exists => {
                return self.fail_retry_for_origin(
                    &source.origin_attempt_id,
                    "retry_execution_support_revalidation_failed",
                    SprintRunnerTransitionError::Conflict,
                )
            }
            Err(_) => return Err(SprintRunnerTransitionError::Conflict),
        ...
        let source_baseline = match self
            .connection
            .lock()
            .map_err(|_| {
                SprintRunnerTransitionError::Unavailable(
            ...
            .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?
        {
            ...
            None if retry_exists => {
                return self.fail_retry_for_origin(
                    &source.origin_attempt_id,
                    "retry_authorization_revalidation_failed",
                    SprintRunnerTransitionError::Forbidden,
                )
            }
            None => return Err(SprintRunnerTransitionError::Forbidden),
        };
        let (candidate_commit, candidate_tree) =
            match retry_git_candidate_facts(&authority, &source_root, &source_baseline) {
                Ok(value) => value,
                Err(error) if retry_exists => {
                    return self.fail_retry_for_origin(
                        &source.origin_attempt_id,
                        "retry_candidate_revalidation_failed",
                        error,
                    )
                }
                Err(error) => return Err(error),
        ...
        if next_ordinal != source.origin_ordinal + 1 {
            return Err(SprintRunnerTransitionError::Conflict);
        }
        ...
        let invocation_id = stable_id(
            "work-unit-next-attempt-implementer-invocation",
            &retry_attempt,
        );
        let capture_intent = stable_id("work-unit-next-attempt-capture", &source.origin_attempt_id);
        let private_ref = format!(
            "refs/codex-orchestrator/retry/{}",
            stable_id("work-unit-next-attempt-ref", &retry_attempt)
        );
        ...
        let capture_fingerprint = stable_id(
            "work-unit-next-attempt-capture-lineage",
            &format!(
                "{}:{}:{}:{}:{}",
                source.origin_attempt_id,
                source.review_invocation_id,
                source.decision_fingerprint,
                candidate_commit,
                candidate_tree
            ),
        );
        let desired = handler.current_implementer_revision().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "immutable later Implementer Harness revision unavailable".into(),
            )
        })?;
        let now = chrono::Utc::now().to_rfc3339();
        {
        ...
        let (verified_commit, verified_tree) =
            match retry_git_candidate_facts(&authority, &source_root, &source_baseline) {
                Ok(facts) => facts,
                Err(error) => {
                    return self.fail_retry(
                        &retry_attempt,
                        "retry_candidate_revalidation_failed",
                        error,
                    )
        ...
        if verified_commit != candidate_commit || verified_tree != candidate_tree {
            return self.fail_retry(
                &retry_attempt,
                "retry_candidate_drift",
                SprintRunnerTransitionError::Conflict,
            );
        }
        if let Err(error) = ensure_private_retry_ref(
            Path::new(&authority.repository_root),
            &private_ref,
            &candidate_commit,
        ) {
    ...
    fn launch_retry_implementer(
        ...
        let git_authority = match self
            .authority_repository
            .load_initiated_sprint_git_authority(&authority)
        {
            Ok(Some(authority)) => authority,
            Ok(None) => {
                return self.fail_retry(
                    &attempt,
                    "retry_authority_missing",
                    SprintRunnerTransitionError::Conflict,
                )
            }
            Err(_) => {
        ...
        let pinned_target =
            match private_retry_ref_target(Path::new(&git_authority.repository_root), &private_ref)
            {
                Ok(target) => target,
                Err(error) => {
                    return self.fail_retry(
                        &attempt,
                        "retry_private_ref_verification_failed",
                        error,
                    )
                }
            };
        if pinned_target != seed {
            return self.fail_retry(
                &attempt,
                "retry_private_ref_mismatch",
                SprintRunnerTransitionError::Conflict,
            );
        }
        let pinned = match handler.load_pinned_implementer_revision(&revision, &digest, &commit) {
            ...
            Err(_) => {
                return self.fail_retry(
                    &attempt,
                    "retry_harness_revision_invalid",
                    SprintRunnerTransitionError::Conflict,
                )
            }
        };
        if let Err(error) = handler.authorize_implementer_attempt_at_seed(
            &attempt,
            &work_unit_id,
            &authority,
            Some(seed.clone()),
            ...
            return self.fail_retry(
                &attempt,
                "retry_execution_authorization_failed",
                SprintRunnerTransitionError::Conflict,
            );
        }
        ...
        let package = match handler.construct_for_pinned_profile(
            ...
            Err(_) => {
                return self.fail_retry(
                    &attempt,
                    "retry_execution_support_failed",
                    SprintRunnerTransitionError::Unavailable(
                        "retry execution-support grant failed".into(),
                    ),
                )
            }
        };
        if let Err(error) = retry_git_workspace_facts(
            &git_authority,
            Path::new(package.working_directory()),
            &seed,
            &tree,
        ) {
        ...
        let session =
            AgentSessionId::new(session_id).map_err(|_| SprintRunnerTransitionError::Conflict)?;
        let invocation = AgentInvocationId::new(invocation_id)
            .map_err(|_| SprintRunnerTransitionError::Conflict)?;
        ...
        if let Err(error) =
            self.sessions
                .create_application_session(CreateApplicationAgentSessionCommand {
                    session_id: session.clone(),
                    session: CreateAgentSessionCommand {
                        title: Some("Work Unit Later Attempt Implementer".into()),
                        working_directory: Some(package.working_directory().into()),
                        requested_options: runtime.requested_options.clone(),
                    },
                })
        {
            return self.fail_retry(
                &attempt,
                "retry_session_creation_failed",
                SprintRunnerTransitionError::Unavailable(error.to_string()),
            );
        }
            ...
            .find(|entry| entry.invocation.id == invocation)
        {
            ...
            if existing.invocation.status.is_terminal() {
                return self.fail_retry(
                    &attempt,
                    "retry_terminal_invocation_incompatible",
                    SprintRunnerTransitionError::Conflict,
                );
            }
        }
        ...
        if let Err(error) = self.sessions.prepare_idempotent_application_invocation(
            SendIdempotentApplicationAgentSessionMessageCommand {
                invocation_id: invocation.clone(),
                message: SendAgentSessionMessageCommand {
                    session_id: Some(session.clone()),
            ...
            return self.fail_retry(
                &attempt,
                "retry_invocation_preparation_failed",
                SprintRunnerTransitionError::Unavailable(error.to_string()),
            );
        }
            ...
            .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))
        {
            ...
            Ok(ApplicationInvocationLaunchEvidence::LaunchAccepted) => {
                self.mark_retry(&attempt, "launch_accepted_at")?;
            }
            Ok(ApplicationInvocationLaunchEvidence::PersistedNotAccepted) => {
                ...
                if launch.launch_accepted {
                    ...
                    return self.fail_retry(
                        &attempt,
                        if terminal {
                            "retry_terminal_launch_failed"
                        } else {
                            "retry_launch_not_accepted"
                        },
                        SprintRunnerTransitionError::Unavailable(
                            "retry Implementer launch was not accepted".into(),
                        ),
                    );
                }
            }
            Ok(ApplicationInvocationLaunchEvidence::NeverPersisted) => {
                return self.fail_retry(
                    &attempt,
                    "retry_launch_missing_prepared_invocation",
                    SprintRunnerTransitionError::Conflict,
                )
            }
        }
    ...
    fn mark_retry(
        ...
        self.mark_retry_at(retry_attempt_id, column, chrono::Utc::now().to_rfc3339())
    }
    fn mark_retry_at(
        &self,
        retry_attempt_id: &str,
        ...
        if ![
            ...
            "isolated_worktree_ready_at",
            "implementer_session_created_at",
            "implementer_invocation_prepared_at",
            "implementer_harness_bound_at",
            "launch_requested_at",
            "launch_accepted_at",
            "provider_activation_observed_at",
    ...
    fn record_retry_failure(
        ...
        self.connection
            ...
            .execute(
                "UPDATE work_unit_retry_attempts SET failure_reason=?2 WHERE retry_attempt_id=?1",
                params![retry_attempt_id, reason],
            )
    ...
    fn fail_retry_for_invocation<T>(
        &self,
        invocation: &AgentInvocationId,
        reason: &str,
    ...
    fn fail_retry<T>(
        ...
        retry_attempt_id: &str,
        reason: &str,
        error: SprintRunnerTransitionError,
    ) -> Result<T, SprintRunnerTransitionError> {
        self.record_retry_failure(retry_attempt_id, reason)?;
        Err(error)
    }
    fn fail_retry_for_origin<T>(
        &self,
        ...
        self.connection
            ...
            .execute(
                "UPDATE work_unit_retry_attempts SET failure_reason=?2 WHERE origin_attempt_id=?1",
                params![origin_attempt_id, reason],
            )
            .map_err(|database_error| {
                SprintRunnerTransitionError::Unavailable(database_error.to_string())
            })?;
    ...
    fn clear_retry_failure(
        ...
        self.connection
            ...
            .execute(
                "UPDATE work_unit_retry_attempts SET failure_reason=NULL WHERE retry_attempt_id=?1",
                [retry_attempt_id],
            )
            .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        Ok(())
    ...
    fn reconcile_work_unit_handler(
        ...
        let existing: bool = self
            ...
            .query_row(
                "SELECT EXISTS(SELECT 1 FROM work_unit_handler_activations WHERE work_unit_id=?1)",
                [work_unit_id],
                |row| row.get(0),
            )
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        if existing {
            ...
            let desired = handler.current_handler_revision().map_err(|_| {
                SprintRunnerTransitionError::Unavailable(
                    "load current immutable Handler revision".into(),
                )
            })?;
            self.connection.lock().map_err(|_| SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?.execute(
        ...
        let configuration_digest =
            configuration_digest.ok_or(SprintRunnerTransitionError::Conflict)?;
        let repository_commit_ref =
            repository_commit_ref.ok_or(SprintRunnerTransitionError::Conflict)?;
        let harness = handler
            .load_pinned_handler_revision(
                &revision_id,
            ...
            .map_err(|error| {
                SprintRunnerTransitionError::Unavailable(format!(
                    "pinned Handler revision evidence is invalid or unavailable: {error:?}"
                ))
            })?;
        if harness.harness_key != harness_key
            || harness.profile.version
                != u16::try_from(harness_version)
                    .map_err(|_| SprintRunnerTransitionError::Conflict)?
        ...
        if handler
            .authorize_handler_attempt(&attempt_id, work_unit_id, &authority)
            .is_err()
        {
            self.fail_handler_recovery(
                work_unit_id,
                "handler_execution_support_authorization_failed",
            )?;
            return Ok(());
        }
        let package = match handler.construct_for_pinned_profile(
        ...
        self.sessions
            .create_application_session(CreateApplicationAgentSessionCommand {
                session_id: session.clone(),
                session: CreateAgentSessionCommand {
                    title: Some("Work Unit Handler".into()),
                    working_directory: Some(package.working_directory().into()),
                    requested_options: runtime.requested_options.clone(),
                },
            })
        ...
        match launch_evidence {
            ApplicationInvocationLaunchEvidence::LaunchAccepted => {
                self.mark_handler(work_unit_id, "launch_accepted_at")?;
                self.mark_handler(work_unit_id, "handler_ready_at")?;
            }
            ApplicationInvocationLaunchEvidence::PersistedNotAccepted => {
                let launch = self
                    .sessions
    ...
    fn reconcile_handler_action_continuation(
        ...
        let Some((attempt_id, session_id, original_invocation_id, _sprint_id, status)) = original
        else {
            return Ok(());
        };
        ...
        let existing: bool = self.connection.lock()
            ...
            .map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?;
        if !existing {
            let desired = handler.current_handler_action_revision().map_err(|_| {
                SprintRunnerTransitionError::Unavailable(
                    "load immutable Handler action revision".into(),
                )
            })?;
        ...
        let pinned = handler
            .load_pinned_handler_revision(&revision_id, &digest, &commit)
            .map_err(|_| SprintRunnerTransitionError::Conflict)?;
        if !pinned.profile.mcp.required
            || pinned.profile.mcp.enabled_tools != ["request_work_unit_implementer"]
        {
        ...
        let package = handler
            .construct_for_pinned_profile(&attempt_id, WorkUnitHarnessRole::Handler, pinned.profile)
            .map_err(|_| SprintRunnerTransitionError::Conflict)?;
        let session = AgentSessionId::new(session_id)
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        let invocation = AgentInvocationId::new(invocation_id)
            ...
            Ok(ApplicationInvocationLaunchEvidence::PersistedNotAccepted) => {
                ...
                if action_status != "pending" {
                    return self.fail_handler_action(
                        ...
                        SprintRunnerTransitionError::Unavailable(
                            "Handler action continuation is terminal without launch acceptance"
                                .into(),
                        ),
                    );
                }
    ...
    fn request_work_unit_implementer(
        self: &Arc<Self>,
        handler_invocation: &AgentInvocationId,
    ) -> Result<(), SprintRunnerTransitionError> {
        self.request_work_unit_implementer_inner(handler_invocation, true)
    }

    fn request_work_unit_implementer_inner(
        self: &Arc<Self>,
        handler_invocation: &AgentInvocationId,
        require_active_action: bool,
    ) -> Result<(), SprintRunnerTransitionError> {
        let handler = self
            .work_unit_handler
            .lock()
            ...
            .ok_or_else(|| {
                SprintRunnerTransitionError::Unavailable(
                    "Work Unit Handler activation is unavailable".into(),
                )
            })?;
        ...
        if specification.trim().is_empty() || specification.len() > 16_000 {
            return Err(SprintRunnerTransitionError::Forbidden);
        }
        if handler_attempt != stable_id("work-unit-handler-attempt", &work_unit)
            ...
            || handler_invocation.as_str()
                != stable_id("work-unit-handler-action-invocation", &handler_attempt)
        {
            return Err(SprintRunnerTransitionError::Forbidden);
        }
        let action = handler
            .load_pinned_handler_revision(&handler_revision, &handler_digest, &handler_commit)
            .map_err(|_| SprintRunnerTransitionError::Forbidden)?;
        if !action.profile.mcp.required
            || action.profile.mcp.enabled_tools != ["request_work_unit_implementer"]
        {
            return Err(SprintRunnerTransitionError::Forbidden);
        }
        ...
        let attempt = handler_attempt.clone();
        let session_id = stable_id("work-unit-implementer-session", &handler_attempt);
        let invocation_id = stable_id("work-unit-implementer-invocation", &handler_attempt);
        let lock = self.transition_lock(&sprint)?;
        let _guard = lock.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("Work Unit activation lock is poisoned".into())
        })?;
        ...
        if !existing {
            let desired = handler.current_implementer_revision().map_err(|_| {
                SprintRunnerTransitionError::Unavailable(
                    "immutable Implementer Harness revision unavailable".into(),
                )
            })?;
            ...
            || invocation_id != stable_id("work-unit-implementer-invocation", &handler_attempt)
        {
            return self.fail_implementer(
                &work_unit,
                "pinned_implementer_identity_invalid",
                SprintRunnerTransitionError::Conflict,
            );
        }
        let pinned = match handler.load_pinned_implementer_revision(&revision_id, &digest, &commit)
        {
            ...
            Err(_) => {
                return self.fail_implementer(
                    &work_unit,
                    "pinned_implementer_harness_invalid",
                    SprintRunnerTransitionError::Conflict,
                )
            }
        };
        if handler
            .authorize_implementer_attempt(&attempt, &work_unit, &authority)
            .is_err()
        {
            return self.fail_implementer(
                &work_unit,
                "implementer_authorization_failed",
                SprintRunnerTransitionError::Conflict,
            );
        }
        ...
        let package = match handler.construct_for_pinned_profile(
            ...
            Err(_) => {
                return self.fail_implementer(
                    &work_unit,
                    "implementer_execution_support_failed",
                    SprintRunnerTransitionError::Unavailable(
                        "Implementer Harness package construction failed".into(),
                    ),
                )
            }
        };
        ...
        let session = match AgentSessionId::new(session_id) {
            ...
            Err(error) => {
                return self.fail_implementer(
                    &work_unit,
                    "implementer_session_identity_invalid",
                    SprintRunnerTransitionError::Unavailable(error.to_string()),
                )
            }
        };
        let invocation = match AgentInvocationId::new(invocation_id) {
            ...
            Err(error) => {
                return self.fail_implementer(
                    &work_unit,
                    "implementer_invocation_identity_invalid",
                    SprintRunnerTransitionError::Unavailable(error.to_string()),
                )
            }
        };
        ...
        if let Err(error) =
            self.sessions
                .create_application_session(CreateApplicationAgentSessionCommand {
                    session_id: session.clone(),
                    session: CreateAgentSessionCommand {
                        title: Some("Work Unit Implementer".into()),
                        working_directory: Some(package.working_directory().into()),
                        requested_options: runtime.requested_options.clone(),
                    },
                })
        {
            return self.fail_implementer(
                &work_unit,
                "implementer_session_creation_failed",
                SprintRunnerTransitionError::Unavailable(error.to_string()),
            );
        }
        ...
        if let Err(error) = self.sessions.prepare_idempotent_application_invocation(
            ...
            return self.fail_implementer(
                &work_unit,
                "implementer_invocation_preparation_failed",
                SprintRunnerTransitionError::Unavailable(error.to_string()),
            );
        }
            ...
            .is_err()
        {
            return self.fail_implementer(
                &work_unit,
                "implementer_harness_binding_failed",
                SprintRunnerTransitionError::Conflict,
            );
        }
            ...
            .application_invocation_launch_evidence(&invocation, &session)
        {
            Err(error) => {
                return self.fail_implementer(
                    &work_unit,
                    "implementer_launch_evidence_failed",
                    SprintRunnerTransitionError::Unavailable(error.to_string()),
                )
            }
            Ok(ApplicationInvocationLaunchEvidence::LaunchAccepted) => {
            ...
            Ok(ApplicationInvocationLaunchEvidence::NeverPersisted) => {
                return self.fail_implementer(
                    &work_unit,
                    "implementer_launch_missing_prepared_invocation",
                    SprintRunnerTransitionError::Conflict,
                )
            }
        };
    ...
    fn mark_implementer(
        ...
        self.mark_implementer_at(work_unit, column, chrono::Utc::now().to_rfc3339())
    }
    fn mark_implementer_at(
        &self,
        work_unit: &str,
        column: &str,
        at: String,
        ...
        {
            return Err(SprintRunnerTransitionError::Unavailable(
                "invalid Implementer activation stage".into(),
            ));
        }
        self.connection.lock().map_err(|_|SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?.execute(&format!("UPDATE work_unit_implementer_activations SET {column}=COALESCE({column},?2) WHERE work_unit_id=?1"),params![work_unit,at]).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        Ok(())
    }
    fn record_implementer_candidate_failure(
        &self,
        work_unit: &str,
        attempt: &str,
        ordinal: i64,
        ...
        {
            return Err(SprintRunnerTransitionError::Unavailable(
                "invalid Implementer candidate failure reason".into(),
            ));
        }
    ...
    fn clear_handler_action_failure(
        &self,
        work_unit: &str,
    ) -> Result<(), SprintRunnerTransitionError> {
    ...
    fn clear_handler_recovery_failure(
        &self,
        work_unit: &str,
    ) -> Result<(), SprintRunnerTransitionError> {
        self.connection.lock().map_err(|_| SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?
    ...
    fn mark_handler_action_at(
        ...
        {
            return Err(SprintRunnerTransitionError::Unavailable(
                "invalid Handler action continuation stage".into(),
            ));
        }
        self.connection.lock().map_err(|_| SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?
    ...
    fn mark_handler_at(
        ...
        if ![
            ...
            "execution_support_granted_at",
            "isolated_worktree_ready_at",
            "handler_session_created_at",
            "handler_invocation_prepared_at",
        ...
        {
            return Err(SprintRunnerTransitionError::Unavailable(
                "invalid Handler activation stage".into(),
            ));
        }
    ...
    fn planner_point_for_invocation(
        &self,
        invocation_id: &AgentInvocationId,
    ) -> Result<(String, String), SprintRunnerTransitionError> {
        let desired = conversation_harness::profile(ConversationHarnessRole::WorkSlicePlanner)
            .map_err(SprintRunnerTransitionError::Unavailable)?;
        self.connection
            .lock()
    ...
    pub(crate) fn read_work_slice_planning_context(
        &self,
        invocation_id: &AgentInvocationId,
    ) -> Result<serde_json::Value, SprintRunnerTransitionError> {
        ...
        if current_sprint != sprint_id
            || intended_movement.trim().is_empty()
            || intended_movement.len() > 4_000
        {
            return Err(SprintRunnerTransitionError::Forbidden);
        }
        Ok(
            serde_json::json!({"planningPoint":point,"sprint":current_sprint,"currentSprint":{"id":sprint_id,"intendedMovement":intended_movement},"repositoryRoute":route,"hasCurrentRevision":revision.is_some(),"validation":validation,"refinementRequested":refinement_requested.is_some()}),
        )
    }
    pub(crate) fn submit_work_slice_proposal(
        self: &Arc<Self>,
        invocation_id: &AgentInvocationId,
        input: WorkSliceProposal,
    ) -> Result<serde_json::Value, SprintRunnerTransitionError> {
        validate_work_slice_proposal(&input)?;
        let (point, sprint) = self.planner_point_for_invocation(invocation_id)?;
        let lock = self.transition_lock(&sprint)?;
        let _guard = lock.lock().map_err(|_| {
        ...
        let encoded = serde_json::to_string(&input)
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        let fingerprint = stable_id("work-slice-proposal-content", &encoded);
        let conn = self.connection.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into())
        })?;
        ...
        if let Some((revision, _)) = parent.as_ref() {
            let state: (String,Option<String>)=conn.query_row("SELECT content_fingerprint,refinement_requested_at FROM work_slice_proposal_revisions WHERE revision_id=?1",[revision],|r|Ok((r.get(0)?,r.get(1)?))).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;
            if state.1.is_none() {
                return if state.0 == fingerprint {
                    Ok(
        ...
        if let Some((prior, _)) = parent.as_ref() {
            conn.execute(
                "UPDATE work_slice_proposal_revisions SET is_current=0 WHERE revision_id=?1",
                [prior],
            )
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        }
        let number = parent.as_ref().map_or(1, |(_, n)| n + 1);
        let command = stable_id(
            "work-slice-proposal-command",
            &format!("{point}:{:?}:{fingerprint}", parent.as_ref().map(|p| &p.0)),
        );
        let revision = stable_id(
            "work-slice-proposal-revision",
            &format!("{point}:{number}:{command}"),
        );
        let now = chrono::Utc::now().to_rfc3339();
    ...
    pub(crate) fn request_work_slice_refinement(
        &self,
        invocation_id: &AgentInvocationId,
        input: WorkSliceRefinement,
    ) -> Result<(), SprintRunnerTransitionError> {
        validate_outcome(&input.reason)?;
        let (point, sprint) = self.planner_point_for_invocation(invocation_id)?;
        let lock = self.transition_lock(&sprint)?;
        let _guard = lock.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("planning lock is poisoned".into())
    ...
    pub(crate) fn complete_work_slice_planning(
        self: &Arc<Self>,
        invocation_id: &AgentInvocationId,
        _input: WorkSliceCompletion,
    ) -> Result<(), SprintRunnerTransitionError> {
        let (point, sprint) = self.planner_point_for_invocation(invocation_id)?;
        let lock = self.transition_lock(&sprint)?;
        let _guard = lock.lock().map_err(|_| {
    ...
    fn reconcile_work_slice_planner(
        self: &Arc<Self>,
        sprint_id: &str,
    ) -> Result<(), SprintRunnerTransitionError> {
        let lock = self.transition_lock(sprint_id)?;
        ...
        let Some((point, session_id, invocation_id, route, key, version, harness_json)) = request
        else {
            return Ok(());
        };
        ...
        let (harness, serialized) = match harness_json {
            Some(snapshot) => {
                let harness =
                    conversation_harness::pinned_profile_snapshot(&key, version, &snapshot)
                        .map_err(|_| SprintRunnerTransitionError::Conflict)?;
                (harness, snapshot)
            }
            None => {
        ...
        self.sessions
            .create_application_session(CreateApplicationAgentSessionCommand {
                session_id: session.clone(),
                session: CreateAgentSessionCommand {
                    title: Some("Work Slice Planner".into()),
                    working_directory: Some(route.clone()),
                    requested_options: harness.runtime_options(),
                },
            })
            .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        ...
        self.reconcile_productive_work_slice_planner(
            sprint_id, &point, session, invocation, route, harness,
        )
    }
    ...
    fn mark_planner(
        ...
        if ![
            "planner_session_created_at",
            "planner_invocation_created_at",
            "planner_harness_applied_at",
            "planner_launch_requested_at",
            "planner_launch_accepted_at",
            "planner_ready_at",
        ]
        .contains(&column)
        {
            return Err(SprintRunnerTransitionError::Unavailable(
                "invalid Planner materialization stage".into(),
            ));
        }
        let sql = if harness_json.is_some() {
            format!("UPDATE work_slice_planning_requests SET {column}=COALESCE({column},?2),planner_harness_json=COALESCE(planner_harness_json,?3) WHERE sprint_id=?1 AND is_current=1")
        } else {
            format!("UPDATE work_slice_planning_requests SET {column}=COALESCE({column},?2) WHERE sprint_id=?1 AND is_current=1")
        };
        let conn = self.connection.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Sprint Runner transition database lock is poisoned".into(),
        ...
        if let Some(harness_json) = harness_json {
            conn.execute(
                &sql,
                params![sprint_id, chrono::Utc::now().to_rfc3339(), harness_json],
            )
        } else {
            conn.execute(&sql, params![sprint_id, chrono::Utc::now().to_rfc3339()])
        }
        .map_err(|e| SprintRunnerTransitionError::Unavailable(e.to_string()))?;
        Ok(())
    }
    ...
    pub(crate) fn set_test_reconcile_snapshot_hook(&self, hook: Arc<dyn Fn() + Send + Sync>) {
        *self
            .test_reconcile_snapshot_hook
            .lock()
            .expect("test reconciliation hook") = Some(hook);
    }

    #[cfg(test)]
    pub(crate) fn set_test_origin_snapshot_hook(&self, hook: Arc<dyn Fn() + Send + Sync>) {
        *self
            .test_origin_snapshot_hook
            .lock()
            .expect("test origin hook") = Some(hook);
    }
    ...
    fn run_test_hook(&self, origin: bool) {
        let hook = if origin {
            self.test_origin_snapshot_hook
                .lock()
                .expect("test origin hook")
                .clone()
        } else {
            self.test_reconcile_snapshot_hook
                .lock()
                .expect("test reconciliation hook")
                .clone()
        };
...
fn handler_review_reconciliation_lock(
    ...
    let mut locks = LOCKS
        ...
        .map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Handler review lock registry is poisoned".into(),
            )
        })?;
    Ok(locks
...
fn sprint_result_realization_lock(
    ...
    let mut locks = LOCKS
        ...
        .map_err(|_| {
            SprintRunnerTransitionError::Unavailable(
                "Sprint-result realization lock registry is poisoned".into(),
            )
        })?;
    Ok(locks
...
fn validate_work_slice_proposal(
    value: &WorkSliceProposal,
) -> Result<(), SprintRunnerTransitionError> {
    if value.lanes.is_empty() || value.lanes.len() > 32 {
        return Err(SprintRunnerTransitionError::Invalid);
    }
    validate_outcome(&value.objective)?;
    let mut names = std::collections::HashSet::new();
    for lane in &value.lanes {
        validate_outcome(&lane.title)?;
        validate_outcome(&lane.specification)?;
        if !names.insert(lane.title.as_str()) {
            return Err(SprintRunnerTransitionError::Invalid);
        }
    }
    for lane in &value.lanes {
        for dependency in &lane.depends_on {
            if dependency == &lane.title || !names.contains(dependency.as_str()) {
                return Err(SprintRunnerTransitionError::Invalid);
            }
        }
    }
    fn visit<'a>(
        ...
        if seen.insert(name) {
            let lane = lanes
                .iter()
                .find(|lane| lane.title == name)
                .expect("validated lane");
            for dep in &lane.depends_on {
                if !visit(dep, lanes, active, seen) {
                    return false;
                }
            }
        }
    ...
    for lane in &value.lanes {
        if !visit(&lane.title, &value.lanes, &mut active, &mut seen) {
            return Err(SprintRunnerTransitionError::Invalid);
        }
    }
    Ok(())
...
fn validate_handler_review_return(
    value: &HandlerReviewReturnReason,
) -> Result<(), SprintRunnerTransitionError> {
    if !safe_id(&value.code)
        || value.code.len() > 96
        || value.explanation.trim().is_empty()
        || value.explanation.len() > 2_000
    {
        return Err(SprintRunnerTransitionError::Invalid);
    }
    Ok(())
}
fn validate_handback_disposition(
    value: &SprintHandbackDisposition,
) -> Result<(), SprintRunnerTransitionError> {
    if !safe_id(&value.movement_kind) || value.movement_kind.len() > 96 {
        return Err(SprintRunnerTransitionError::Invalid);
    }
    ...
    match value.movement_kind.as_str() {
        "continue_eligible_work" => {
            if value
                .eligible_work_summary
                .as_deref()
                .map(validate_outcome)
                .transpose()?
                .is_none()
                || value.dependency_owner.is_some()
                || value.dependency_owner_classification.is_some()
                || value.enabling_result.is_some()
                || value.resumption_path.is_some()
        ...
        "wait_for_agent_dependency" => {
            ...
            if value.dependency_owner_classification.is_none()
                || value
                    .enabling_result
                    .as_deref()
                    .map(validate_outcome)
            ...
            if ["human", "external", "approval", "manual", "user"]
                .iter()
...
fn validate_epic_escalation_disposition(
    value: &EpicEscalationReassessmentDisposition,
) -> Result<(), SprintRunnerTransitionError> {
    if !safe_id(&value.movement_kind) || value.movement_kind.len() > 96 {
        return Err(SprintRunnerTransitionError::Invalid);
    }
    validate_outcome(&value.rationale)?;
    if let Some(intent) = value.considered_intent.as_deref() {
        validate_outcome(intent)?
    }
    if let Some(request) = &value.downstream_request {
        validate_outcome(&request.request)?;
        validate_outcome(&request.resumption_path)?
    }
    if let Some(attention) = &value.human_external_attention {
        for text in [
            &attention.reason,
            &attention.authority_needed,
            &attention.evidence_context,
            &attention.resumption_path,
        ] {
            validate_outcome(text)?
        }
    }
    match value.movement_kind.as_str() {
        "return_context_to_sprint_runner" => {
            if !matches!(
                value.downstream_request.as_ref().map(|r| &r.target),
                Some(EpicEscalationDownstreamTarget::SprintRunner)
            ) || value
                .downstream_request
                .as_ref()
                .and_then(|r| r.dependency.as_ref())
                .is_some()
                || value.human_external_attention.is_some()
                || value.considered_intent.is_some()
            {
                return Err(SprintRunnerTransitionError::Invalid);
            }
        }
        ...
        "human_or_external_attention" => {
            if value.downstream_request.is_some()
                || value.human_external_attention.is_none()
                || value.considered_intent.is_some()
            {
                return Err(SprintRunnerTransitionError::Invalid);
            }
        ...
        "consider_other_epic_work" | "advance_to_next_approved_sprint" => {
            if value.downstream_request.is_some()
                || value.human_external_attention.is_some()
                || value.considered_intent.is_none()
            {
                return Err(SprintRunnerTransitionError::Invalid);
            }
        }
...
fn map_authority_bind_error(
    ...
    match error {
        ...
        | BindInitiatedSprintGitAuthorityError::Unavailable => {
            SprintRunnerTransitionError::Unavailable(
                "application Sprint Git authority runtime unavailable".into(),
            )
        }
...
impl ApplicationSprintGitAuthorityRuntime {
    fn new() -> Result<Self, SprintRunnerTransitionError> {
        let source_root = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            ...
            .map_err(|_| {
                SprintRunnerTransitionError::Unavailable(
                    "canonicalize application source root".into(),
                )
            })?;
        Ok(Self { source_root })
    }
    ...
    fn git(&self, arguments: &[&str]) -> Result<String, BindInitiatedSprintGitAuthorityError> {
        ...
        if !output.status.success()
            || output.stdout.len() > 256_000
            || output.stderr.len() > 256_000
        {
...
fn retry_git_text(root: &Path, arguments: &[&str]) -> Result<String, SprintRunnerTransitionError> {
    let output = Command::new("git")
        ...
        .map_err(|error| {
            SprintRunnerTransitionError::Unavailable(format!(
                "retry Git invocation failed: {error}"
            ))
        })?;
...
fn retry_git_candidate_facts(
    ...
    let root = root
        .canonicalize()
        .map_err(|_| SprintRunnerTransitionError::Conflict)?;
    let authority_root = PathBuf::from(&authority.worktree_root)
        .canonicalize()
        .map_err(|_| SprintRunnerTransitionError::Conflict)?;
    let repository_root = PathBuf::from(&authority.repository_root)
        .canonicalize()
        .map_err(|_| SprintRunnerTransitionError::Conflict)?;
    let expected_common = PathBuf::from(&authority.repository_common_dir)
        .canonicalize()
        .map_err(|_| SprintRunnerTransitionError::Conflict)?;
    if root == authority_root
        || root == repository_root
        || retry_git_text(&root, &["status", "--porcelain"])? != ""
    {
        return Err(SprintRunnerTransitionError::Conflict);
    }
    let top = PathBuf::from(retry_git_text(&root, &["rev-parse", "--show-toplevel"])?)
        .canonicalize()
        .map_err(|_| SprintRunnerTransitionError::Conflict)?;
    let common = PathBuf::from(retry_git_text(
        &root,
        &["rev-parse", "--path-format=absolute", "--git-common-dir"],
    )?)
    .canonicalize()
    .map_err(|_| SprintRunnerTransitionError::Conflict)?;
    if top != root || common != expected_common {
        return Err(SprintRunnerTransitionError::Conflict);
    }
    ...
    if commit.len() != 40
        ...
        || retry_git_text(
            &root,
            &["merge-base", "--is-ancestor", source_baseline, &commit],
        )
        .is_err()
    {
        return Err(SprintRunnerTransitionError::Conflict);
...
fn retry_git_workspace_facts(
    ...
    if root == authority_root
        || root == repository_root
        || retry_git_text(&root, &["status", "--porcelain"])? != ""
        || retry_git_text(&root, &["rev-parse", "--abbrev-ref", "HEAD"])? != "HEAD"
    {
        return Err(SprintRunnerTransitionError::Conflict);
    }
    let top = PathBuf::from(retry_git_text(&root, &["rev-parse", "--show-toplevel"])?)
...
fn private_retry_ref_target(
    repository_root: &Path,
    private_ref: &str,
) -> Result<String, SprintRunnerTransitionError> {
    if !private_ref.starts_with("refs/codex-orchestrator/retry/")
        || !safe_id(private_ref.rsplit('/').next().unwrap_or(""))
    {
        return Err(SprintRunnerTransitionError::Conflict);
    }
    retry_git_text(
...
fn ensure_private_retry_ref(
    ...
    let existing = Command::new("git")
        .args(["show-ref", "--verify", "--quiet", private_ref])
        .current_dir(repository_root)
        .env("GIT_TERMINAL_PROMPT", "0")
        .output()
        .map_err(|error| {
            SprintRunnerTransitionError::Unavailable(format!(
                "inspect retry candidate pin: {error}"
            ))
        })?;
    if existing.status.success() {
        return if private_retry_ref_target(repository_root, private_ref)? == commit {
            Ok(())
        } else {
            Err(SprintRunnerTransitionError::Conflict)
        };
    ...
    if !output.status.success() || private_retry_ref_target(repository_root, private_ref)? != commit
    {
        return Err(SprintRunnerTransitionError::Conflict);
    }
    Ok(())
...
mod implementer_specification_tests {
    ...
    fn application_issued_implementer_prompt_preserves_the_bounded_specification() {
        let specification =
            "Update the exact README marker, verify git status, and create one local-only commit.";
        let prompt = work_unit_implementer_prompt(specification);
...
struct EpicRunnerEscalationReassessmentMcp {
    service: Arc<SprintRunnerTransitionService>,
    invocation_id: AgentInvocationId,
    tool_router: ToolRouter<Self>,
}
impl EpicRunnerEscalationReassessmentMcp {
...
struct EpicRunnerSprintResultReassessmentMcp {
    service: Arc<SprintRunnerTransitionService>,
    invocation_id: AgentInvocationId,
    tool_router: ToolRouter<Self>,
}
impl EpicRunnerSprintResultReassessmentMcp {
    fn new(service: Arc<SprintRunnerTransitionService>, invocation_id: AgentInvocationId) -> Self {
        Self {
            service,
            invocation_id,
...
impl EpicRunnerEscalationReassessmentMcp {
    #[tool(
        description = "Read only the application-correlated accepted Epic plan horizon, current Sprint state, concern, known dependencies, and other available Epic work. Input is ONLY {}."
    )]
    fn read_epic_escalation_reassessment_context(&self) -> CallToolResult {
        match self
            .service
            .epic_escalation_reassessment_context(&self.invocation_id)
        {
            Ok(context) => CallToolResult::success(vec![ContentBlock::text(context.to_string())]),
            Err(SprintRunnerTransitionError::Forbidden) => {
            ...
            Err(_) => CallToolResult::success(vec![ContentBlock::text(
                "{\"status\":\"rejected\",\"code\":\"unavailable\"}",
            )]),
        }
    }
    #[tool(
        description = "Record one identity-free, concern-preserving Epic disposition. It may persist one bounded request to the Sprint Runner or an existing agent-achievable dependency, one human/external attention record, or intent only. It never delivers, activates, starts a Sprint, settles, completes, or accepts."
    )]
    fn record_epic_escalation_disposition(
        &self,
        Parameters(input): Parameters<EpicEscalationReassessmentDisposition>,
    ) -> CallToolResult {
        match self
            .service
            .record_epic_escalation_disposition(&self.invocation_id, input)
        {
            Ok(()) => CallToolResult::success(vec![ContentBlock::text(
                "{\"status\":\"epic_escalation_disposition_recorded\",\"accepted\":false}",
            )]),
            Err(SprintRunnerTransitionError::Forbidden) => {
                CallToolResult::success(vec![ContentBlock::text(
                    "{\"status\":\"rejected\",\"code\":\"forbidden\"}",
...
impl ServerHandler for EpicRunnerEscalationReassessmentMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions("Read the bounded Epic reassessment context, then record one concern-preserving disposition. A request is not delivery, activation, or continuation. No Sprint selection/start, settlement, completion, or acceptance action is available.")
    }
}
#[tool_router]
impl EpicRunnerSprintResultReassessmentMcp {
    #[tool(
        description = "Read only the exact application-correlated Sprint-result reassessment context. Input is ONLY {}."
    )]
    fn read_epic_sprint_result_reassessment_context(&self) -> CallToolResult {
        match self
            .service
            .strict_sprint_result_context(&self.invocation_id)
        {
            Ok(context) => CallToolResult::success(vec![ContentBlock::text(context.to_string())]),
            Err(SprintRunnerTransitionError::Forbidden) => {
                CallToolResult::success(vec![ContentBlock::text(
    ...
    #[tool(
        description = "Record one identity-free, concern-preserving Epic disposition for this exact Sprint result. It never starts a Sprint or settles, completes, or accepts."
    )]
    fn record_epic_sprint_result_disposition(
        &self,
        Parameters(input): Parameters<EpicEscalationReassessmentDisposition>,
    ) -> CallToolResult {
        match self
            .service
            .record_sprint_result_disposition_strict(&self.invocation_id, input)
        {
            Ok(()) => CallToolResult::success(vec![ContentBlock::text(
                "{\"status\":\"epic_sprint_result_disposition_recorded\",\"accepted\":false}",
            )]),
            Err(SprintRunnerTransitionError::Forbidden) => {
                CallToolResult::success(vec![ContentBlock::text(
                    "{\"status\":\"rejected\",\"code\":\"forbidden\"}",
...
#[tool_handler(router=self.tool_router)]
impl ServerHandler for EpicRunnerSprintResultReassessmentMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions("Read one exact Sprint result and record one concern-preserving disposition. No later Sprint action, settlement, completion, or acceptance is available.")
    }
}
...
struct SprintPreStartMcp {
    service: Arc<SprintRunnerTransitionService>,
    invocation_id: AgentInvocationId,
    tool_router: ToolRouter<Self>,
}
impl SprintPreStartMcp {
    fn new(service: Arc<SprintRunnerTransitionService>, invocation_id: AgentInvocationId) -> Self {
        Self {
            service,
            invocation_id,
...
impl SprintPreStartMcp {
    #[tool(
        description = "Record the one pre-start outcome. Input is ONLY {forecastAndConcerns, materialUncertainty, applicationOwnedPrerequisite}; the application derives all identities and accepts it only after the matching terminal lifecycle."
    )]
    fn report_pre_start_outcome(
        &self,
        Parameters(input): Parameters<PreStartOutcome>,
    ) -> CallToolResult {
        match self
            .service
            .record_pre_start_outcome(&self.invocation_id, input)
        {
            Ok(()) => CallToolResult::success(vec![ContentBlock::text(
                "{\"status\":\"pre_start_semantic_outcome_recorded\",\"accepted\":false}",
            )]),
            Err(SprintRunnerTransitionError::Forbidden) => {
                CallToolResult::success(vec![ContentBlock::text(
                    "{\"status\":\"rejected\",\"code\":\"forbidden\"}",
            ...
            Err(_) => CallToolResult::success(vec![ContentBlock::text(
                "{\"status\":\"rejected\",\"code\":\"invalid_or_unavailable\"}",
            )]),
        }
    }
}
#[tool_handler(router=self.tool_router)]
impl ServerHandler for SprintPreStartMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions(
            "Report one pre-start outcome only. Completion alone is not acceptance.",
        )
    }
}
...
impl EpicStartMcp {
    #[tool(
        description = "Semantically authorize the correlated selected Sprint start. Input is ONLY {}. The application derives Epic, Sprint, authority, and all routing identities."
    )]
    fn start_selected_sprint(&self) -> CallToolResult {
        match self.service.start_selected_sprint(&self.invocation_id) {
            Ok(()) => CallToolResult::success(vec![ContentBlock::text(
                "{\"status\":\"sprint_start_authorized\"}",
            )]),
            Err(SprintRunnerTransitionError::Forbidden) => {
                CallToolResult::success(vec![ContentBlock::text(
                    "{\"status\":\"rejected\",\"code\":\"forbidden\"}",
...
#[tool_handler(router=self.tool_router)]
impl ServerHandler for EpicStartMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions(
            "Only start_selected_sprint can authorize start; delivery and lifecycle do not.",
        )
    }
}

struct SprintStartedMcp {
    service: Arc<SprintRunnerTransitionService>,
    invocation_id: AgentInvocationId,
    tool_router: ToolRouter<Self>,
}
impl SprintStartedMcp {
    fn new(service: Arc<SprintRunnerTransitionService>, invocation_id: AgentInvocationId) -> Self {
        Self {
            service,
            invocation_id,
...
impl SprintStartedMcp {
    #[tool(
        description = "Record repository and branch reevaluation after the application-authorized start. Input is ONLY {repositoryBranchEvaluation, startedForecastAndConcerns}. It may make the Sprint planning-ready but cannot create planning or Work Units."
    )]
    fn record_started_reevaluation(
        &self,
        Parameters(input): Parameters<StartedReevaluation>,
    ) -> CallToolResult {
        match self
            .service
            .record_started_reevaluation(&self.invocation_id, input)
        {
            Ok(()) => CallToolResult::success(vec![ContentBlock::text(
                "{\"status\":\"planning_ready\",\"downstreamNotStarted\":true}",
            )]),
            Err(SprintRunnerTransitionError::Forbidden) => {
                CallToolResult::success(vec![ContentBlock::text(
                    "{\"status\":\"rejected\",\"code\":\"forbidden\"}",
...
#[tool_handler(router=self.tool_router)]
impl ServerHandler for SprintStartedMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions(
            "Record only durable repository/branch reevaluation. Do not create downstream work.",
        )
    }
}

struct SprintPlanningControlMcp {
    service: Arc<SprintRunnerTransitionService>,
    invocation_id: AgentInvocationId,
    tool_router: ToolRouter<Self>,
}
impl SprintPlanningControlMcp {
    fn new(service: Arc<SprintRunnerTransitionService>, invocation_id: AgentInvocationId) -> Self {
        Self {
            service,
            invocation_id,
...
impl SprintHandbackReassessmentMcp {
    ...
    fn result(result: Result<(), SprintRunnerTransitionError>, status: &str) -> CallToolResult {
        match result {
            Ok(()) => CallToolResult::success(vec![ContentBlock::text(
                serde_json::json!({"status":status,"accepted":false}).to_string(),
            )]),
            Err(SprintRunnerTransitionError::Forbidden) => {
                CallToolResult::success(vec![ContentBlock::text(
                    "{\"status\":\"rejected\",\"code\":\"forbidden\"}",
...
impl SprintHandbackReassessmentMcp {
    #[tool(
        description = "Read only the application-bound no-progress concern and aggregate current Sprint work state. Input is ONLY {}."
    )]
    fn read_sprint_handback_reassessment_context(&self) -> CallToolResult {
        match self
            .service
            .handback_reassessment_context(&self.invocation_id)
        {
            Ok((_, context)) => {
                CallToolResult::success(vec![ContentBlock::text(context.to_string())])
            }
            Err(error) => Self::result(Err(error), "handback_reassessment_context"),
        }
    }
    #[tool(
        description = "Record one identity-free, concern-preserving next movement. Known movementKind values are continue_eligible_work, wait_for_agent_dependency, and local_exhaustion_escalate; other safe bounded kinds remain extensible. This does not settle the concern or activate an Epic receiver."
    )]
    fn record_sprint_handback_disposition(
        &self,
        Parameters(input): Parameters<SprintHandbackDisposition>,
    ) -> CallToolResult {
        Self::result(
            self.service
                .record_handback_disposition(&self.invocation_id, input),
            "handback_disposition_recorded",
        )
    }
}
#[tool_handler(router=self.tool_router)]
impl ServerHandler for SprintHandbackReassessmentMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions("Read only the bounded concern and aggregate Sprint state; record one concern-preserving movement. No Epic activation, final blockage, settlement, or new work creation is available.")
    }
}

struct WorkSlicePlannerMcp {
    service: Arc<SprintRunnerTransitionService>,
    invocation_id: AgentInvocationId,
    tool_router: ToolRouter<Self>,
}
impl WorkSlicePlannerMcp {
    ...
    fn rejected(error: SprintRunnerTransitionError) -> CallToolResult {
        let code = match error {
            SprintRunnerTransitionError::Forbidden => "forbidden",
            SprintRunnerTransitionError::Invalid => "invalid",
            SprintRunnerTransitionError::Conflict => "conflict",
            SprintRunnerTransitionError::Unavailable(_) => "unavailable",
        };
        CallToolResult::success(vec![ContentBlock::text(
            serde_json::json!({"status":"rejected","code":code}).to_string(),
        )])
    }
}
#[tool_router]
impl WorkSlicePlannerMcp {
    ...
    fn read_current_planning_context(&self) -> CallToolResult {
        match self
            .service
            .read_work_slice_planning_context(&self.invocation_id)
        {
            Ok(context) => CallToolResult::success(vec![ContentBlock::text(context.to_string())]),
    ...
    #[tool(
        description = "Submit a bounded proposal. Input has only objective and proposal-local lanes {title,specification,dependsOn}; no identities, tokens, routes, Work Unit IDs, idempotency key, or acceptance."
    )]
    fn submit_work_slice_proposal(
        &self,
        Parameters(input): Parameters<WorkSliceProposal>,
    ) -> CallToolResult {
        match self
            .service
            .submit_work_slice_proposal(&self.invocation_id, input)
        {
    ...
    #[tool(
        description = "Request bounded refinement of the current uncompleted revision using only {reason}."
    )]
    fn request_work_slice_refinement(
        &self,
        Parameters(input): Parameters<WorkSliceRefinement>,
    ) -> CallToolResult {
        match self
            .service
            .request_work_slice_refinement(&self.invocation_id, input)
        {
            Ok(()) => CallToolResult::success(vec![ContentBlock::text(
                "{\"status\":\"refinement_recorded\"}",
            )]),
            Err(error) => Self::rejected(error),
        }
    }
    #[tool(
        description = "Complete planning only for the application-derived current validated revision. Input is ONLY {}. The application alone later observes lifecycle and accepts it."
    )]
    fn complete_work_slice_planning(
        &self,
        Parameters(input): Parameters<WorkSliceCompletion>,
    ) -> CallToolResult {
        match self
            .service
            .complete_work_slice_planning(&self.invocation_id, input)
        {
            Ok(()) => CallToolResult::success(vec![ContentBlock::text(
                "{\"status\":\"semantic_completion_recorded\",\"accepted\":false}",
            )]),
            Err(error) => Self::rejected(error),
        }
    }
...
#[tool_handler(router=self.tool_router)]
impl ServerHandler for WorkSlicePlannerMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions("Use only the listed application-owned planning actions. No action accepts a revision, creates Work Units, or launches downstream work.")
    }
}

struct WorkUnitHandlerMcp {
    service: Arc<SprintRunnerTransitionService>,
    invocation_id: AgentInvocationId,
    tool_router: ToolRouter<Self>,
}
impl WorkUnitHandlerMcp {
    fn new(service: Arc<SprintRunnerTransitionService>, invocation_id: AgentInvocationId) -> Self {
        Self {
            service,
            invocation_id,
...
impl WorkUnitHandlerMcp {
    #[tool(
        description = "Request the one Implementer for this exact ready Handler invocation. Input is ONLY {}. The application derives and validates attempt, worktree, Harness revision, Session, invocation, and launch authority."
    )]
    fn request_work_unit_implementer(&self) -> CallToolResult {
        match self
            .service
            .request_work_unit_implementer(&self.invocation_id)
        {
            Ok(()) => CallToolResult::success(vec![ContentBlock::text(
                "{\"status\":\"implementer_request_recorded\",\"accepted\":false}",
            )]),
            Err(SprintRunnerTransitionError::Forbidden) => {
                CallToolResult::success(vec![ContentBlock::text(
                    "{\"status\":\"rejected\",\"code\":\"forbidden\"}",
...
#[tool_handler(router=self.tool_router)]
impl ServerHandler for WorkUnitHandlerMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions("Use only request_work_unit_implementer. It cannot submit outcomes or perform review, settlement, retry, dependent activation, or continuation.")
    }
}

struct WorkUnitImplementerReportingMcp {
    service: Arc<SprintRunnerTransitionService>,
    invocation_id: AgentInvocationId,
    tool_router: ToolRouter<Self>,
}
impl WorkUnitImplementerReportingMcp {
    fn new(service: Arc<SprintRunnerTransitionService>, invocation_id: AgentInvocationId) -> Self {
        Self {
            service,
            invocation_id,
...
impl WorkUnitImplementerReportingMcp {
    #[tool(
        description = "Submit only {summary,validationStatement}; these are claims, not evidence. The application derives all identities."
    )]
    fn submit_implementation_outcome(
        &self,
        Parameters(input): Parameters<ImplementationOutcomeClaims>,
    ) -> CallToolResult {
        Self::result(
            self.service
                .submit_implementation_outcome(&self.invocation_id, input),
            "implementation_outcome_recorded",
        )
    }
    #[tool(
        description = "Semantically complete only the already-valid application-bound outcome. Input is ONLY {}."
    )]
    fn complete_implementation_outcome(&self) -> CallToolResult {
        Self::result(
            self.service
                .complete_implementation_outcome(&self.invocation_id),
            "implementation_semantic_completion_recorded",
        )
    }
}
#[tool_handler(router=self.tool_router)]
impl ServerHandler for WorkUnitImplementerReportingMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions("Both identity-free, context-bound reporting tools are available and non-accepting. Claims are not evidence; Handler review remains absent.")
    }
}

struct WorkUnitHandlerReviewMcp {
    service: Arc<SprintRunnerTransitionService>,
    invocation_id: AgentInvocationId,
    tool_router: ToolRouter<Self>,
}
impl WorkUnitHandlerReviewMcp {
    fn new(service: Arc<SprintRunnerTransitionService>, invocation_id: AgentInvocationId) -> Self {
        Self {
            service,
            invocation_id,
...
impl WorkUnitHandlerReviewMcp {
    #[tool(
        description = "Read the exact application-delivered claims and bounded evidence for this review invocation. Input is ONLY {}."
    )]
    fn read_handler_review_evidence(&self) -> CallToolResult {
        match self
            .service
            .handler_review_context(&self.invocation_id, true)
        {
            Ok(context) => {
                CallToolResult::success(vec![ContentBlock::text(context.delivered_payload_json)])
            }
            Err(error) => Self::result(Err(error), "review_evidence"),
        }
    }
    #[tool(
        description = "Accept the exact application-bound implementation outcome. Input is ONLY {} and acceptance remains pending until this exact review invocation is observed Completed."
    )]
    fn accept_implementation_outcome(&self) -> CallToolResult {
        Self::result(
            self.service
                .record_handler_review_judgment(&self.invocation_id, "accept", None),
            "review_acceptance_recorded",
        )
    }
    #[tool(
        description = "Return the exact application-bound incomplete outcome with {code,explanation,classification,meaningfulProgress}. This records a disposition only; it never launches a later attempt or contacts an upward receiver."
    )]
    fn return_implementation_outcome(
        &self,
        Parameters(disposition): Parameters<HandlerReviewIncompleteDisposition>,
    ) -> CallToolResult {
        Self::result(
            self.service
                .record_handler_incomplete_disposition(&self.invocation_id, disposition),
            "incomplete_disposition_recorded",
        )
    }
}
#[tool_handler(router=self.tool_router)]
impl ServerHandler for WorkUnitHandlerReviewMcp {
    fn get_info(&self) -> ServerInfo {
        ServerInfo::new(ServerCapabilities::builder().enable_tools().build()).with_instructions("Only bounded application-delivered evidence and one identity-free accept or structured return judgment are available. Neither action settles work, creates a retry, or activates dependents.")
    }
}
...
start_scoped_server!(
    start_handback_reassessment_server,
    SprintHandbackReassessmentMcp
);
start_scoped_server!(
    start_epic_escalation_reassessment_server,
    EpicRunnerEscalationReassessmentMcp
);
start_scoped_server!(
    start_sprint_result_reassessment_server,
    EpicRunnerSprintResultReassessmentMcp
);
...
start_scoped_server!(
    start_work_unit_implementer_reporting_server,
    WorkUnitImplementerReportingMcp
);
start_scoped_server!(
    start_work_unit_handler_review_server,
    WorkUnitHandlerReviewMcp
);
#[cfg(test)]
...
mod implementer_activation_migration_tests {
    use super::{
        ensure_handler_action_failure_reason, ensure_handler_activation_failure_reason,
        migrate_legacy_implementer_activations,
    };
    ...
    fn incoherent_legacy_activation_rolls_back_then_retries_after_handler_recovery() {
        ...
        let legacy_column: String = connection
            .query_row(
                "SELECT name FROM pragma_table_info('work_unit_implementer_activations')
             WHERE name='implementer_attempt_id'",
                [],
                |row| row.get(0),
            )
            .unwrap();
        ...
        assert_eq!(
            connection
                .query_row::<String, _, _>(
                    "SELECT attempt_id FROM work_unit_implementer_activations",
                    [],
                    |row| row.get(0)
                )
                .unwrap(),
...
mod handler_review_payload_tests {
    ...
    fn review_payload_revalidates_structured_evidence_across_json_key_ordering() {
        ...
        assert_eq!(
            serde_json::from_str::<serde_json::Value>(&sqlite_payload).unwrap(),
            expected
        );
    }
}
...
mod handback_disposition_tests {
    use super::{
        validate_epic_escalation_disposition, validate_handback_disposition,
        AgentAchievableDependencyOwner, EpicEscalationAttention, EpicEscalationDownstreamRequest,
        EpicEscalationDownstreamTarget, EpicEscalationReassessmentDisposition,
        SprintHandbackDisposition, SprintRunnerTransitionError,
    };

    fn disposition(kind: &str) -> SprintHandbackDisposition {
        SprintHandbackDisposition {
            movement_kind: kind.into(),
            rationale: "bounded current concern".into(),
            eligible_work_summary: None,
            dependency_owner: None,
            dependency_owner_classification: None,
    ...
    fn known_handback_movements_preserve_a_bounded_non_human_route() {
        ...
        alternate.eligible_work_summary =
            Some("another eligible Work Unit remains authorized".into());
        ...
        dependency.dependency_owner_classification =
            Some(AgentAchievableDependencyOwner::WorkUnitHandler);
        ...
        exhaustion.local_exhaustion_summary =
            Some("all local Sprint-runner movements are exhausted".into());
    ...
    fn dependency_wait_rejects_human_external_and_unclassified_gates() {
        for owner in ["human approval", "external vendor", "manual confirmation"] {
            ...
            assert!(matches!(
                validate_handback_disposition(&value),
                Err(SprintRunnerTransitionError::Invalid)
            ));
        }
    }
    ...
    fn epic_reassessment_keeps_requests_and_attention_on_separate_safe_routes() {
        let request = EpicEscalationReassessmentDisposition {
            movement_kind: "return_context_to_sprint_runner".into(),
            rationale: "the unresolved concern needs a bounded Sprint decision".into(),
            considered_intent: None,
            downstream_request: Some(EpicEscalationDownstreamRequest {
                target: EpicEscalationDownstreamTarget::SprintRunner,
                dependency: None,
                request: "return missing context only".into(),
                resumption_path: "reassess this concern".into(),
            }),
            human_external_attention: None,
        };
        assert!(validate_epic_escalation_disposition(&request).is_ok());
        let attention = EpicEscalationReassessmentDisposition {
            movement_kind: "human_or_external_attention".into(),
            rationale: "the unresolved concern needs outside authority".into(),
            considered_intent: None,
            downstream_request: None,
            human_external_attention: Some(EpicEscalationAttention {
                reason: "a policy decision is absent".into(),
                authority_needed: "designated product authority".into(),
                evidence_context: "the exact bounded concern and local exhaustion rationale".into(),
                resumption_path: "resume this exact reassessment after that decision".into(),
            }),
        };
        ...
        assert!(matches!(
            validate_epic_escalation_disposition(&unsafe_request),
            Err(SprintRunnerTransitionError::Invalid)
        ));
    }
}
