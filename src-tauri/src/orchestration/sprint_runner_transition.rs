...
impl SprintRunnerTransitionService {
    ...
    fn reconcile_sprint_result_receivers(self:&Arc<Self>)->Result<(),SprintRunnerTransitionError>{let sources:Vec<(String,String,String,String,String,String,String)>=self.connection.lock().map_err(|_|SprintRunnerTransitionError::Unavailable("planning database lock is poisoned".into()))?.prepare("SELECT r.result_id,r.decision_id,r.sprint_id,t.epic_id,t.epic_runner_session_id,t.epic_runner_invocation_id,r.chronology_fingerprint FROM sprint_upward_results r JOIN sprint_continuation_decisions d ON d.decision_id=r.decision_id AND d.sprint_id=r.sprint_id JOIN sprint_runner_transitions t ON t.sprint_id=r.sprint_id AND t.epic_id=(SELECT epic_id FROM initiated_sprints WHERE id=r.sprint_id) LEFT JOIN sprint_continuation_current_decisions current ON current.sprint_id=r.sprint_id LEFT JOIN epic_runner_sprint_result_receivers x ON x.result_id=r.result_id WHERE (x.result_id IS NULL AND current.decision_id=r.decision_id) OR (x.result_id IS NOT NULL AND x.semantic_reassessment_recorded_at IS NULL AND (x.delivery_persisted_at IS NULL OR x.launch_accepted_at IS NULL)) ORDER BY r.recorded_at,r.result_id").and_then(|mut s|s.query_map([],|r|Ok((r.get(0)?,r.get(1)?,r.get(2)?,r.get(3)?,r.get(4)?,r.get(5)?,r.get(6)?)))?.collect::<Result<Vec<_>,_>>()).map_err(|e|SprintRunnerTransitionError::Unavailable(e.to_string()))?;for source in sources{self.reconcile_sprint_result_receiver(source)?;}self.observe_sprint_result_receiver_terminals()}
