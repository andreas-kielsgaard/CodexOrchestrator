...
impl SprintRunnerTransitionService {
    ...
    fn prepare_epic_start_action(self: &Arc<Self>, invocation_id: AgentInvocationId) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        ...
        })?; let injection=CodexMcpInjection::new_named("epic_runner_start",&server.url(),bearer,&["start_selected_sprint".into()],true); let mut active=self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("Sprint Runner action registry is poisoned".into())
        })?;if let Some(existing)=active.get(&invocation_id){let existing=existing.injection.clone();server.stop();return Ok(existing);
        }active.insert(invocation_id,ManagedEpicRunnerAction{server,injection:injection.clone()});Ok(injection)
    }
    fn prepare_started_action(self: &Arc<Self>, invocation_id: AgentInvocationId) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        ...
        })?;let injection=CodexMcpInjection::new_named("sprint_runner_started",&server.url(),bearer,&["record_started_reevaluation".into()],true);let mut active=self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("Sprint Runner action registry is poisoned".into())
        })?;if let Some(existing)=active.get(&invocation_id){let existing=existing.injection.clone();server.stop();return Ok(existing);
        }active.insert(invocation_id,ManagedEpicRunnerAction{server,injection:injection.clone()});Ok(injection)
    }
    fn prepare_planning_control_action(self: &Arc<Self>, invocation_id: AgentInvocationId) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        ...
        })?;let injection=CodexMcpInjection::new_named("sprint_runner_planning_control",&server.url(),bearer,&["request_work_slice_planner".into()],true);let mut active=self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("Sprint Runner action registry is poisoned".into())
        })?;if let Some(existing)=active.get(&invocation_id){let existing=existing.injection.clone();server.stop();return Ok(existing);
        }active.insert(invocation_id,ManagedEpicRunnerAction{server,injection:injection.clone()});Ok(injection)
    }
    fn prepare_handback_reassessment_action(self: &Arc<Self>, invocation_id: AgentInvocationId) -> Result<CodexMcpInjection, SprintRunnerTransitionError> {
        ...
        })?;let injection=CodexMcpInjection::new_named("sprint_runner_handback_reassessment",&server.url(),bearer,&["read_sprint_handback_reassessment_context".into(),"record_sprint_handback_disposition".into()],true);let mut active=self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("Sprint Runner handback action registry is poisoned".into())
        })?;if let Some(existing)=active.get(&invocation_id){let existing=existing.injection.clone();server.stop();return Ok(existing);
        }active.insert(invocation_id,ManagedEpicRunnerAction{server,injection:injection.clone()});Ok(injection)
    }
    fn prepare_epic_escalation_reassessment_action(self:&Arc<Self>,invocation_id:AgentInvocationId)->Result<CodexMcpInjection,SprintRunnerTransitionError>{let bearer=uuid::Uuid::new_v4().simple().to_string();let server=start_epic_escalation_reassessment_server(self.clone(),invocation_id.clone(),bearer.clone(),vec!["tauri://localhost".into()]).map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!("start Epic escalation receiver server: {e}"))
        })?;let injection=CodexMcpInjection::new_named("epic_runner_escalation_reassessment",&server.url(),bearer,&["read_epic_escalation_reassessment_context".into(),"record_epic_escalation_disposition".into()],true);let mut active=self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("Epic escalation receiver registry is poisoned".into())
        })?;if let Some(existing)=active.get(&invocation_id){let existing=existing.injection.clone();server.stop();return Ok(existing);
        }active.insert(invocation_id,ManagedEpicRunnerAction{server,injection:injection.clone()});Ok(injection)}
    fn prepare_sprint_result_reassessment_action(self:&Arc<Self>,invocation_id:AgentInvocationId)->Result<CodexMcpInjection,SprintRunnerTransitionError>{let bearer=uuid::Uuid::new_v4().simple().to_string();let server=start_sprint_result_reassessment_server(self.clone(),invocation_id.clone(),bearer.clone(),vec!["tauri://localhost".into()]).map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!("start Sprint-result receiver server: {e}"))
        })?;let injection=CodexMcpInjection::new_named("epic_runner_sprint_result_reassessment",&server.url(),bearer,&["read_epic_sprint_result_reassessment_context".into(),"record_epic_sprint_result_disposition".into()],true);let mut active=self.mcp.lock().map_err(|_| {
    ...
    fn prepare_work_unit_handler_action(self:&Arc<Self>,invocation_id:AgentInvocationId)->Result<CodexMcpInjection,SprintRunnerTransitionError>{let bearer=uuid::Uuid::new_v4().simple().to_string();let server=start_work_unit_handler_server(self.clone(),invocation_id.clone(),bearer.clone(),vec!["tauri://localhost".into()]).map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!("start Work Unit Handler action server: {e}"))
        })?;let injection=CodexMcpInjection::new_named("work_unit_handler",&server.url(),bearer,&["request_work_unit_implementer".into()],true);let mut active=self.mcp.lock().map_err(|_| {
    ...
    fn prepare_work_unit_implementer_reporting_action(self:&Arc<Self>,invocation_id:AgentInvocationId)->Result<CodexMcpInjection,SprintRunnerTransitionError>{let bearer=uuid::Uuid::new_v4().simple().to_string();let server=start_work_unit_implementer_reporting_server(self.clone(),invocation_id.clone(),bearer.clone(),vec!["tauri://localhost".into()]).map_err(|e| {
        ...
        })?;let injection=CodexMcpInjection::work_unit_implementer_reporting(&server.url(),bearer);if !injection.is_exact_work_unit_implementer_reporting_transport(){server.stop();return Err(SprintRunnerTransitionError::Conflict);
        }let mut active=self.mcp.lock().map_err(|_| {
            SprintRunnerTransitionError::Unavailable("Work Unit Implementer reporting registry is poisoned".into())
        })?;if let Some(existing)=active.get(&invocation_id){let existing=existing.injection.clone();server.stop();return Ok(existing);
        }active.insert(invocation_id,ManagedEpicRunnerAction{server,injection:injection.clone()});Ok(injection)}
    fn prepare_work_unit_handler_review_action(self:&Arc<Self>,invocation_id:AgentInvocationId)->Result<CodexMcpInjection,SprintRunnerTransitionError>{let bearer=uuid::Uuid::new_v4().simple().to_string();let server=start_work_unit_handler_review_server(self.clone(),invocation_id.clone(),bearer.clone(),vec!["tauri://localhost".into()]).map_err(|e| {
            SprintRunnerTransitionError::Unavailable(format!("start Work Unit Handler review server: {e}"))
        })?;let injection=CodexMcpInjection::new_named("work_unit_handler_review",&server.url(),bearer,&["read_handler_review_evidence".into(),"accept_implementation_outcome".into(),"return_implementation_outcome".into()],true);let mut active=self.mcp.lock().map_err(|_| {
    ...
    fn reconcile_no_progress_handback(self: &Arc<Self>, handback: &str, sprint: &str, session: &str, context_fingerprint: &str) -> Result<(), SprintRunnerTransitionError> {
        ...
        match self.sessions.application_invocation_launch_evidence(&invocation, &session).map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))? {
            ...
            ApplicationInvocationLaunchEvidence::PersistedNotAccepted => {
                ...
                self.mark_handback_delivery(handback, "harness_bound_at")?;
                self.mark_handback_delivery(handback, "launch_requested_at")?;
                let launch = self.sessions.launch_prepared_application_invocation_with_launch_observation(SendIdempotentApplicationAgentSessionMessageCommand { invocation_id: invocation, message: SendAgentSessionMessageCommand { session_id: Some(session), submitted_text: "The application delivered one exact no-progress Work Unit concern. Read only the supplied reassessment context, record one truthful next movement, then stop. Continuing eligible work does not settle the concern; do not contact an Epic Runner or declare Sprint/Epic blockage.".into(), title: None, working_directory: Some(conversation_harness::role_discovery_root(ConversationHarnessRole::SprintRunnerHandbackReassessment).map_err(SprintRunnerTransitionError::Unavailable)?), requested_options: Some(harness.runtime_options()) } }, Some(RuntimeLaunchExtension { native_mcp_enabled: None, provider_options: None, managed_mcp_servers: Vec::new(), skill_inputs: Vec::new(), invoked_skill_ids: Vec::new(), ignore_user_rules: false, reasoning_mode: None, config_overrides: args, environment: vec![injection.environment], initial_prompt_prefix: Some(harness.initial_prompt_prefix()) })).map_err(|error| SprintRunnerTransitionError::Unavailable(error.to_string()))?;
