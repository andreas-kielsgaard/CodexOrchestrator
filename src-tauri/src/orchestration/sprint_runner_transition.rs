...
struct WorkUnitImplementerReportingMcp { invocation_id: AgentInvocationId, tool_router: ToolRouter<Self> }
impl WorkUnitImplementerReportingMcp { fn new(_service:Arc<SprintRunnerTransitionService>,invocation_id:AgentInvocationId)->Self{Self{invocation_id,tool_router:Self::tool_router()}} fn unavailable(&self)->CallToolResult{let _=&self.invocation_id;CallToolResult::success(vec![ContentBlock::text("{\"status\":\"rejected\",\"code\":\"unavailable\"}")])} }
#[tool_router] impl WorkUnitImplementerReportingMcp { #[tool(description="Submit the application-bound implementation outcome. This reporting semantic is not yet available.")] fn submit_implementation_outcome(&self)->CallToolResult{self.unavailable()} #[tool(description="Semantically complete the application-bound implementation outcome. This reporting semantic is not yet available.")] fn complete_implementation_outcome(&self)->CallToolResult{self.unavailable()} }
