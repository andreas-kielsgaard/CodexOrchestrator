...
pub(crate) struct CodexAppServerRuntime {
...
impl CodexAppServerRuntime {
    pub(crate) fn system(program: impl Into<String>) -> Self {
        Self::new(program, Arc::new(DuplexProcessFactory))
    }

    pub(crate) fn new(program: impl Into<String>, factory: Arc<dyn ChildProcessFactory>) -> Self {
...
fn initialize_turn(
    ...
    let method = if let Some(external) = external {
        ...
        params["config"]["model_reasoning_effort"] = fresh["reasoningEffort"].clone();
