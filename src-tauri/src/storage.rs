...
mod tests {
    ...
    #[test]
    fn startup_leaves_retired_workflow_tables_and_bytes_intact() {
        let directory = tempfile::tempdir().unwrap();
        let path = directory.path().join("retired.sqlite");
        let connection = open_active_database(&path).unwrap();
        connection.execute_batch("CREATE TABLE workflow_instances(id TEXT PRIMARY KEY, data TEXT); INSERT INTO workflow_instances VALUES('old','retain this'); PRAGMA user_version=44;").unwrap();
        drop(connection);
        let reopened = open_active_database(&path).unwrap();
        assert_eq!(
            reopened
                .query_row(
                    "SELECT data FROM workflow_instances WHERE id='old'",
                    [],
                    |r| r.get::<_, String>(0)
                )
                .unwrap(),
