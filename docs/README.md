...
The guides describe the product at `60c3798` plus the integrated tooling cleanup, with current behavior, responsibility boundaries and the decisions that explain them. Follow the subject you need; historical task logs are not prerequisites.
...
The tooling cleanup is integrated: npm setup, validation command names and recorded Session simulator retirement are reflected in [development](development.md). Its [implementation and validation record](cleanup/tooling-build-cleanup-validation.md) preserves the scope of the checks.
