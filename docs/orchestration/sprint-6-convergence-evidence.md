...
## GD-04 candidate evidence through 076898b

This is a scoped documentation checkpoint/candidate for Plan Slice evaluation; it does not
replace or supersede the preceding Sprint 6 evidence. The application-owned bounded graph drain
reconciles every eligible Handler generation, including multiple roots and later-generation
activation, reopen, and missed-notification convergence. Per-Work-Unit public execution state
uses exactly `waiting_on_prerequisites`, `ready`, `active`, `retry_authorized`, `handed_back`,
`settled`, and `attention`. Malformed graphs, cycles, stalls, invalid handback, missing authority,
and attention paths fail closed; replay preserves terminal facts and identities.

Graph completion means every canonical Work Unit is settled through accepted integration, required
dependency contributions are coherent, and no retry, handback, attention, or unresolved
prerequisite remains. A handback or other terminal-looking outcome is not graph completion. Work
Slice execution-settled is the separate boundary requiring graph completion and durable execution
settlement facts. Planning-point-settled is a separate planning boundary. None implies Sprint or
Epic settlement or continuation.
...
attention, and replay behavior. The terminal-authority fixture uses real initiated Epic/Sprint
materialization services, local Git, and the accepted-candidate, integration, settlement, and
contribution reconcilers. The Rust-authorized canonical frontend fixture through exact normalized
comparison with real `native_query` output, plus TypeScript/UI consumers, proves public
projection. These are complementary proofs; no single fixture proves every boundary.

Focused executed evidence is exact: the Handler-drain test passed **1/1**;
`cargo test --lib work_unit_dependency_wave -- --nocapture` passed **15/15**; the final
terminal-authority Rust test at `076898b` passed **1/1**; and the frontend `nativeQuery` plus
`WorkUnitDetail` fixture consumers passed **28/28**. The retained `db9f3d7` Handler-drain and
retained-lineage gateway checks passed **1/1 each** where cited. These are focused validations,
not broad or full-suite proof.

This scoped evidence adds no Sprint/Epic settlement or continuation, handback consumption or
