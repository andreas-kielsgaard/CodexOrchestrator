...
Future eligible events are `human_interaction`, `agent_session_completed`, `work_unit_approved`, `sprint_completed`, and `epic_completed`. A trigger should record an immutable evidence reference and request compilation asynchronously. Human interaction should mean an application-recognized product stance or review outcome, not every click or message.
...
The current tree is a navigable projection, not the final storage shape. Durable storage should evolve toward nodes for decisions, evidence, candidates, conflicts, and compliance-review requests, with edges such as `derived_from`, `parent_of`, `targets`, `conflicts_with`, and `supersedes`. Agents should query the current canonical subgraph for an Epic and follow provenance or history only when needed.
...
`EpicProductDecisionSource` is the reusable Epic-scoped read boundary. The existing Epic detail flow renders `EpicProductDecisionsPanel` in its contained primary region. Only internal tree, detail, and review regions may overflow-scroll; the outer detail workspace remains fixed.

The adapter and data are recorded development fixtures available through the existing `?recorded-plan-builder` development composition. Product boot does not inject them. The demo proves typed validation, read-state handling, canonical/candidate/conflict separation, provenance presentation, a recorded compliance-review request, and in-app placement. It does not prove AI extraction, durable persistence, automatic reconciliation, product invocation, conflict resolution, graph queries, or codebase audit execution.
