...
Harness Management resolves one relationship by `session_id`. Epic planning, Agent Sessions, and
the management preview consume that session-keyed read model; no view chooses a Harness, assembles
individual settings, or infers a revision. The production adapter can inspect the existing managed
Plan Builder association, but it truthfully reports the applied revision as untracked.
...
Agent identity is injected as Session-owned read data. This convergence does not add assignment,
random generation, browser storage, or durable identity persistence. Avery/sunflower is an explicit
recorded-development fixture used consistently in session lists, workspaces, planning, Harness
Management, and authored-result headings.
...
- The permitted-name summary opens the Harness-specific subset. Editing changes only the recorded
  Harness draft; the Session-owned injected name remains stable.
...
A future `AgentSessionHarnessService` could own creation-time assignment,
`resolveSessionHarness(sessionId)`, and binding commands. That service and its identity-assignment
rules are deferred design only; this convergence neither implements nor accepts durable identity
assignment or persistence.
...
- The recorded identity catalog covers the 100-name pool and Harness visual choices. Avery with the
  sunflower marker is injected explicitly; no assignment engine or identity persistence is added.
- Recorded-source tests cover committed version history, the complete checked-in skill catalog,
  recorded tool/model catalogs, full atomic configuration, stable injected identity across draft pool
...
- The converged `src` aggregate passes 92 files and 638 tests with two workers. TypeScript/Vite
  build, changed-file ESLint, touched-file Prettier, Rust formatting, and diff checks pass. Two
  focused Rust tests cover bound/unbound inspection and durable launch-acceptance truth. The
  unscoped Vitest command also discovers four current-main `node:test` app-inspector files; their
  native runner passes 20 tests, while Vitest reports them as files with no Vitest suite.
