...
Commands below match `package.json`, Vite, Tauri and the retained scripts at `60c3798`. The separate tooling cleanup owns proposed command renames and retirement of the recorded Session page; those changes are not applied here.
...
`npm run install:app` currently runs `npm install`; it is not the lockfile-clean install above. npm is the repository's documented setup route. The leftover pnpm files do not establish a second supported development workflow.
...
| `npm run validate:product`                                           | Builds the frontend, runs those Worktree Review suites and checks Rust. Its broad name does not make it validation of every product capability.                                                 |
| `npm run build:tauri`                                                | Runs `tauri build`, including the configured frontend prerequisite.                                                                                                                             |
| `npm run validate:release`                                           | Explicitly builds the frontend, checks release Rust and invokes `tauri build --no-bundle`. Tauri's build hook also invokes the frontend build; this current overlap belongs to tooling cleanup. |
...
The inspector has its own optional [Node/browser checks](../review-tools/app-inspector/README.md#checks). They are excluded from Vitest and require separate invocation; the browser check launches an installed Chromium browser.
...
| `/agent-session-harness.html`                        | Older standalone Session scenario page, still a Vite build entry at this checkpoint and scheduled for retirement by tooling cleanup. |
