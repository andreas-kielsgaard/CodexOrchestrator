...
The original conversation remains independent. Imports read only the selected local Codex home; unavailable attachments have visible placeholders with original payloads retained. Packaged desktop validation and a hosted-model run were not performed. The documented orphan-fork boundary after an uncertain provider response remains.
