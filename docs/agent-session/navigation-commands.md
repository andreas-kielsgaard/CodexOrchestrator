...
| `{"kind":"reorder_navigation","scope":{"kind":"pinned"},"orderedIds":["SESSION_B","SESSION_A"]}` | Reorder all pinned shortcuts. |
| `{"kind":"reorder_navigation","scope":{"kind":"sessions","folderId":"FOLDER_ID"},"orderedIds":["SESSION_B","SESSION_A"]}` | Reorder sessions in one display container. |
| `{"kind":"set_group_expanded","groupId":"GROUP_ID","expanded":false}` | Collapse Added sessions or Workflow sessions independently. |
| `{"kind":"open_session","sessionId":"SESSION_ID","source":"pinned"}` | Open a pinned shortcut while preserving folder disclosure. |
| `{"kind":"open_workflow","instanceId":"INSTANCE_ID"}` | Navigate to a workflow instance. |
| `{"kind":"open_session_workflow","sessionId":"SESSION_ID"}` | Navigate to the owning workflow and select the session and node. |

`move_session` also accepts `orderedIds`, containing every destination session including the moved session. This saves placement and insertion order in one transaction. Inspection returns `groups` and session `orders` with reusable IDs. Workflow-opening replies include `openedWorkflow`; navigation follows acknowledgement.

Each order must contain all current siblings exactly once. Inspection returns each container's `role`, `parentId`, `order` (scope and item ID), and `orderedSiblingIds`. Use these fields directly. Reordering preserves selection, placement, working directory, and workflow ownership; its saved order survives an application restart. New siblings append in the default name order. Session rows default to activity order and pinned shortcuts to pin time until reordered. Added sessions remain above workflow-owned sessions.

Logical folder IDs are unchanged, including for containers now drawn as blocks. The same projection controls the nested blocks, keyboard traversal, and `visibleRows`. Each block shows five sessions initially, with Added sessions first and one shared limit across expanded workflow groups. Pinned is unlimited.
