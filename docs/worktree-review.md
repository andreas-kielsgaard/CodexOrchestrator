...
4. Click **Build** and confirm the selected source. **Enable debugging** is unchecked for each confirmation; normal builds use release mode. Building produces a retained application and does not launch it.
5. Inspect the recorded result and use **Launch** separately. It requests focus for an existing window using that executable when supported, or launches it independently.
...
Compiler caches persist per worktree and are separate from retained runnable outputs. Builds automatically reuse local artifacts for the selected profile or try the shared compiler cache for a fresh target. New results record Normal or Debugging; older records without mode evidence remain Unspecified. See [build commands and cache clearing](development.md#build-outputs-and-compiler-caches).
...
| Selection, details, build confirmation, and build draft                     | [WorktreeReviewScreen.tsx](../src/features/worktreeReview/WorktreeReviewScreen.tsx), [useReviewSelection.ts](../src/features/worktreeReview/useReviewSelection.ts), [buildDraft.ts](../src/features/worktreeReview/buildDraft.ts), [BuildConfirmationDialog.tsx](../src/features/worktreeReview/BuildConfirmationDialog.tsx) |
