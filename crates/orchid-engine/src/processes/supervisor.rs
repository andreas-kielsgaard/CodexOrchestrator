...
use crate::contracts::domain::{AgentInvocationId, AgentSessionId};
use std::{
    collections::HashMap,
    io,
...
impl ProcessSupervisor {
    pub fn write_input(&self, id: &AgentInvocationId, bytes: &[u8]) -> Result<(), SupervisorError> {
    ...
    pub fn close_input(&self, id: &AgentInvocationId) -> Result<(), SupervisorError> {
        self.input_child(id)?
            .close_input()
            .map_err(|error| SupervisorError::new(SupervisorErrorKind::Internal, error.to_string()))
    }
    ...
    pub fn release_session(&self, id: &AgentInvocationId) -> Result<(), SupervisorError> {
    ...
    pub fn system(sink: Arc<dyn ProcessEventSink>) -> Self {
        Self::new(Arc::new(SystemProcessFactory), sink)
    }

    pub fn new(factory: Arc<dyn ChildProcessFactory>, sink: Arc<dyn ProcessEventSink>) -> Self {
        Self {
    ...
    pub fn cancel(&self, invocation_id: &AgentInvocationId) -> Result<(), SupervisorError> {
        let process = {
            let mut registry = self.lock_registry()?;
    ...
    pub fn shutdown_with_grace_period(
        &self,
    ...
    pub fn active_count(&self) -> Result<usize, SupervisorError> {
        Ok(self.lock_registry()?.active.len())
    }

    pub fn is_active(&self, invocation_id: &AgentInvocationId) -> Result<bool, SupervisorError> {
