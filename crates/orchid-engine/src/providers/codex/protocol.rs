...
pub enum JsonlTerminalEvidence {
    Completed,
    Failed,
    Error,
}
...
pub struct ProtocolOutput {
    ...
    pub terminal: Option<JsonlTerminalEvidence>,
}

#[derive(Default)]
pub struct CodexJsonlProtocol {
    buffer: Vec<u8>,
    line_number: u64,
...
impl CodexJsonlProtocol {
    pub fn push(&mut self, bytes: &[u8]) -> Vec<ProtocolOutput> {
    ...
    pub fn finish(&mut self) -> Vec<ProtocolOutput> {
        let mut outputs = Vec::new();
    ...
    pub fn normalize(&mut self, raw: Value) -> ProtocolOutput {
