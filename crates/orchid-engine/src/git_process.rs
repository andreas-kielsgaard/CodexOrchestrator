...
impl GitExecutable {
    pub fn discover() -> Result<Self, GitProcessError> {
    ...
    pub fn resolve(path: &Path) -> Result<Self, GitProcessError> {
...
pub enum GitCommandEnvironment<'a> {
    Clean,
...
pub struct HardenedGitProcess {
    executable: GitExecutable,
}

impl HardenedGitProcess {
    pub fn new(executable: GitExecutable) -> Self {
        Self { executable }
    }
...
fn minimal_child_path(executable_directory: &Path) -> Result<OsString, GitProcessError> {
    #[allow(unused_mut)] // Windows adds its system executables below.
