...
The inspector uses Node's test runner, not Vitest. It has no additional package dependency for ordinary checks. Use Node.js 24+ on Windows with `powershell.exe`. Run the eight non-browser test files through the npm command:

```powershell
npm run test:app-inspector
```

The separate `npm run test:app-inspector:browser` runs `webview-control-live.test.mjs`, launching installed Microsoft Edge with a disposable profile and local fixture page. It skips when Edge is unavailable; a skip does not verify browser behavior. Windows adapter checks exercise their own local boundaries. Both commands run separately from frontend and Worktree Review validation; neither is a live Codex or full product acceptance test.
