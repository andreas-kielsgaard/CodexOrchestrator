...
async function main() {
  ...
  let text;
  if (action === 'paste') {
    text = await resolveText(options);
  } else if (options.text || options['text-file']) {
    throw new Error('--text and --text-file apply only to paste.');
  }
  const result = await interact({ executablePath, pid, action, x, y, text });
  const receipt = {
...
async function interact({ executablePath, pid, action, x, y, text }) {
  ...
  const { stdout } = await execFileAsync(
    ...
    ],
    {
      ...
      windowsHide: true,
      timeout: 15_000,
      env: text ? { ...process.env, REVIEW_APP_INTERACTION_TEXT: text } : process.env,
    },
  );
...
async function resolveText(options) {
  if (Boolean(options.text) === Boolean(options['text-file']))
    throw new Error('paste requires exactly one of --text or --text-file.');
...
function parseArguments(args) {
  ...
  if (!['click', 'paste'].includes(action))
...
function helpText() {
  return `Codex Orchestrator development desktop interaction companion\n\nTarget one exact running Windows application without foregrounding it:\n  node review-tools/app-inspector/interact-app.mjs click --exe <absolute-path> --pid <pid> --x <client-x> --y <client-y>\n  node review-tools/app-inspector/interact-app.mjs paste --exe <absolute-path> --pid <pid> --x <client-x> --y <client-y> --text-file <utf8-file> --out <receipt.json>\n\nCoordinates are relative to the selected application's main-window client area. The receipt proves only that the target child window acknowledged input messages. It does not prove UI, provider, or application semantics; retain a separate visual or durable observation.\n`;
}
