...
export type ProductNavigationDestination =
  ...
  | { readonly kind: 'workflow'; readonly workflowTypeId: string | null }
  | {
...
export function isProductNavigationDestination(
  ...
  switch (value.kind) {
    ...
    case 'workflow':
      return (
        hasOnlyKeys(value, ['kind', 'workflowTypeId']) &&
...
export function sameProductNavigationDestination(
  ...
  switch (left.kind) {
    ...
    case 'workflow':
      return right.kind === 'workflow' && left.workflowTypeId === right.workflowTypeId;
    case 'agent_sessions':
      return (
