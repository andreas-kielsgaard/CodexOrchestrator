...
function outcome(value: unknown, label: string): EpicPauseRestartOutcome {
  ...
  return { actionId: text(item.actionId, `${label}.actionId`), kind: item.kind, status: item.status as EpicPauseRestartOutcome['status'], targetCount: count(item.targetCount, `${label}.targetCount`), launchedCount: count(item.launchedCount, `${label}.launchedCount`) };
}
