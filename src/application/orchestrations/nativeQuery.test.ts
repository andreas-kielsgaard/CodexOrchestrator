...
describe('orchestration native query v1', () => {
  ...
  it('projects only a correlated initiated Epic with ordered preparatory Sprints and no Work Units', () => {
    ...
    expect(read.epics[0]?.overview).toMatchObject({
      currentMovement: { source: { status: 'unavailable' } },
      state: { source: { status: 'unavailable' } },
      readyWork: { source: { status: 'unavailable' } },
      humanInput: { source: { status: 'unavailable' } },
    });
