...
export type ScopedFileReviewResponse =
...
export type ScopedFileReviewInvoke = (
  command: 'load_scoped_file_review',
  args: { readonly input: { readonly opaqueReference: string } },
) => Promise<ScopedFileReviewResponse>;

const tauriScopedFileReviewInvoke: ScopedFileReviewInvoke = (command, args) =>
  invoke<ScopedFileReviewResponse>(command, args);
...
export function createTauriScopedFileReviewPorts(
  ...
  invokeCommand: ScopedFileReviewInvoke = tauriScopedFileReviewInvoke,
): {
  readonly documents: FileReviewDocumentPort;
  readonly artifacts: StoredFileReviewArtifactPort;
} {
  let cached: Document | undefined;
  const load = async () => {
    const result = await invokeCommand('load_scoped_file_review', { input: { opaqueReference } });
    if (result.status === 'unauthorized')
      throw new FileReviewSourceError(
        'source_unauthorized',
        'The scoped changed-files Document is not authorized.',
      );
    if (result.status === 'invalid')
      throw new FileReviewSourceError(
        'artifact_invalid',
        'The scoped File Review facts are invalid.',
      );
  ...
  return {
    ...
    artifacts: {
      async loadArtifact(request) {
        ...
        if (
          !document ||
          request.documentRefId !== document.documentRefId ||
          request.artifactId !== document.artifactId
        )
          return null;
        return {
          documentRefId: document.documentRefId,
          artifactId: document.artifactId,
          bytes: new Uint8Array(document.payload),
        };
      },
    },
  };
