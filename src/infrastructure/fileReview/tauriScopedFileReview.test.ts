...
const available = { status: 'available' as const, document: { documentRefId: 'doc', title: 'Review', artifactId: 'artifact', payload: [1, 2], changedFiles: [{ changedFileReferenceId: 'file', displayName: 'src/a.ts', changeKind: 'modified' }] } };
...
describe('scoped File Review Tauri adapter', () => {
  it('uses only the opaque reference and maps available facts', async () => {
    ...
    await expect(ports.documents.loadDocument()).resolves.toMatchObject({ documentRefId: 'doc', artifactIds: ['artifact'] });
    expect(invoke).toHaveBeenCalledWith('load_scoped_file_review', { input: { opaqueReference: 'opaque' } });
    await expect(ports.artifacts.loadArtifact({ documentRefId: 'doc', artifactId: 'artifact' })).resolves.toMatchObject({ bytes: new Uint8Array([1, 2]) });
  });
  ...
  it('rejects caller-supplied identity mismatch without widening authority', async () => {
    ...
    await expect(ports.artifacts.loadArtifact({ documentRefId: 'other', artifactId: 'artifact' })).resolves.toBeNull();
    await expect(ports.artifacts.loadArtifact({ documentRefId: 'doc', artifactId: 'other' })).resolves.toBeNull();
  });
});
