...
describe('Tauri Epic Pause/Restart controller', () => {
  it('uses exact commands and validates returned state', async () => {
    const query = {
      epicId: 'epic-1',
      pause: { availability: 'available', reason: 'ready', current: null },
      restart: { availability: 'unavailable', reason: 'not interrupted', current: null },
    };
    const invoke = vi
      .fn()
    ...
    await expect(controller.load('epic-1')).resolves.toEqual({
      epicId: 'epic-1',
      pause: { availability: 'available', reason: 'ready' },
      restart: { availability: 'unavailable', reason: 'not interrupted' },
    });
