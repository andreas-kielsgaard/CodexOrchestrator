...
export function AppSidebar({
  ...
  return (
    <aside className="sidebar" aria-label="Primary navigation">
      ...
      {backendMaintenance && (
        <div
          className={`backend-maintenance ${backendMaintenance.status}`}
          title={backendMaintenance.title}
        >
...
export function RuntimeStaleNotice({ message, onRefresh, onDismiss }: RuntimeStaleNoticeProps) {
  return (
    <section className="notice stale" role="status" aria-label="App notifications">
      ...
      <span>{message}</span>
      <button className="text-button" type="button" onClick={onRefresh} aria-label="Refresh App">
        Refresh
      </button>
      <button
        className="icon-button"
