...
function actionResultMessage(name: string, label: string): string {
  ...
  if (name === 'dangerCanary') return 'Danger Full Access canary request returned; review the durable request, launch, process, provider-activity, receipt, cleanup, and readiness facts. A request return does not prove any of them.';
  if (name === 'authorizeDanger') return 'Danger Full Access authorization was recorded for the current selected profile. It covers full-machine filesystem authority and unrestricted network; launch, UAC, provider activity, and readiness remain separate facts.';
  return `${label} completed; state was refreshed from durable storage.`;
}

function ProfileCard({ profile, busy, onSelect, onLogin, onRefresh, onSelectExecutionMode, onAuthorizeDanger, onRevokeDanger, onSandbox, onConfirmSandbox, onVerifyPreprovisionedSandbox, onConfirmPreprovisionedSandboxAdoption, onCanary, onDangerCanary }: { readonly profile: NativeProfile; readonly busy: string | null; readonly onSelect?: () => void; readonly onLogin?: () => void; readonly onRefresh?: () => void; readonly onSelectExecutionMode?: (mode: NativeProfile['execution']['selectedMode']) => void; readonly onAuthorizeDanger?: () => void; readonly onRevokeDanger?: () => void; readonly onSandbox?: () => void; readonly onConfirmSandbox?: () => void; readonly onVerifyPreprovisionedSandbox?: () => void; readonly onConfirmPreprovisionedSandboxAdoption?: () => void; readonly onCanary?: () => void; readonly onDangerCanary?: () => void }) {
  ...
  return <article className="native-profile-card" aria-label={`Codex home ${profile.id}`}>
    ...
    <dl className="native-profile-facts"><Fact label="Identity" value={profile.id} /><Fact label="Ownership" value={profile.ownership} /><Fact label="Lifecycle" value={profile.lifecycle} /><Fact label="Authentication" value={readiness.authentication} /><Fact label="Sandbox" value={readiness.sandboxInitialization} /><Fact label="Workspace Write canary" value={readiness.workspaceWriteCanary} /><Fact label="Full-access canary" value={readiness.dangerFullAccessCanary} /></dl>
...
function FullAccessCanaryAttempt({ attempt }: { readonly attempt: NativeProfile['fullAccessCanaryAttempt'] }) { return <section className="native-profile-login-attempt" aria-label="Danger Full Access canary attempt"><h3>Danger Full Access canary attempt</h3><p><strong>{fullAccessCanaryDisposition(attempt.disposition)}</strong></p><dl><Fact label="Authorization contract" value={attempt.authorizationVersion ?? 'not recorded'} /><Fact label="Authorization correlation" value={attempt.authorizationCorrelationId ?? 'not recorded'} /><Fact label="Canary correlation" value={attempt.correlationId ?? 'not recorded'} /><Fact label="Requested" value={attempt.requestedAt ?? 'not recorded'} /><Fact label="Launch accepted" value={attempt.launchAcceptedAt ?? 'not observed'} /><Fact label="Deadline" value={attempt.deadlineAt ?? 'not recorded'} /><Fact label="Settled" value={attempt.settledAt ?? 'not settled'} /><Fact label="Process activity" value={attempt.processActivity} /><Fact label="Provider activity" value={attempt.providerActivity} /><Fact label="Receipt observed" value={String(attempt.receiptObserved)} /><Fact label="Cleanup" value={attempt.cleanupDisposition} /><Fact label="Terminal classification" value={attempt.terminalClassification} /><Fact label="Terminal exit code" value={attempt.terminalExitCode === null ? 'not observed' : String(attempt.terminalExitCode)} /></dl><p>Request, launch, process activity, provider activity, receipt, cleanup, and readiness are separate facts. Provider activity is unobserved unless this product has a separate safe observation.</p></section>; }
function fullAccessCanaryDisposition(value: NativeProfile['fullAccessCanaryAttempt']['disposition']) { return ({ not_requested: 'No Danger Full Access canary attempt recorded.', pending: 'Danger canary request is pending a durable process observation.', passed: 'The exact owned receipt was observed and the owned artifact was removed.', launch_failed: 'The Danger canary process was not launched.', terminal_failed: 'The Danger canary did not meet the receipt contract.', timed_out: 'The Danger canary timed out before a terminal outcome was observed.', cancelled: 'The Danger canary was cancelled before a passing result.', recovered_unobserved: 'The Danger canary was recovered without an owned process to observe.', cleanup_failed: 'A receipt may have been observed, but owned-artifact cleanup did not complete.', legacy_unverified: 'A historical canary record lacks the current receipt and cleanup evidence.' } as Record<NativeProfile['fullAccessCanaryAttempt']['disposition'], string>)[value]; }
...
function ExecutionSettings({ profile, busy, onSelectExecutionMode, onAuthorizeDanger, onRevokeDanger }: { readonly profile: NativeProfile; readonly busy: string | null; readonly onSelectExecutionMode?: (mode: NativeProfile['execution']['selectedMode']) => void; readonly onAuthorizeDanger?: () => void; readonly onRevokeDanger?: () => void }) {
  ...
  return <section className="native-profile-execution" aria-labelledby={`execution-${profile.id}`}>
    ...
    <p className="native-profile-danger-warning"><strong>Danger Full Access warning.</strong> This permits commands to read and write outside the assigned worktree and can affect the full machine under the OS rights of the user launching Codex. Unrestricted network access is enabled in this Danger mode. It does not grant administrator rights or suppress Windows UAC.</p>
    <dl className="native-profile-facts"><Fact label="Authorization" value={dangerFullAccessAuthorized ? 'authorized for this selected profile' : dangerAuthorization.disposition.replace('_', ' ')} /><Fact label="Authorization scope" value={dangerAuthorization.authorityScope ?? 'not recorded'} /><Fact label="Authorization contract" value={dangerAuthorization.authorityVersion ?? 'not recorded'} /><Fact label="Authorization correlation" value={dangerAuthorization.correlationId ?? 'not recorded'} /><Fact label="Authorized" value={dangerAuthorization.authorizedAt ?? 'not recorded'} /><Fact label="Revoked" value={dangerAuthorization.revokedAt ?? 'not revoked'} /></dl>
    {dangerFullAccessAuthorized ? <button type="button" disabled={busy !== null} onClick={onRevokeDanger}>Revoke Danger Full Access authorization</button> : <button type="button" disabled={busy !== null || selectedMode !== 'danger_full_access'} onClick={onAuthorizeDanger}>Authorize full-machine access and unrestricted network for this profile</button>}
  </section>;
}
