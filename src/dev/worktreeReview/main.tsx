...
const client = {
  listSources: async () => sources.filter((source) => source.attached),
  listRepositoryHistory: async () => sources,
  ...
  attachWorktree: async (sourceRef: string) => {
    const index = sources.findIndex((source) => source.sourceRef === sourceRef);
    const selected = {
      ...sources[index],
      attached: true,
